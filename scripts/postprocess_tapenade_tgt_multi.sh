...
sed -i -e 's/j, jd, nbdirs/j, jd, npar_opt-nsigma_opt-nmean_opt-nshift_opt-ncorr_opt/g' b2mn_d.F90
sed -i '/&                         , state, stated, state_ext, state_extd, switch&/d' b2mod_driver_diffv.F90
sed -i '/&                         %boris, j, jd, nbdirs)/d' b2mod_driver_diffv.F90
...
sed -i -e '/&              state, stated, state_ext, state_extd, ierr, nbdirs)/a\      CALL B2USR_COST_FUNCTION_DV(ncv, nfc, nvx, ns, geo, geod, mpg, mpgd\&' b2mod_driver_diffv.F90
sed -i -e '/&              state, stated, state_ext, state_extd, ierr, nbdirs)/a\!     manually inserted call to cost function, for output purposes only' b2mod_driver_diffv.F90
sed -i -e "/&                  state_ext, ierr)/a\      end do" b2mod_driver_diffv.F90
sed -i -e "/&                  state_ext, ierr)/a\        write(\*, \*) 'Cost function value '//ss//': ', j(icf)" b2mod_driver_diffv.F90
sed -i -e "/&                  state_ext, ierr)/a\        write(ss, '(I1)') icf" b2mod_driver_diffv.F90
sed -i -e "/&                  state_ext, ierr)/a\      do icf=1,ncf" b2mod_driver_diffv.F90
sed -i -e "/&                  state_ext, ierr)/a\&                             state_ext, switch\%boris, j)" b2mod_driver_diffv.F90
sed -i -e "/&                  state_ext, ierr)/a\      call b2usr_cost_function_nodiff(ncv, nfc, nvx, ns, geo, mpg, state, \&" b2mod_driver_diffv.F90
sed -i -e "/&                  state_ext, ierr)/a\!     manually inserted call to cost function, for output purposes only" b2mod_driver_diffv.F90
...
sed -i -e "/B2USR_COST_FUNCTION_DV/a\&                         %boris, j, jd, nbdirs+nsigma_opt+nmean_opt+nshift_opt+ncorr_opt)" b2mod_driver_diffv.F90
sed -i -e "/B2USR_COST_FUNCTION_DV/a\&                         , state, stated, state_ext, state_extd, switch&" b2mod_driver_diffv.F90
