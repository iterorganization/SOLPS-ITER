function iout=plot_poloidal_arrow(nx,ny,nc1,nc2,nc3,nc4,ntt,nsep,nsep2,PATH_PREFIX,value,Gtitle,Figname,Levels,Plot2DSize,location);
A=(ny+2:-1:1);
...
NNcolours=size(Colours);
Ncolours=NNcolours(1);
...
%                 (  Zc(i,jj+jbeg-1)  - 0.5*( Z2(i,jj+jbeg-1) + Z0(i,jj+jbeg-1) ) )^2);                        
                ...
                icolour=floor( (value(i,j1) - min_Levels) / (max_Levels - min_Levels) * size(Colours,1) ) + 1;
                icolour=max(icolour,1);
                icolour=min(icolour,Ncolours);
                ...
                if location == 'center'
                  if j1 == jbeg
                    plot([Rc(i,j1) 0.5*( R2(i,rightp(i,j1)+2) + R0(i,rightp(i,j1)+2) )], [Zc(i,j1) 0.5*( Z2(i,rightp(i,j1)+2) + Z0(i,rightp(i,j1)+2) )], 'Color',Colours(icolour,:),'LineWidth',1.0);
                  elseif j1 == jend
                    plot([0.5*( R2(i,j1) + R0(i,j1) ) Rc(i,j1) ], [0.5*( Z2(i,j1) + Z0(i,j1) ) Zc(i,j1) ], 'Color',Colours(icolour,:),'LineWidth',1.0);
                  else 
                    plot([0.5*( R2(i,j1) + R0(i,j1) ) Rc(i,j1) 0.5*( R2(i,rightp(i,j1)+2) + R0(i,rightp(i,j1)+2) )], ...
                        [0.5*( Z2(i,j1) + Z0(i,j1) ) Zc(i,j1) 0.5*( Z2(i,rightp(i,j1)+2) + Z0(i,rightp(i,j1)+2) )], 'Color',Colours(icolour,:),'LineWidth',1.0);
                  end;
                elseif location == 'face  '
                    if j1 == jend                      
                       plot([0.5*( R2(i,j1) + R0(i,j1) ) Rc(i,j1)], [0.5*( Z2(i,j1) + Z0(i,j1) ) Zc(i,j1) ], 'Color',Colours(icolour,:),'LineWidth',1.0);                    
                    else
                       plot([0.5*( R2(i,j1) + R0(i,j1) ) 0.5*( R1(i,j1) + R3(i,j1) ) ], [0.5*( Z2(i,j1) + Z0(i,j1) ) 0.5*( Z1(i,j1) + Z3(i,j1) ) ], 'Color',Colours(icolour,:),'LineWidth',1.0);                    
                    end;
                end;
...
%                plot(yy(1,:),yy(2,:),'-r','LineWidth',1.0);            
                line([Rc(i,leftp(i,jend)+2) R0(i,leftp(i,jend)+2)], [Zc(i,leftp(i,jend)+2) Z0(i,leftp(i,jend)+2)],'LineStyle','-','Color',Colours(icolour,:),'LineWidth',1.0);            
                line([Rc(i,leftp(i,jend)+2) R2(i,leftp(i,jend)+2)], [Zc(i,leftp(i,jend)+2) Z2(i,leftp(i,jend)+2)],'LineStyle','-','Color',Colours(icolour,:),'LineWidth',1.0);       
              else              
%                plot(yy(1,:),yy(2,:),'Color',[0.5,0.0,1.0],'LineWidth',1.0);            
                line([Rc(i,jbeg) R1(i,jbeg)], [Zc(i,jbeg) Z1(i,jbeg)],'LineStyle','-','Color',Colours(icolour_beg,:),'LineWidth',1.0);            
                line([Rc(i,jbeg) R3(i,jbeg)], [Zc(i,jbeg) Z3(i,jbeg)],'LineStyle','-','Color',Colours(icolour_beg,:),'LineWidth',1.0);                    
              end; 
            elseif location == 'face  '
              if value(i,jbeg) > 0             
%                plot(yy(1,:),yy(2,:),'-r','LineWidth',1.0);            
                line([R2(i,jend) Rc(i,jend)], [Z2(i,jend) Zc(i,jend)],'LineStyle','-','Color',Colours(icolour,:),'LineWidth',1.0);            
                line([R0(i,jend) Rc(i,jend)], [Z0(i,jend) Zc(i,jend)],'LineStyle','-','Color',Colours(icolour,:),'LineWidth',1.0);       
              else              
%                plot(yy(1,:),yy(2,:),'Color',[0.5,0.0,1.0],'LineWidth',1.0);            
                line([0.5*( R0(i,jbeg) + R2(i,jbeg) ) 0.5*( R2(i,jbeg) + R3(i,jbeg) )], [0.5*( Z0(i,jbeg) + Z2(i,jbeg) ) 0.5*( Z2(i,jbeg) + Z3(i,jbeg) )],'LineStyle','-','Color',Colours(icolour_beg,:),'LineWidth',1.0);            
                line([0.5*( R0(i,jbeg) + R2(i,jbeg) ) 0.5*( R0(i,jbeg) + R1(i,jbeg) )], [0.5*( Z0(i,jbeg) + Z2(i,jbeg) ) 0.5*( Z0(i,jbeg) + Z1(i,jbeg) )],'LineStyle','-','Color',Colours(icolour_beg,:),'LineWidth',1.0);       
              end; 
            end;
...
%for i=nsep+3:ny+2
i=nsep+3;
    plot(R0(i,[1:nc2+2 nc3+2:nx+2]),Z0(i,[1:nc2+2 nc3+2:nx+2]),'-k','LineWidth',0.5,'Color',[0.25,0.25,0.25]);
