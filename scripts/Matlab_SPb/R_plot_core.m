function iout = R_plot_core(length,Plot_Flag,value,ny,nsep,npl,Gtitle,Ylabel);

% R_plot_core plots value versus distance counted from the separatrix
% in a same manner as R_plot does, but only for a core part
...
plot(length(i_begin:i_end,npl+2),value(i_begin:i_end,npl+2),'-sr','LineWidth',2);
