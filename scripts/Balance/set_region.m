...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [indrad,indpol,reverse] = set_region(region_name,geomb2)
...
if length(xcut)==2
    ...
    switch region_name(1:2)
        case {'li','ui'}
            ...
            indpol(2:xcut(1)-1,geomb2.sep+2) = true;
            reverse = true;
        case {'uo','lo'}
            indrad(xcut(2)+1:end-1,2:end-1) = true;
            indpol(xcut(2)+1:end-1,geomb2.sep+2) = true;
            reverse = false;
        otherwise
...
elseif length(xcut)==5
    ...
    switch region_name(1:2)
        ...
        case 'ui'
            ...
            indpol(xcut(2):xcut(3)-1,geomb2.sep+2) = true;
            reverse = false;
        case 'uo'
            indrad(xcut(3)+2:xcut(4),2:end-1) = true;
            indpol(xcut(3)+2:xcut(4),geomb2.sep+2) = true;
            reverse = true;
        case 'lo'
            indrad(xcut(5)+1:end-1,2:end-1) = true;
            indpol(xcut(5)+1:end-1,geomb2.sep+2) = true;
            reverse = false;
        otherwise
            error('Region name ''%s'' not supported.',region_name);
    end
elseif isempty(xcut)
    ...
    switch region_name(1:2)
        case {'li','ui','uo','lo'}
            ...
            indpol(2:end-1,geomb2.sep+1) = true;
            reverse = false;
        otherwise
            error('Region name ''%s'' not supported.',region_name);
    end
