...
btn = radial_balance(...
 ...
 cat(2,raddiv_pll,raddiv_drift,raddiv_nanom,raddiv_panom,raddiv_pinch,raddiv_ch,raddiv_pschused,...
       sum(eirene_mc_papl_sna,3),sum(eirene_mc_pmpl_sna,3),sum(eirene_mc_pipl_sna,3),sum(eirene_mc_pppl_sna,3),sum(eirene_mc_core_sna,3),...
...
if strata_plot
    make_strata_plots({squeeze(eirene_mc_papl_sna)},{squeeze(eirene_mc_pmpl_sna)},{squeeze(eirene_mc_pipl_sna)},{squeeze(eirene_mc_pppl_sna)},...
                      ...
                      {''},comuse,indrad,indpol,nstra,axstrat,axbal,btn.area_divide,pb.area_divide,reverse,false,btn.index,pb.pbCv,pb.pbCvP);
end
%%
end
