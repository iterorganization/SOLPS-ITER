...
raddiv_flua = raddiv(fmox_flua,fmoy_flua,comuse,indrad,facesup,facesdown,'parallel2');
raddiv_visc = raddiv(zeros(size(fmox_cvsa)),fmoy_cvsa,comuse,indrad,facesup,facesdown,'parallel2');
raddiv_hybr = raddiv(zeros(size(fmox_hybr)),fmoy_hybr,comuse,indrad,facesup,facesdown,'parallel2');
%%

%% Calculate the poloidal divergence of the viscous, hybrid correction and new ion viscosity form parts...
raddiv_viscx = raddiv(zeros(size(fmox_cvsa)),fmox_cvsa,comuse,indrad,facesup,facesdown,'parallel2');
raddiv_hybrx = raddiv(zeros(size(fmox_hybr)),fmox_hybr,comuse,indrad,facesup,facesdown,'parallel2');
%%

%% Divergence of the fluxes in indpol
poldiv_flua = div(fmox_flua,fmoy_flua,comuse,facesup_pol,facesdown_pol);
poldiv_visc = div(zeros(size(fmox_cvsa)),fmoy_cvsa,comuse,facesup_pol,facesdown_pol);
poldiv_hybr = div(zeros(size(fmox_hybr)),fmoy_hybr,comuse,facesup_pol,facesdown_pol);

poldiv_viscx = div(zeros(size(fmox_cvsa)),fmox_cvsa,comuse,facesup_pol,facesdown_pol);
poldiv_hybrx = div(zeros(size(fmox_cvsa)),fmox_hybr,comuse,facesup_pol,facesdown_pol);
...
bm = radial_balance(...
 ...
 cat(2,sum(eirene_mc_mapl_smo,3)./apll,sum(eirene_mc_mmpl_smo,3)./apll,sum(eirene_mc_mipl_smo,3)./apll,sum(eirene_mc_cppv_smo,3)./apll,...
 ...
 cat(2,raddiv_viscx,raddiv_hybrx,raddiv_flua,raddiv_visc,raddiv_hybr),...
 resmo./apll,...
 {'total upstream flux',...
...
areadownpol = poloidal_balance(...
 ...
 [],...
 cat(2,poldiv_viscx./apll,poldiv_hybrx./apll,poldiv_flua./apll,poldiv_visc./apll,poldiv_hybr./apll,...
 ...
 {'viscous','hybrid','net radial convected flux','net radial viscous flux','net radial hybrid flux',...
  ...
  'b2stbm','external source',...
  'b2srsm','b2srdt','b2srst','b2stbc','geometric term'},...
 comuse,indpol,facesup_pol,facesdown_pol,ones(nFc),reverse,true,axbal(5:7),'Nm^{-2}','right',polbaldist);
