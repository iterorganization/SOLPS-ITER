...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function baltotpress(SIMID,b2fpstr,indrad,indpol,geomb2,axbal,reverse,strata_plot,axstrat,resaccuracy)
...
if ~isnumeric(SIMID)
    ...
    tmp = reshape(getdata(b2fpstr,[SIMID,'b2fplasma'],'b2sigp_pstat'),nx,ny,ns);
...
areadownrad = radial_balance(...
 ...
 cat(3,visc./apll,raddiv_flua./apll,raddiv_vis./apll,geo_smo./apll,b2stbc_smo./apll,sum(eirene_mc_t0_mapl,4)./apll,...
       sum(eirene_mc_t0_mmpl,4)./apll,sum(eirene_mc_t0_mipl,4)./apll,sum(eirene_mc_t0_cppv,4)./apll,b2stbm_smo./apll,ext_smo./apll,...
       b2stel_smq_ion./apll,b2stel_smq_rec./apll,b2stcx_smq./apll,b2srsm_smo./apll,b2srdt_smo./apll,b2srst_smo./apll,b2sifr_smo./apll,...
       b2siav_smovh./apll,b2siav_smovv./apll,smcf,b2nxdv_smo./apll),...
 resmo./apll,resaccuracy,...
 ...
 {'parallel viscosity','rad. diverg. $nmu_\parallel u_y$','rad. diverg. visc.','$-\frac{1}{\sqrt g}mnu_\parallel^2\frac{\partial}{\partial x}\left(\frac{\sqrt g b_x}{h_x}\right)$','b2stbc',...
  ...
  'b2stel\_rec','b2stcx','b2srsm','b2srdt','b2srst','b2sifr','b2siav\_smovh','b2siav\_smovv','b2sicf','b2nxdv\_smo'},...
 geomb2,indrad,ones(size(apll)),reverse,true,axbal(1:4),'Nm$^{-2}$');
...
areadownpol = poloidal_balance(...
 ...
 {'parallel viscosity','rad. diverg. $nmu_\parallel u_y$','rad. diverg. visc.','$-\frac{1}{\sqrt g}mnu_\parallel^2\frac{\partial}{\partial x}\left(\frac{\sqrt g b_x}{h_x}\right)$','b2stbc',...
  'eirene\_mc atm.-plasma','eirene\_mc mol.-plasma','eirene\_mc t.ion-plasma','eirene\_mc recomb.','b2stbm','source\_input','b2stel\_ion',...
  'b2stel\_rec','b2stcx','b2srsm','b2srdt','b2srst','b2sifr','b2siav\_smovh','b2siav\_smovv','b2sicf','b2nxdv\_smo'},...
 geomb2,indpol,ones(size(apll)),reverse,true,axbal(5:7),'Nm$^{-2}$');

if strata_plot
    make_strata_plots({squeeze(eirene_mc_t0_mapl)./repmat(apll,1,1,size(eirene_mc_t0_mapl,4))},...
                      {squeeze(eirene_mc_t0_mmpl)./repmat(apll,1,1,size(eirene_mc_t0_mmpl,4))},...
                      {squeeze(eirene_mc_t0_mipl)./repmat(apll,1,1,size(eirene_mc_t0_mipl,4))},...
                      {squeeze(eirene_mc_t0_cppv)./repmat(apll,1,1,size(eirene_mc_t0_cppv,4))},...
