...
for i in range(nareg.shape[1]):
  plt.plot(times[:],nareg[:,i,R], label=species[i])
plt.plot(times[:],numpy.sum(nareg[:,:,R],axis=1), label='Sum')

ncol=max(1,nareg.shape[1]/10)
...
if os.getenv('SOLPS_PYTHON_PLOT') is None:
  ...
  plt.savefig('nareg.' + os.getenv('SOLPS_PYTHON_PLOT'))
