...
In Refs.~\cite{nf_niels} and \cite{phd_niels}, it was assumed that $\Ri(\xb)$ and $\Rnx(\xb)$ are user-defined constants, and equal for fast and thermal particles. When pumping in {\tt EIRENE} is imposed  through the {\tt RECYCT} variable (as was done in all published AFN work with fluid-kinetic benchmarks in SOLPS-ITER), then the thermally emitted particles are pumped first, and fast particles are only removed if the pumped fraction is larger than the thermal fraction. It will be explained in Section \ref{sec:pumped_fraction} how $\Ri(\xb)$ and $\Rnx(\xb)$ must be adapted to be compatible with the EIRENE pumping mechanism. For now, we keep the notation identical to Ref. \cite{phd_niels}. 
...
Up till now, it was assumed that the particle reflection coefficients $\Ri$ and $\Rnx$ are independent constants. However, EIRENE assumes that if $0<\Rnx <1$ or $ 0 < \Ri < 1$, first the thermally released particles are pumped away. Fast particles are only pumped if the pumped fraction is larger than the probability of thermal release. This is the case at least when {\tt RECYCT} is used to impose the pumping in {\tt EIRENE}, which is the common practise. Furthermore, there is a subtle difference between the treatment on the fluid and kinetic side. On the kinetic side, the correction for absorption is done on a per-particle basis. On the fluid side, the correction for absorption will be done after the integration over velocity space.
