...
\item Others
  ...
  \item[monoblock\_ht\_model]\index{monoblock\_ht\_model} Runs the monoblock heat transfer model. It requires b2ymb.dat to be present in the run directory. It runs b2plot to produce the heat load profile data of the targets, it runs b2ymb to produce the input files (see \ref{sec:b2ymb}) and finally runs the FreeFem++ heat transfer model of the monoblock.
  ...
  \item[refplate\_ht\_model]\index{refplate\_ht\_model} Runs the reflector plate heat transfer model. It requires b2yrp.dat to be present in the run directory. It runs b2plot to produce the heat load profile data of the RPs, it runs b2yrp to produce the input files (see \ref{sec:b2yrp}) and finally runs the FreeFem++ heat transfer model of the RP.
...
\item[BCENE]\index{BCENE} {\tt integer array, length NBC}, specifying
  ...
  feedback scheme.
  \item[21] constant electron temperature feedback scaled by electron temperature on the ring {\tt bc\_type21\_ref} away,
...
This routine first reads the formatted file 'b2ymb.dat' and writes the unformatted file 'mnb\_in.dat' that is needed by the FreeFem++ heat transfer model of the monoblock.
Then, it reads the files 'wlld\_trg[i-o].dat' produced by b2plot with the "1000 wlld" command and writes the unformatted files 'W\_tg[i-o].dat', 'X\_tg[i-o].dat' and 'tg[i-o]\_size.dat' that are needed  by the FreeFem++ heat transfer model of the monoblock.
In 'W\_tg[i-o].dat' is stored the value of the heat flux for each tile of the monoblock in [W]. 
In 'X\_tg[i-o].dat' is stored the value of the poloidal coordinate of the center of each tile along the monoblock axis (in [m]).
In 'tg[i-o]\_size.dat' is stored the number of points (i.e. tiles) in which the inner/outer monoblocks have been subdivided according to the assigned tile poloidal length, which is read from the 'b2ymb.dat' file.
To use the routine:
\begin{enumerate}
  \item Define in the DG model the plot zone variables for inner and outer targets with labels 'trgi' and 'trgo' respectively
  \item Create the file 'b2ymb.dat' in the run directory (see below for an example)
  \item {\tt echo "1000 wlld" | b2plot} will produce the 'wlld\_trg[i-o].dat' files
  \item Copy the 'wlld\_trg[i-o].dat' files in the run directory
  \item {\tt b2plot b2ymb}
\end{enumerate}

Example of the 'b2ymb.dat' file.\\

\begin{tabular}{llll}
\&GEOMETRY& & &\\
HEIGTH&=& 28.0e-3,&! tile heigth [m]\\
T\_LENGTH& =& 28.0e-3,&! tile toroidal length [m]\\
CU\_TH&=& 1.0e-3,&! Cu interlayer thickness [m]\\
CUCRZR\_TH&=& 1.5e-3,&! CuCrZr tube thickness [m]\\
DIAMETER&=& 12.0e-3,&! CuCrZr tube internal diameter [m]\\
W\_TH&=& 8.0e-3, &! W armour thickness (from plasma facing surface to Cu interlayer) [m]\\
P\_LENGTH&=& 1.2e-2, &! tile poloidal length [m]\\
BETA&=& 1.0, &! poloidal chamfer [degrees]\\
/& & &\\

\&MESH& & &\\
NN&=& 50,&! \# of mesh points on longest edge\\
/& & &\\

\&COOLANT& & &\\
T\_COOLANT&=& 70.0,  &! Coolant inlet temperature [°C]\\
M\_FLOW&=& 0.734,& ! Coolant mass flow rate    [kg/s]\\
CP\_COOLANT&=& 4.22e3,&! Coolant specific heat capacity [J/kg/K]\\
/& & &\\
\end{tabular}
...
This routine first reads the formatted file 'b2yrp.dat' and writes the unformatted file 'rps\_in.dat' that is needed by the FreeFem++ heat transfer model of the reflector plates.
Then, it reads the files 'wlld\_[i-o]rp.dat' produced by b2plot with the "1000 wlld" command and writes the unformatted files 'W\_[i-o]rp.dat', 'X\_[i-o]rp.dat' and '[i-o]rp\_size.dat' that are needed  by the FreeFem++ heat transfer model.
In 'W\_[i-o]rp.dat' is stored the value of the heat flux for each tile of the RPs in [W]. 
In 'X\_[i-o]rp.dat' is stored the value of the poloidal coordinate of the center of each tile along the RPs axis (in [m]).
In '[i-o]rp\_size.dat' is stored the number of points (i.e. tiles) in which the inner/outer RPs have been subdivided according to the assigned tile poloidal length, which is read from the 'b2yrp.dat' file.
To use the routine:
\begin{enumerate}
  \item Define in the DG model the plot zone variables for inner and outer reflector plates with labels 'irp' and 'orp' respectively
  \item Create the file 'b2yrp.dat' in the run directory (see below for an example)
  \item {\tt echo "1000 wlld" | b2plot} will produce the 'wlld\_[i-o]rp.dat' files
  \item Copy the 'wlld\_[i-o]rp.dat' files in the run directory
  \item {\tt b2plot b2yrp}
\end{enumerate}

Example of the 'b2yrp.dat' file.\\
...
In {\tt B2}, we assume $T_a = T_i$ and use $n_i = \sum_a n_a$.
...
& \left\{
\begin{matrix}
-r_{CX_a} n_e n_a, & \ a=1,\ldots,ns-1,z_{a-1}<z_a \text{ and } z_{{n-1}_a}=z_{n_a}\\
0
\end{matrix} \right.
\end{matrix}
\end{equation}
...
max \{ 5; 15,2 - 0,5 \cdot log_e \left( \frac{n_e}{10^{20}} \right) + log_e \left( \frac{T_e}{e \cdot 10^3} \right)  \} && \text{Wesson formula for electron-ion collisions} \\
\end{matrix} \right.
\end{matrix}
$
...
max \{ 5; 23,4 - 1,15 \cdot log_{10} \left( \frac{n_e}{10^6} \right) + 3,45 \cdot log_{10} \left( \frac{T_e}e \right)  \}, & \frac{T_e}{e} \leq 50 & \text{Braginsky formula} \\
max \{ 5; 25,3 - 1,15 \cdot log_{10} \left( \frac{n_e}{10^6} \right) + 2,30 \cdot log_{10} \left( \frac{T_e}e \right)  \}, & \frac{T_e}{e} > 50  \\
max \{ 5; 14,9 - 0,5 \cdot log_e \left( \frac{n_e}{10^{20}} \right) + log_e \left( \frac{T_e}{e \cdot 10^3} \right)  \} && \text{Wesson formula for electron-electron collisions} \\
\end{matrix} \right.
\end{matrix}
$
...
$j_y^{(AN)} = - \sigma_{AN} \frac{\partial \varphi}{h_y \partial y} $
...
\right] \right|_{S_0}$, delete term with max !
...
\frac{\sqrt g}{h_x} e \left. \left[
	...
	\sqrt{\frac{T_e}{m_e}} exp \left( -\frac{e \varphi}{T_e} \right)
...
\frac{\sqrt g}{h_x} e \left. \left[
	...
	\left( 1 - \gamma_e \right) |b_x| n_e \frac 1{\sqrt{2\pi}}
	\sqrt{\frac{T_e}{m_e}} exp \left( -\frac{e \left( \varphi - p_{\varphi,2}\right)}{T_e} \right)
\right] \right|_W$, dpc
...
\frac{
	...
	\frac{3}{2} \sum \limits_{a=0}^{ns-1} \sum \limits_{j=0}^{n_xy-1} \sqrt{g_{i,j}} n_{a,i,j}
...
\frac{
	...
	\frac{3}{2} \sum \limits_{a=0}^{ns-1} \sum \limits_{j=0}^{n_xy-1} \sqrt{g_{i,j}} z_a e n_{a,i,j}
}
\end{equation}
...
(for \SOLPSfive runs) and {\tt fort.44} (when running
...
values stored in the {\tt B2SXDR} file are used.
...
palette contains. Figure~\ref{fig:palette} was produced using the command in
the caption, which loads the 'jet\_16' palette file.

\begin{figure}[hbtp]
  \centering
\ifpdf
  \includegraphics[height=7cm]{palette_example.pdf}
\else
  \includegraphics[height=7cm]{palette_example.eps}
\fi
  \caption{ne te m* ev rm* 1.e-3 rm* 'jet\_16.pal' cltb 0. 30. 16 clvn 'Electron Static Pressure [kPa]' glbl vesl phys surf}\label{fig:palette}
\end{figure}
...
The current default settings of {\tt DIMENSIONS.F}, corresponding to a D+He+C DN case, are:
