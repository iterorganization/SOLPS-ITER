...
\item Others
  ...
  \item[itmsubmit]\index{itmsubmit} Command file to launch runs on the ITM Gateway machine. See Section \ref{b2run} for details.
  ...
  \item[monoblock\_ht\_model]\index{monoblock\_ht\_model} Runs the monoblock heat transfer model. It requires {\tt b2ymb.dat} to be present in the run directory. It runs \btwoplot to produce the heat load profile data of the targets, it runs {\tt b2ymb} to produce the input files (see \ref{sec:b2ymb}) and finally runs the {\tt FreeFem++}\cite{MR3043640} heat transfer model of the monoblock.
  ...
  \item[refplate\_ht\_model]\index{refplate\_ht\_model} Runs the reflector plate (RP) heat transfer model. It requires {\tt b2yrp.dat} to be present in the run directory. It runs \btwoplot to produce the heat load profile data of the RPs, it runs {\tt b2yrp} to produce the input files (see \ref{sec:b2yrp}) and finally runs the {\tt FreeFem++}\cite{MR3043640} heat transfer model of the RP.
...
of {\tt ../}s in the {\tt ../../Makefile} would reflect how many
levels the actual run directory is below the
\${\tt \{SOLPSTOP\}/runs} directory (the {\tt Makefile} is in
\${\tt \{SOLPSTOP\}/runs} and the {\tt Makefile} could also be
accessed via \${\tt \{SOLPSTOP\}/runs/Makefile} in the
...
\begin{description}
  ...
  \item{itmsubmit}\index{itmsubmit} for submissions on the ITM Gateway machine
...
\begin{enumerate}
  ...
  \item {\tt b2run b2co} which will create a {\tt b2fstate} file
...
obtained by means of the command:
\begin{verbatim}
   b2run b2fu
\end{verbatim}
...
\begin{enumerate}
  ...
  \item {\tt b2run b2rd} which will create a {\tt b2fstate} file
  \item {\tt mv b2fstate b2fstati}
...
\begin{description}
  ...
  \item[edena\_int] Integral energy carried by atoms over the entire {\tt Eirene} computational grid, dimension ({\tt NATM}, {\tt 0:NSTRA})
  \item[edenm\_int] Integral energy carried by molecules over the entire {\tt Eirene} computational grid, dimension ({\tt NMOL}, {\tt 0:NSTRA})
  \item[edeni\_int] Integral energy carried by test ions over the entire {\tt Eirene} computational grid, dimension ({\tt NION}, {\tt 0:NSTRA})
  \item[edena\_int\_b2] Integral energy carried by atoms over the {\tt B2.5} computational grid, dimension ({\tt NATM}, {\tt 0:NSTRA})
  \item[edenm\_int\_b2] Integral energy carried by molecules over the {\tt B2.5} computational grid, dimension ({\tt NMOL}, {\tt 0:NSTRA})
  \item[edeni\_int\_b2] Integral energy carried by test ions over the {\tt B2.5} computational grid, dimension ({\tt NION}, {\tt 0:NSTRA})
\end{description}
...
\left. h_z \frac{\sqrt g}{h_y} \Gamma_{ay}^m \right|_{S_1} =
\left. \left[
	p_{m,a,1} h_z  \frac{\sqrt g}{h_y} \frac{\langle B \rangle}B
	-
	h_z \frac{\sqrt g}{h_y} V_{\parallel a}
\right] \right|_{S_1}$, where  $\langle B \rangle =
