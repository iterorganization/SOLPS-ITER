...
The \SOLPSITER Git repository also includes the repositories for {\tt B2.5}, {\tt EIRENE}, {\tt Carre} and {\tt DivGeo} individually as submodules. Some service libraries (such as MSCL and GR), can be found in the repository in the Libraries section.
...
\begin{equation}
\begin{matrix}
\widetilde{ \Gamma}_{ax} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + V_{ax}^{\left( AN \right)} + V^{corr \_ dpc} \right) n_a + 0 \cdot \frac{\delta_{1,a}}{e} \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} + \widetilde{j_x}^{\left( s \right)} \right) \\
...
\begin{equation}
\begin{matrix}
\widetilde{ \Gamma}_{ay} =& \left( V_{ay}^{\left( E \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} + \widetilde{j_y}^{\left( s \right)} \right) \\
...
\begin{equation}
\begin{matrix}
\Gamma_{ax} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + \widetilde{V}_{ax}^{\left( dia \right)} + V_{ax}^{\left( AN \right)} + V^{corr \_ dpc} \right) n_a + \\
0 \cdot \frac{\delta_{1,a}}{e} & \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} + \widetilde{j_x}^{\left( s \right)} \right) - D_{n,a} \frac{1}{h_x} \frac{\partial n_a}{\partial x} - \widetilde{D}_{p,ax} \frac{1}{h_x} \frac{\partial p_a}{\partial x}
\end{matrix}
\end{equation}

\begin{equation}
\begin{matrix}
\Gamma_{ay} =& \left( V_{ay}^{\left( E \right)} + \widetilde{V}_{ay}^{\left( dia \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} + \widetilde{j_y}^{\left( s \right)} \right) \\
...
\begin{equation}
\begin{matrix}
\Gamma_{ax}^{Cor} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + 2 \widetilde{V}_{ax}^{\left( dia \right)} + V_{ax}^{\left( AN \right)} + V^{corr \_ dpc} \right) n_a + \\
...
\begin{equation}
\begin{matrix}
\Gamma_{ax}^{\left( Eir \right)} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + V_{ax}^{\left( dia \right)} + V_{ax}^{\left( AN \right)} + V^{corr \_ dpc} \right) n_a + \\
0 \cdot \frac{\delta_{1,a}}{e} & \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + \widetilde{j_x}^{\left( vis \perp \right)} + 0 \cdot \widetilde{j_x}^{\left( visq \right)} + \widetilde{j_x}^{\left( s \right)} \right) - D_{n,a} \frac{1}{h_x} \frac{\partial n_a}{\partial x}
\end{matrix}
\end{equation}
...
+ \frac{\delta_{1,a}}{e} & \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} + \widetilde{j_y}^{\left( a \right)}  \right) - D_{n,a} \frac{1}{h_y} \frac{\partial n_a}{\partial y}
\end{matrix}
\end{equation}
...
and $\delta_{a,b}$ is the Kronecker delta (1 if $a=b$, 0 otherwise).
...
& \left\{
\begin{matrix}
-r_{I_a} n_e n_a, & \ a=0,\ldots,ns-2, z_a < z_{a+1} \mbox{ and } z_{n_a}=z_{n_{a+1}}\\
...
& \left\{
\begin{matrix}
-r_{R_a} n_e n_a, & \ a=1,\ldots,ns-1, z_{a-1} < z_a \text{ and } z_{{n-1}_a}=z_{n_a}\\
0
\end{matrix} \right.
\end{matrix}
\end{equation}
...
& \left\{
\begin{matrix}
-r_{I_a} n_e n_a \left( \frac{m_a+m_{a+1}}2 \right) V_{\parallel a}, & \ a=0,\ldots,ns-2, & \ z_a < z_{a+1} \mbox{ and } z_{n_a} = z_{n_{a+1}}, \\
0, \\
\end{matrix} \right.
\end{matrix}
\end{equation}
...
& \left\{
\begin{matrix}
r_{I_a} n_e n_a \left( \frac{m_a+m_{a+1}}2 \right) V_{\parallel a}, & \ a=0,\ldots,ns-2, & \ z_a < z_{a+1} \text{ and } z_{n_a} = z_{n_{a+1}}, \\
0, \\
\end{matrix} \right.
\end{matrix}
\end{equation}
...
& \left\{
\begin{matrix}
-r_{R_a} n_e n_a \left( \frac{m_a+m_{a-1}}2 \right) V_{\parallel a}, & \ a=0,\ldots,ns-1, & \ z_{a-1} < z_a \text{ and } z_{n_{a-1}}= z_{n_a}, \\
0, \\
\end{matrix} \right.
\end{matrix}
\end{equation}
...
\frac 4 3 \eta_{ax} \frac{\partial V_{\parallel a}}{h_x \partial x}, & z_a = 1 \\
m_a V_{\parallel a} \Gamma_{ax}^{Cor} -
\frac 4 3 \eta_{ax} \frac{\partial V_{\parallel a}}{h_x \partial x}, & z_a \neq 1 \\
\end{matrix}
\right.
\end{equation}
...
$\eta_{ax} = \eta^{(AN)}_a + \widetilde{\widetilde{\eta}}^{(CL)}_{ax}$
...
$\widetilde{q}^{(0)} = q^{(1)} \arctan \left( \frac{q^{(0)}}{q^{(1)}} \right), \; q^{(1)} = c_{q1} n_i T_i \sqrt{\frac{T_i + T_e}{m_i}}$
...
-\frac 5 2 \frac{B_z B}{B_x \left\langle B^2 \right\rangle} \frac{nT_i}e \frac{\partial T_i}{h_y \partial y}
...
\widetilde f_{iy} = \frac 3 2 \Gamma_{iy} + V_{iy}^{(str)} n_i + \frac 3 2 \frac{\left(
	j_y^{(AN)} + \widetilde j_y^{(vis \parallel)} + 0 \cdot \widetilde j_y^{(vis \perp)} + \widetilde j_y^{(visq)} + \widetilde j_y^{(s)} + j_y^{(in)}
\right)} e
\end{equation*}
...
\text{ , } z_a = 1 \text{ and } z_n =1
\end{equation}

For definition of $q^{(0)}$, see Eqs. (\ref{eq:q0-0}) and (\ref{eq:q0-1}).

\begin{equation}
\widetilde j_x^{(s)} =
- \sigma_{a0} b_z^2 \frac{\partial \Phi}{h_x \partial x}
...
\sigma_{a0} =
\frac{
m_a n_a \left\langle V_{a0} \sigma_{ex} \right\rangle n_0 
}
{2 B^2}
\text{ , } z_a = 1,\;\;\;\;\;\;\;?
\end{equation}
...
\begin{equation}
\left\langle V_{a0} \sigma_{ex} \right\rangle = 3.2 \cdot 10^{-15} \sqrt{\frac{T_i}{0.026e}}
\end{equation}
...
\begin{equation}
\widetilde j_y^{(s)} =
- \sigma_{a0} \frac{\partial \Phi}{h_y \partial y}
...
$\left. \frac{\sqrt g}{h_y} \widetilde q_{iy} \right|_{S_1} = \left. p_{h_i,1} \frac{
	\frac{\sqrt g}{h_y}}
	{\sum \limits_{core} \left( \frac{\sqrt g}{h_y} \right)}
\right|_{S_1}$
