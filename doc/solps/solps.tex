...
\${\tt \{SOLPSTOP\}/lib/\allowbreak}\${\tt \{HOST\_NAME\}.\allowbreak}\${\tt \{COMPILER\}} directory.
...
\begin{description}\index{Compiling the code}\index{B2.5, compiling}\index{SOLPS-ITER, compiling}\index{Eirene, compiling}\index{DivGeo, compiling}\index{Carre, compiling}
  ...
  \item[nc2text\_simple] The {\tt nc2text\_simple} replacement program for systems where {\tt nc2text} is not available (requires the {\tt NetCDF} library) 
...
Contains the Matlab routines used for balance diagnostics.

\subsection{scripts/nc2text\_simple}
\index{nc2text}

Contains the replacement skeleton program for the {\tt nc2text} utility.
...
Contains various Matlab routines used for postprocessing run results when activating the SPb code output switches ({\tt b2wdat\_iout} and the like).
...
\index{QSUB files}\index{SLURM files}
\begin{description}
  ...
  \item{ } Any otherwise named file specified by the user and given as an argument to the submission command (in which case the "-z" option is ineffective). The specified file must be located in the \${\tt \{SOLPSTOP\}/runs} directory, or in the actual run directory on one above it.
\end{description}
...
If {\tt <<Submission\_file>>} is not specified, {\tt itersubmit} looks for {\tt SLURM.iter\_coupled} or
for {\tt SLURM.iter} depending on whether a coupled or a standalone run is required. To request a stand-alone run explicitly, the user can use the "-s" option. Otherwise, the script will assume a coupled run is requested if an {\tt Eirene} {\tt input.dat} file is present in the run directory, and a stand-alone run otherwise.
...
The {\tt SLURM.iter} script first of all changes directory to \${\tt \{SOLPSTOP\}}
...
\item[ADD\_TE\_CORR\_TO\_PO]\index{ADD\_TE\_CORR\_TO\_PO}{\tt logical array, length (NREG+1)},
  indicates whether the electron temperature correction is added to the electric potential correction ({\tt .true.} by default)
\end{description}

When referring to array length {\tt (NREG+1)} above, we mean range {\tt [0:NREG]} in the code.
...
\begin{description}
  \item[LCHKQUD] Check quadrangles switch ???
...
The default strategy depends on the version of the MPI library with which {\tt Eirene} was compiled. If MPI version 3 or greater was used, then the default strategy is the {\bf BALANCED} one. Otherwise, the {\bf ORIGINAL} strategy is chosen.
...
\max \{ 5; 17.3 - 0.5 \cdot \ln \left( \frac{n_e}{10^{20}} \right) + 1.5 \cdot \ln \left( \frac{T_i}{e \cdot 10^3} \right)  \} & \mbox{\scriptsize{Wesson formula}} &\mbox{\scriptsize{for ion-ion collisions}}\\
\end{matrix} \right.
\end{matrix}
\end{equation}
...
\max \{ 5; 23.4 - 1.15 \cdot \log_{10} \left( \frac{n_e}{10^6} \right) + 3.45 \cdot \log_{10} \left( \frac{T_e}e \right) \}, & \frac{T_e}{e} \leq 50 & \text{Braginsky formula} \\
\max \{ 5; 25.3 - 1.15 \cdot \log_{10} \left( \frac{n_e}{10^6} \right) + 2.30 \cdot \log_{10} \left( \frac{T_e}e \right) \}, & \frac{T_e}{e} > 50  \\
\max \{ 5; 17.3 - 0.5 \cdot \ln \left( \frac{n_e}{10^{20}} \right) + 1.5 \cdot \ln \left( \frac{T_i}{e \cdot 10^3} \right) \} && \text{Wesson formula for ion-ion collisions} \\
\end{matrix} \right.
\end{matrix}
$
...
\begin{center}\index{b2plot, plasma quantities}
  \begin{longtable}{|l|l|}
    ...
    ffbz     & \index{b2plot, ffbz} Toroidal magnetic flux function {\em ffbz} $(Wb)$ \\
...
\begin{center}\index{b2plot, stack commands}
  \begin{longtable}{|c|c|c|c|c|c|c|c|c|l|}
    ...
    fpsi \index{b2plot, fpsi}      & 0 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:fpsi}  & Plot data against poloidal magnetic flux ($\Psi$) value \\
    ...
    mpsi \index{b2plot, mpsi}      & 0 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:mpsi}  & Plot data against normalised poloidal magnetic flux ($\psi$) value \\
...
{\bf mpsi} (see \ref{sec:mpsi}) or {\bf rpsi} (see \ref{sec:rpsi})
commands allows for plotting versus the outer midplane (or another position
chosen by the user) radial coordinate, full poloidal magnetic flux,
normalized flux, or $\rho_{pol}$, respectively.
...
If turned on, then turns off {\bf mpsi} and {\bf rpsi}.
...
If turned on, then turns off {\bf fpsi} and {\bf rpsi}.
...
If turned on, then turns off {\bf fpsi} and {\bf mpsi}.
...
HPUX & file-handling/wrmesh.f \\
HPUX & file-handling/wrplam.f \\
HPUX & file-handling/wrplam\_long.f \\
HPUX & file-handling/wrplam\_shrt.f \\
HPUX & file-handling/wrplam\_xdr\_shrt.f \\
HPUX & interfaces/couple\_SOLPS-ITER/eirmod\_extrab25.F90 \\
HPUX & interfaces/couple\_SOLPS-ITER/eirmod\_infcop.F \\
HPUX & user-routines/user\_iter/geomd\_linda.F \\
