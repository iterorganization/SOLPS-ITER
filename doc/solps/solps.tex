...
It is best afterwards to position oneself on the main develop braches, which is achieved by:
...
There is also a dedicated directory \${\tt SOLPSTOP/runs/examples} which contains links to a series of sample cases. These are actually stored in the {\tt /work/imas/shared/boundary/solps-iter/assets/MD5} directory hosted at the IO, and the means to retrieve these cases is detailed in the \${\tt SOLPSTOP/runs/examples/README.md} file. Following the instructions therein will extract an archived file in gzipped tar format containing a {\tt baserun} and one or more run directories and associated files. In some cases, a {\tt DIMENSIONS.F} file is also provided, which shows the dimensions necessary to run those cases (see Appendix \ref{sec:DIMENSIONS} for full details about the contents of such files).
...
\item {\tt SOLPS\_CPP}\index{SOLPS\_CPP}, an environment variable containing the global "defines"
  to use during compilation of {\tt SOLPS} code components
...
\item two aliases to swap between the debug and no\_debug versions of
  the code ({\tt set\_debug} and {\tt set\_nodebug})
\item two aliases to swap between the MPI and no\_MPI versions of
  the code ({\tt set\_mpi} and {\tt set\_nompi})
\end{enumerate}
...
\item[SPUTER] \index{SPUTER} is an {\tt Eirene} input file containing
  sputtering data
\item[TRIM] \index{TRIM} is an {\tt Eirene} input file containing atomic
  physics rate for methane
\end{description}
...
\begin{description}
  ...
  \item[copy\_tracing\_files]\index{copy\_tracing\_files}
...
\end{description}
This option {\em must} also be used if converting from an older {\bf SOLPS4.3} or {\bf SOLPS5.0} case, if the geometry file has been rebuilt, as the sign convention for the parallel velocity now depends on the sign of the magnetic field in \SOLPSITERns.
...
\label{sec:b2_ual_write}\index{b2\_ual\_write}
...
This routine saves the final output state of a run into an IDS.

It reads the {\tt b2mndr\_id*} switchgroup, defined in {\tt b2mn.dat}:
\begin{itemize}
  \item {\tt b2mndr\_user} defines the user value and it should be the same as
    {\tt \$USER} environment variable.
  \item {\tt b2mndr\_device} defines the device variable and it should be
  	{\tt \$DEVICE} environment variable.
  \item {\tt b2mndr\_shot\_number} defines the shot number
  \item {\tt b2mndr\_run\_number} defines the run number
\end{itemize}
These switches are used as identification parameters for IDS. Afterwards, a
b2mn run is being initialized but {\em not} started, so that the final
output state data is read. Then the data is written to IDS, with the
{\tt b2mndr\_id*} switchgroup used as id descriptors.
...
To activate a coupled run, the master switch is 'b2mndr\_eirene' which should be set to '1'. In addition, the densities of the fluid neutral species need to be scaled down, along with the sources that affect them. This is achieved, respectively, by use of the switches 'b2mndr\_rescale\_neutrals' and 'b2mndr\_rescale\_neutral\_sources'. Typically, it is recommended to use a value of the order of $10^{-10}$ for scaling down the fluid neutral sources. For the fluid neutral densities, one can scale them down only on the first run, by the same amount as the sources, and then turn off the rescaling, as they are no longer being replenished.
...
\section{b2\_ual\_write, writing last state to IDS}
Same descriptions as in \ref{sec:b2_ual_write}.
...
\section*{Sign conventions}\index{sign conventions}
...
$c_{q1} = 0.5$ or 'b2siav\_cqip1'  '0.1' in b2mn.dat
...
activating the ``b2mndt\_av'' switch, the {\tt b2time.nc} file will also
...
b2frates \index{b2frates} & un*formatted & b2ar & b2md b2mn b2pl b2yn b2yp b2yr b2yt & atomic physics rates \\
b2fstate \index{b2fstate} & un*formatted & b2co b2mn b2rd & b2fu b2md b2pl b2ts b2uf b2xd b2yi b2yi\_gnuplot b2yp b2yr b2yt & final plasma state \\
