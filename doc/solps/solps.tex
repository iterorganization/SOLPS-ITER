...
\begin{description}
  ...
  \item[NPTCM] This corresponds to the {\tt NMINPTS} flag in input block 7, and is needed only if INDSRC=6, i.e., if the source is specified by data from block 14 alone. As for {\tt NPTC}, only the value from the first stratum segment is used.
  \item[NEMOD] This corresponds to the {\tt NEMODS} flag in input block 7, and is needed only if INDSRC=6, i.e., if the source is specified by data from block 14 alone. As for {\tt NPTC}, only the value from the first stratum segment is used.
