...
+---runs
+---SETUP
|   +---linux.pgi.xt6.openmp
|   +---linux_pgf90_GW_openmp
|   +---linux.Fujitsu
|   +---linux_pgf90
|   +---linux.pgi.xt6
|   +---linux.ifort64.openmp
|   +---HPUX
|   +---linux.lf95
|   +---linux.ifort64
|   +---IBMaix
...
machine architecture ({\it e.g.} \verb!linux.ifort64!).
...
\item \verb!USE_EIRENE!\index{USE\_EIRENE}, an environement variable used by the compiler to
  know whether to compile a coupled code version (when it contains -DB25\_EIRENE)
...
\begin{verbatim}
ln -s SETUP/linux.ifort64/* .
ln -s SETUP/setup.csh.* .
source setup.csh
\end{verbatim}

where \verb!linux.ifort64! gets replaced by the proper value of \$\verb!OBJECTCODE!.
...
%%find $SOLPSTOP/bin -follow -maxdepth 2 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
\newpage\subsection{src/B2.5}
\index{B2.5}

The different versions of \verb!B2.5! are shown below:

%%find $SOLPSTOP/src/B2.5 -follow -maxdepth 1 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
\newpage\subsubsection{src/b2/src}

Within the \verb!src/b2/src! source directory, we have:

%%find $SOLPSTOP/src/B2.5/src -follow -maxdepth 1 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
|   |   +---couple_Dummy
|   |   +---assistant
|   |   +---couple_B2.5_SOLPS-ITER
|   |   +---couple_B2.5_Tria.mpi
|   |   +---plotting
|   |   +---geometry
|   |   |   +---trc-time-routines
|   |   |   +---time-routines
...
set to contain \verb!-DB25_EIRENE!. Then \verb!Eirene! should be compiled with
...
\$\verb!SOLPSTOP/src/DivGeo! is a link (for SOLPS4.x versions).  If it is, do
\index{gtdr, DivGeo}
\begin{verbatim}
  cd $SOLPSTOP/src
  gtdr DivGeo
...
\begin{verbatim}
  sdg
\end{verbatim}
will move you to the \$\verb!SOLPSTOP/data/DivGeo/class/!\$\verb!DEVICE! 
directory. The environment variable \$\verb!DG! contains the path 
\$\verb!SOLPSTOP/src/DivGeo!.
...
and, as a prototypical example, the \verb!compiler.linux.ifort64! in
the \verb!config! subdirectory: \index{B2.5, compiling options}

\VerbatimInput[fontsize=\footnotesize]{B2_5.compiler.linux.ifort64}
...
\section{solps5.1}
\index{GIT repository log, solps5.1}

\VerbatimInput[fontsize=\footnotesize]{solps-iter_git.log}

\section{solps\_data}
\index{GIT repository log, solps\_data}

\VerbatimInput[fontsize=\footnotesize]{solps_data_git.log}
