...
\item Others
  ...
  \item[ippsubmit]\index{ippsubmit} Command file to launch runs on the
    new (2019) IPP cluster. See Section \ref{b2run} for details.
...
are provided:
\begin{description}
  \item{ippsubmit}\index{ippitmsubmit} for submissions on the IPP
    cluster
    \begin{itemize}
    \item the command supports a ``-h'' option to provide help on
      using the script
    \item additional options are:
      \begin{description}
      \item[-j <<JOB\_NAME>>] set the job name (default SOLPS-ITER)
      \item[-z] compress run.log using gzip
      \item[-s] specify a standalone B2.5 run 
      \item[-m <<MPI\_OPTIONS>>] use the MPI options specified (default serial version)
      \item[-T <<hours>>] number of elapsed hours requested (default 24)
      \item[-M <<MEM\_SIZE>>] memory size (default 4GB)
      \end{description}
    \end{itemize}
