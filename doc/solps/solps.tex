...
\item K.~and A.~Kukushkin, the \verb!DG! manual, with revisions by A.~Chin and D.~Stotler
\item M.~Stanojevic, on boundary conditions
\item X.~Bonnin, for many contributions
\end{itemize}
All errors, omissions {\it etc.} should be e-mailed to \verb!David.Coster@ipp.mpg.de!.

\chapter{Introduction}
\label{cha:intro}

This document describes the \SOLPSfiveone code package and its components.
...
version of the \SOLPSfour \btwoplot manual, \ref{cha:Misc}
...
in the \verb!B2.5! part of the \verb!SOLPS! package.

The \verb!SOLPS! (Scrape-Off Layer Plasma Simulation)
code package actually comes in several implementations, according
...
This document is devoted solely to \SOLPSfiveone although several features are
common to multiple versions. Most of the current development and support 
is done exclusively on \SOLPSfive (with the important exception of the ITER
and Forschungszentrum J{\"u}lich groups where \SOLPSfour is still in use). The main
...
from \SOLPSfour to \SOLPSfive and the other way around:
\begin{description}
  \item[From 4 to 5] \verb!b2sxdr! located in \$\verb!{SOLPS4.0}/src/Braams/solps4-5!.
  \item[From 5 to 4] \verb!b2xd!\index{b2xd} located in \$\verb!{SOLPS5.1}/base/b2!.
\end{description}
...
Several of the new {\bf SOLPS4.2} features are detailed in \verb!Kotov_SOLPS42_Report.pdf!.

A Wiki\index{Wiki} for SOLPS can be found at\\
 \href{https://solps-mdsplus.aug.ipp.mpg.de/wiki/SOLPS/}{https://solps-mdsplus.aug.ipp.mpg.de/wiki/SOLPS/}\\
In order to access the Wiki you will need to have obtained a subversion password (see the next section).
 
The next section discusses obtaining and updating the code.

\section{Obtaining and updating the code}
\label{solpsdistribution}\index{Obtaining the code}\index{B2, obtaining}\index{B2.5, obtaining}\index{SOLPS, obtaining}\index{Eirene, obtaining}\index{dg, obtaining}\index{DG, obtaining}\index{DivGeo, obtaining}\index{Carre, obtaining}\index{agg, obtaining}\index{Updating the code}\index{B2, updating}\index{B2.5, Updating}\index{SOLPS, Updating}\index{Eirene, Updating}\index{dg, Updating}\index{DG, updating}\index{DivGeo, Updating}\index{Carre, Updating}\index{agg, Updating}

To obtain the code a request should be sent to David Coster
(\verb!David.Coster@ipp.mpg.de!) in IPP--Garching.  

In addition, the form at\\
\href{https://solps-mdsplus.aug.ipp.mpg.de/SOLPS/SOLPS\_application\_form\_external.pdf}{https://solps-mdsplus.aug.ipp.mpg.de/SOLPS/SOLPS\_application\_form\_external.pdf}\\
should be completed and faxed to +49.89.3299-2580.

Assuming that the formalities have been completed, you will need to obtain access to the subversion repositories.  To do this, the following information is required:
\begin{verbatim}
<<<
For you to be able to access the SOLPS subversion server, you first need to do the 
following:
  a. access the website "https://solps-mdsplus.aug.ipp.mpg.de/passwd.html"
  b. fill out the username and password fields
      if you have an account at IPP, I suggest you use the that username, but it 
      is your choice
      I would suggest that you not use your AFS password, since:
        the password is stored in cleartext in your home file-space
        the password travels across the network in cleartext (if you use the http
        version instead of https)
  c. click on the "Encrypt" button
  d. send me the complete line below the heading "Password Creator" (it should start 
     with your username)

I will then add you to the list of people who can access the repository.

The repository is available at
  https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS/
with the main codes at the level of
  https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS/trunk/

As an example:

15:15> svn list https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS/trunk/
README
solps3.1/
solps4.0/
solps4.2/
...
The currently supported method for distributing the SOLPS suite of
codes is via Subversion.  The repository is at
\href{https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS}{https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS}.
Assuming that
the user wants \SOLPSfiveonens, the following commands should be used:
\begin{verbatim}
svn co https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS/trunk/solps_data solps_data

svn co https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS/trunk/solps_other solps_other

svn co https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS/trunk/solps5.1 solps5.1
\end{verbatim}

The first checks out the ``data'' that is shared by a number of SOLPS
versions; the second checks out some additional shared information;
and the third then checks out the trunk version of {\bf SOLPS5.1}.

It should be possible to obtain a ``tarball'' using the web interface to the SOLPS repository at
\begin{verbatim}
https://solps-mdsplus.aug.ipp.mpg.de/wsvn/SOLPS/trunk/
\end{verbatim}

When using Subversion, ensuring that your local version is current is
trivially achieved using the \verb!svn update! command.

\verb!SOLPS! is a complicated code package and it would be difficult to learn
how to install, compile and run it from this document and the internal
documentation alone.  In the past we have found that an initial visit
of at least two weeks to Garching with subsequent visits, or about two
months and no further visits are desirable.  As mentioned above, if
more than minimal support is required from Garching, an account with
\verb!ssh! access and sufficient disk space for testing would be
needed.

When using distribution methods 1 through 4, some of the pieces of \SOLPSfive rely on the
presence of \SOLPSfour at the same level of the tree (\verb!DivGeo!
and \verb!Eirene! are identical for the two codes).  This means that
if you want to use these parts, \SOLPSfour also needs to be
transferred.  This restriction has been partially lifted by the Subversion scheme,
which instead provides you with a self-consistent tree structure.

Two other useful tools for users and developers are the PlasmaWiki\index{PlasmaWiki}, found at\\
\href{http://solps-mdsplus.aug.ipp.mpg.de/mediawiki/}{http://solps-mdsplus.aug.ipp.mpg.de/mediawiki/}\\
and the mailing lists
\verb!solps-user! and \verb!solps-developer! (both moderated), to which one can
subscribe by sending an email to \verb!majordomo@rzg.mpg.de! containing
\begin{verbatim}
  subscribe solps-user
\end{verbatim}
or 
\begin{verbatim}
  subscribe solps-developer
\end{verbatim}
or both. For all of those, confirmation of membership will be provided by
Garching. \index{solps-user mailing list}\index{solps-developer mailing list}

\section{Requirements}
\index{Requirements}

\begin{description}
\item[Fortran 90/95 Compiler] the main programs are written in \verb!FORTRAN 90!;
  some routines still require a ``-r8'' option
...
If there exists a locally available version of the NAG mathematical library, a copy or
link to it should be put in the \$\verb!{SOLPSTOP}/src/lib/!\$\verb!{OBJECTCODE}! directory,
under the name \verb!libnag.a!. If the library isn't available, then the BLAS and
LAPACK packages should be put instead in the same directory. These latter libraries
are publicly available. When establishing the \verb!SOLPS! environment, the code
will check for the presence of the NAG library and adjust compiler options
accordingly.

The source code or pre-compiled versions of most of the libraries used by \verb!SOLPS! can be found
in the \verb!solps_other! directory, located at the same level as \$\verb!SOLPSTOP! in the SVN
repository and on your installation if you followed the instructions in the previous section.
One can then compile and install all the libraries by invoking, from the \verb!solps_other! directory:
\begin{verbatim}
source install.sh
\end{verbatim}
...
\end{verbatim}
for an installation with a clean of any previously existing compiled files. These scripts require the user
to have properly defined the \verb!FC! environment variable which gives the preferred Fortran compiler.
The default value for this is ``f77'' but that may not be apropriate for all systems. If compiling the
\verb!Sonnet! library, the user should first do:
\begin{verbatim}
fs sysname
\end{verbatim}
and copy the result in \verb!solps_other\sonnet-light\Makefile!. 

There is also a Windows version of \verb!B2.5! that is used in Saint-Petersburg and Paris
but is {\bf not} supported by Garching.
...
\begin{description}
\item[IBM AIX] well supported; used in Garching
\item[SUN] used in Garching
  \begin{description}
  \item[SUN's compiler suite] still somewhat problematical
  \item[Fujitsu] somewhat better
  \end{description}
\item[SGI] has been used in the past, not under active maintenance;
  should not be a problem (except for the plethora of compilation models)
\item[LINUX] used extensively in Garching and at JET
  \begin{description}
  \item[Fujitsu's PC compiler] main one in use at Garching; problem
    with \verb!BLOCK DATA! not being included from library --- workaround in
    place for \verb!NCAR!
  \item[Portland Group PC compiler] in use at JET
  \item[Absoft PC compiler] in use at Culham; strange options required
    --- still being tested
  \item[Intel compiler] recently implemented --- seems to work
  \end{description}
\end{description}
...
The \SOLPSfiveone directory has the following global structure:
...
Two important environment variables are \verb!SOLPSTOP! which points
to the top level directory and \verb!OBJECTCODE! which identifies the
machine architecture ({\it e.g.} \verb!sun5!).

\section{setup.csh}
\index{setup.csh}\label{sec:setup}

\verb!setup.csh! is used to setup
\begin{enumerate}
\item \verb!SOLPSMASTER!\index{SOLPSMASTER}, a variable giving the root of the Master
  distribution, usually it contains either the IPP--Garching address
  (\verb!/afs/ipp-garching.mpg.de/u/dpc!) or the PPPL one 
  (\verb!/afs/pppl.gov/u/dcoster!).  It can also be set to the local
  copy of the Master distribution;
\item \verb!NAG!\index{NAG}, the appropriate loader instruction to find \verb!NAG!
\item \verb!NCARG_ROOT!\index{NCARG\_ROOT}, the root of the \verb!NCAR! tree, and
  \verb!NCARG!, the appropriate loader instruction to load \verb!NCAR!
\item \verb!OBJECTCODE!\index{OBJECTCODE}, a variable indicating the machine
  architecture
\item \verb!nOBJECTCODE!\index{nOBJECTCODE}, a similar variable used by HPZ derived from
  \verb!OBJECTCODE!
\item the \verb!PATH!\index{PATH} variable by prepending some directories
\item \verb!MANPATH!\index{MANPATH} by adding some directories
\item \verb!DEVICE!\index{DEVICE} the name of the fusion device to be simulated (see \ref{cha:divgeo} for available choices)
\item \verb!USE_EIRENE!\index{USE\_EIRENE}, an environement variable used by the compiler to
  know whether to compile a coupled code version
...
\item two aliases to swap between the debug and no debug versions of
  the code (\verb!set_debug! and \verb!set_nodebug!)
\end{enumerate}
The original version of the script assumed everybody had AFS --- since
this proved not to be true various {\it ad hoc} hacks have had to be
introduced, though, over time, it is hoped that the code will become
cleaner.

For setting up the \verb!SOLPSMASTER!, \verb!OBJECTCODE!, \verb!NAG!
and \verb!NCARG! variables, an attempt is made to source
\begin{verbatim}
    ...
    setup.csh.NCARG.guess
\end{verbatim}
in that order.  If the respective variables have not been set, they are
set to some default value.  In the Master tree in Garching only the 
\verb!.guess! files are supplied.  However, the Subversion server does 
hold several examples in the \$\verb!SOLPSTOP/SETUP/!\$\verb!OBJECTCODE! directories.  
At the end of \verb!setup.csh! an attempt is made to source
\begin{verbatim}
    setup.csh.$OBJECTCODE
    setup.csh.local
    ...
    setup.csh.mdsplus
    setup.csh.mdsplus.$OBJECTCODE
\end{verbatim}
%% $
if any of these exist. The \verb!setup.csh.local! file is provided in
the Master tree. For specificities of local implementations, it is thus
recommended to use \verb!setup.csh.local.!\$\verb!OBJECTCODE!, and, for
user-specific settings, \verb!setup.csh.!\$\verb!USER!, where \$\verb!USER!
is the user's login ID. 

All the files above should be stored in the \$\verb!{SOLPSTOP}!/SETUP and
\$\verb!{SOLPSTOP}!/SETUP/\$\verb!{OBJECTCODE}! directories, according to need.
When setting up a tree, one should do, for example:

\begin{verbatim}
ln -s SETUP/linux.Fujitsu/* .
ln -s SETUP/setup.csh.* .
source setup.csh
\end{verbatim}

where \verb!linux.Fujitsu! gets replaced by the proper value of \$\verb!OBJECTCODE!.
...
The data subtree contains data files and directories used by various
programs.
If the user wishes to add data to this list, the recommended
procedure is to create a parallel \verb!data.local! directory, with 
the same structure (only putting in the needed new files and subdirectories),
at the level \$\verb!{SOLPSTOP}/data.local!. The code routinely checks for
...
\item[H2VIBR] \index{H2VIBR} is an \verb!Eirene! input file containing atomic
  physics rates for the vibrational states of hydrogen
\item[HYDHEL] \index{HYDHEL} is an \verb!Eirene! input file containing atomic
  physics rates for hydrogen
...
\item[TRIM] \index{TRIM} is an \verb!Eirene! input file containing atomic
  physics rate for methane
\item[amjuel.2.ps*] \index{amjuel.2.ps} contain PostScript versions of
  the \verb!AMJUEL! data
...
The \verb!adas! subdirectory contains files which are used to tell various programs
...
Contains atomic data for tungsten, following various degrees of bundling.
See documentation within for details.
...
Contains various \btwoplot chords.
...
Contains experimental data. Further subdivided by device.

\subsection{gnuplot}
\index{gnuplot}

Contains various \verb!gnuplot! commands.

\subsection{mdsplus}
\index{mdsplus}

Contains a bare-bones example file of \verb!b2md.dat!.

\subsection{meshes}
\index{meshes}

Contains various meshes for \verb!b2ag! (see section \ref{sec:b2ag}).

\subsection{mesh.extra}
\index{mesh.extra}

Contains standard \verb!mesh.extra! files, used by \btwoplotns.

\subsection{palettes}
\index{palettes}

Contains the palettes that can be used by \btwoplotns.

\subsection{plot\_set}

\subsection{structures}
\index{structures}

Contains standard structure files for use by \verb!agg!, 
\verb!aggs! and \verb!Carre!.

\subsection{Summary\_plots}
...
data used by \verb!SOLPS!. Files ending in \verb!e! are for energy, in
\verb!n! are for particles.

\newpage\section{src}
\index{src}\index{include}
...
The \verb!include! subtree contains global include files.

The \verb!src/b2! subtree contains the source directories
for \verb!B2.5!; \verb!src/Eirene! for the Monte-Carlo code, \verb!src/Carre!
the grid generator, \verb!src/agg! for the automatic grid generator
interface to \verb!Carre! (for ASDEX--Upgrade), \verb!src/aggj! for the automatic grid generator
interface to \verb!Carre! (for JET), \verb!src/DivGeo! for the \verb!DG! graphical interface
to the grid preprocessor, and \verb!src/Triang! for the \verb!Eirene! triangular mesh generator.

\newpage\section{bin}
\index{bin}

%%find $SOLPSTOP/src/bin -follow -maxdepth 2 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'

The \verb!bin! subtree contain programs (binaries and shell
scripts) used for compiling as well as for analyzing runs.
...
|   +---linux.Intel.debug
|   +---linux.Intel_EM
|   +---linux.Intel_EM.debug
|   +---linux.g95.debug
|   +---linux.g95
|   +---linux.gfortran.debug
|   +---linux.gfortran
|   +---linux.ifort32
|   +---linux.ifort32.debug
|   +---linux.ifort64
...
The command\index{sbin}
\begin{verbatim}
    sbin
\end{verbatim}
will take you to \$\verb!{SOLPSTOP}/bin/!\$\verb!{OBJECTCODE}!. This directory contains all 
machine-specific implementations of analysis and post-processing scripts. Non-machine-specific 
scripts can be found in the \$\verb!{SOLPSTOP}/bin/common! directory.
Again, a user wanting to create local scripts is encouraged to create a
parallel \$\verb!SOLPSTOP/bin.local! directory, with subdirectories
\verb!common! and \$\verb!{OBJECTCODE}! in which to store any personal scripts
or modifications to the provided ones. The \verb!PATH! environment variable
already includes those two subdirectories (superceding the Master copies)
in the standard setup. It is also best to create a shadow link for debug runs
by means of:
\begin{verbatim}
unset_debug
cd $SOLPSTOP/bin.local
ln -s $OBJECTCODE $OBJECTCODE.debug
\end{verbatim}
%%$

\newpage\subsubsection{src/common}
\label{sec:bincommon}

The command\index{sbinc}
\begin{verbatim}
    sbinc
\end{verbatim}
will take you to \$\verb!{SOLPSTOP}/bin/common!. 
...
negative quantities on the same scale and ``M'' the absolute values of negative quantities in a separate plot)
  ...
  \item[fluxt \#1]\index{fluxt} Gives the recycling fluxes for the first \#1 \verb!Eirene! strata. The default value is all the strata, as read from the \verb!b2.neutrals.parameters! file, or if not present, the \$\verb!SOLPSTOP/include.local/DIMENSIONS.F! file, or if not present, the \$\verb!SOLPSTOP/include/DIMENSIONS.F! file.
...
\item Others
  ...
  \item[b2plot]\index{b2plot} Shortcut to \btwoplotns. A ``-n'' option allows for dry-run tests.
  ...
  \item[b2run]\index{b2run} Shortcut to the \verb!Makefile! launching \verb!B2.5!. A ``-n'' option allows for dry-run tests.
  ...
  \item[b2sxdr]\index{b2sxdr} Shortcut to the \SOLPSfour to \SOLPSfive
    converter. A ``-n'' option allows for dry-run tests.
  ...
  \item[update\_data]\index{update\_data} Updates the \$\verb!SOLPSTOP/data! directory.
  \item[update\_eirene]\index{update\_eirene} Updates the \$\verb!SOLPSTOP/src/Eirene! directory.
...
%%find $SOLPSTOP/src/lib -follow -maxdepth 2 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'

The \verb!lib! subtree contains libraries used during compilation.

The \verb!bin! and \verb!lib! subtree have similar structures:
...
\begin{verbatim}
    slib
\end{verbatim}
will take you to \$\verb!{SOLPSTOP}/lib/!\$\verb!{OBJECTCODE}!.

\newpage\subsection{src/b2}
\index{B2.5}

The different versions of \verb!B2.5! are shown below:

%%find $SOLPSTOP/src/b2 -follow -maxdepth 1 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
\item[config.local] \index{config.local} contains local modifications to the compiler option files
\item[local] \index{src.local}\index{B2.5,
    local}\label{src.local} contains locally modified files
\item[xpb] \index{xpb}\index{B2.5, xpb}\label{xpb}
  contains Xavier Bonnin's merging of {\bf SOLPS5.0} with additional
  physics and corrections
\item[current]\index{B2.5, current} a link to the actual version of the code to be used ---
  usually a pointer to \verb!xpb!
\end{description}

A more varied selection of \verb!B2.5! historical versions is available within the {\bf SOLPS5.0} distribution.

The command\index{ssb}
\begin{verbatim}
    ssb
\end{verbatim}
will take you to \$\verb!{SOLPSTOP}/src/b2!.
...
When people establish a shadow tree, they usually just have a link to
the \verb!B2.5! source, and {\bf this is the only supported option!}  In the
past if you wanted to make changes to the source, you needed to use
\verb!gtdr! to establish a local shadow of the source, and then
\verb!gtfl! to make a particular file local so that it could be
modified.  {\bf This is no longer supported!}  The presently supported
way of making local changes is to copy the file you want to change to
\verb!local! and then edit that file.  You will then need to do
\begin{verbatim}
  sbb
  gmake listobj
  gmake tags
  ...
  gmake
\end{verbatim}
so that the local file is used.  If this local change should no longer
be needed, then it should be removed from \verb!local! and the
following performed:
\begin{verbatim}
  sbb
  gmake clean
...
If you have performed the \verb!gtdr/gtfl! operations on a source
file, and you expect Garching support, you should move your changed
files to \verb!local!, move \verb!current! to \verb!old! and
re-establish the link of \verb!current! to the Master version.

\newpage\subsubsection{src/b2/*}

Within any one of these \verb!src/b2/*! source versions, we
have:

%%find $SOLPSTOP/src/b2/current -follow -maxdepth 1 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
\newpage
\subsection{src/Eirene}
\index{Eirene}\label{cha:eirene}

%%find $SOLPSTOP/src/Eirene -follow -maxdepth 2 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
The command\index{sse}
\begin{verbatim}
    sse
\end{verbatim}
will take you to \$\verb!{SOLPSTOP}/src/Eirene!.

The usual version of \verb!B2.5! is not compiled with \verb!Eirene!.  To compile the
coupled version, the environment variable \verb!USE_EIRENE! should be
set to contain \verb!-DUSE_EIRENE!. Then \verb!Eirene! should be compiled with
\index{Eirene, compilation}\index{USE\_EIRENE}
\begin{verbatim}
    gmake
\end{verbatim}
followed by the compilation of \verb!B2.5!.

If routines have been added or removed, if the dependencies have been
altered or if this is the first compilation in a new tree, you should do
...
of a \verb!config.local! subdirectory is recommended, in which to
store \verb!compiler.!\$\verb!OBJECTCODE! files. Since the \verb!Makefile!
reads those after the \verb!config/compiler.!\$\verb!OBJECTCODE! file,
...
The compilation of \verb!B2.5! is done in \verb!base/b2! (see also
section \ref{sec:compilingb2}, page \pageref{sec:compilingb2}).

%%find $SOLPSTOP/base/b2 -follow -maxdepth 1 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
The command\index{sbb}
\begin{verbatim}
    sbb
\end{verbatim}
will take you to \$\verb!{SOLPSTOP}/base/b2!.

Contrary to previous {\bf SOLPS} versions, it is now required to
compile \verb!Eirene! first before compiling \verb!B2.5!. See
\ref{cha:eirene}.

If routines have been added or removed, if the dependencies have been
altered or if this is the first compilation in a new tree, you should do
\index{B2.5, compilation}
\begin{verbatim}
    gmake depend
    gmake listobj
    gmake tags
...
Normally compilation is achieved with
\begin{verbatim}
    gmake
\end{verbatim}

Note that if you want to use the coupled version of the code, you
should compile \verb!Eirene! first (see chapter \ref{cha:eirene}).
...
\newpage
\subsection{base/Eirene}
\label{sec:baseEirene}

The compilation of \verb!Eirene! is done in \verb!base/Eirene!.

%%find $SOLPSTOP/base/Eirene -follow -maxdepth 1 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
The command\index{sbe}
\begin{verbatim}
    sbe
\end{verbatim}
will take you to \$\verb!{SOLPSTOP}/base/Eirene!.
...
The runs for \verb!B2.5! are done in \verb!runs!.
...
\$\verb!{SOLPSTOP}/runs!.  Sometimes, as in the example of
\verb!bench_mark! shown above, further subdirectories are created to
...
(typified here by the \verb!ibmtok1! directory).  The \verb!baserun!
directory contains the defaults for all of the other directories at
its same level, and typically \verb!b2ag!, \verb!b2ah!, \verb!b2ar!
...
The compilation of \verb!B2! is described in section \ref{sec:b2} on page
\pageref{sec:b2} with more detail given in section \ref{sec:compilingb2}, page \pageref{sec:compilingb2}.

The run directory is described in section \ref{sec:b2runs} on page
\pageref{sec:b2runs}. It is accessible with the \verb!sbr! command.
...
Another option has now been added which is to use
\begin{verbatim}
    b2run b2XX
\end{verbatim}
This automatically does the \verb!gmake! for you, finding the \verb!Makefile!
for itself by climbing up the tree.
...
\item[*param] at least 100 additional numbers, of which only the first
  is relevant for us
  \begin{description}
  \item[-1.0] read the mesh data using the ``simplified'' format
...
or to the write-up from one of Kuhn's group (see Appendix \ref{cha:Mladen}).
...
particle transport and radial current can be established. If the transport is given by
...
\verb!baserun! directory where \verb!b2ar! is run.
...
each homonuclear sequence, as well as a \verb!b2yr.prt! ASCII file containing
...
the scripts described in section \ref{sec:bincommon} can be run.
...
for \verb!Eirene! before compilation.
...
\$\verb!SOLPSTOP/include/DIMENSIONS.F!. If the dimensions are not right for you,
...
\end{verbatim}
%% $
and edit the file \$\verb!SOLPSTOP/include.local/DIMENSIONS.F! so that the 
dimensions (mainly used by \verb!Eirene!) are
big enough (see the description of the relevant variables in section \ref{sec:DIMENSIONS}).

Then
\begin{verbatim}
  setenv USE_EIRENE '-DUSE_EIRENE'
  sei
  gmake depend
  gmake
  sbb
  gmake
\end{verbatim}
If you have previously compiled \verb!B2.5! in the stand-alone mode, you
should also do
\begin{verbatim}
  sbb
  rm ${OBJECTCODE}/b2stbr.? ${OBJECTCCODE}/b2mndr.? ${OBJECTCODE}/b2plot.? ${OBJECTCODE}/b2mds.? ${OBJECTCODE}/b2mwti.?
  gmake
\end{verbatim}
%% $
...
Some differences between the \SOLPSfiveone and {\bf SOLPS5.0} behaviours
should be noted here.

First, the number of strata \verb|NSTRAI| need no longer include the time-dependent ``T'' stratum.

Second, the array sizes of \verb|chemical_sputter_yield| now need to match the 
...
Third, in \SOLPSfiveonens, the North and South boundaries of the \verb!B2.5! grid are 
...
In order to use this facility, you need to ask the author of this document to
...
\end{verbatim}
to one of the \verb!setup.csh.*! files in \$\verb!SOLPSTOP!.]
...
in order. See Appendix \ref{lis:b2cdcn} for a content description.
...
In another, more complicated, example, python was used to create the frames of a movie on the Gateway machine.
...
The server is a dual processor 2.8~GHz AMD Opteron machine with 4~GB RAM
...
When using for the first time, check to see whether
\$\verb!SOLPSTOP/src/DivGeo! is a link.  If it is, do
\index{gtdr, DivGeo}
\begin{verbatim}
  cd $SOLPSTOP/src
  gtdr DivGeo
\end{verbatim}
%% $
to make yourself the owner of the \verb!DivGeo! directory.
The files will remain links to the Master.
...
\item[dream] (Japan)
\item[ht7u] HT7U, now EAST (Hefei, China)
...
The command\index{sdg}
\begin{verbatim}
  sdg
\end{verbatim}
will move you to the \$\verb!SOLPSTOP/src/DivGeo/dg/class/!\$\verb!DEVICE! 
directory.
...
When using for the first time, check to see whether
\$\verb!SOLPSTOP/src/Carre! (accessible with the \verb!ssc! command)
is a link.  If it is, do
\index{gtdr, Carre}
\begin{verbatim}
  cd $SOLPSTOP/src
  gtdr Carre
...
You will need to do \index{Carre, compiling}
\begin{verbatim}
  ssc
  gmake local
  gmake tags
  ...
  gmake
\end{verbatim}
so that output files that were links to the Master are either removed
or are made local and compile the code.
...
When using for the first time, check to see whether
\$\verb!SOLPSTOP/src/agg! is a link.  If it is, do
\index{gtdr, agg}
\begin{verbatim}
  cd $SOLPSTOP/src
  gtdr agg
...
\begin{figure}[bt]
\ifpdf 
    \centerline{\includegraphics[width=11cm]{cflmi_1e19.pdf}} %%{cflim_1e19.pdf}}
\else
    \centerline{\includegraphics[angle=-90,width=11cm]{cflmi_1e19.ps}} %%{cflim_1e19.epsi}}
\fi
...
\begin{figure}[bt]
\ifpdf 
    \centerline{\includegraphics[width=11cm]{cflmi_2e19.pdf}} %%{cflim_2e19.pdf}}
\else
    \centerline{\includegraphics[angle=-90,width=11cm]{cflmi_2e19.ps}} %%{cflim_2e19.epsi}}
\fi
...
Generates the spectral line emissivities according to the \verb!ADAS! database \cite{ADAS}, stored in \$\verb!SOLPSTOP/data/adas!
...
\$\verb!SOLPSTOP/data/palettes! are searched in order.
...
\$\verb!SOLPSTOP/data/commands! are searched in order.
...
from the files stored in \$\verb!SOLPSTOP/data/adas! or \$\verb!SOLPSTOP/data.local/adas!.
...
The file \$\verb!SOLPSTOP/include/DIMENSIONS.F! and its local version
\$\verb!SOLPSTOP/include.local/DIMENSIONS.F! contain key dimensions 
that determine
...
additional cells outside the plasma domain in \verb!Eirene!
...
passed from \verb!B2! or \verb!B2.5! to \verb!Eirene!
\item[DEF\_NION]\index{DEF\_NION}\index{NION} the maximum number of test
...
\item[DEF\_NPLS]\index{DEF\_NPLS}\index{NPLS} the maximum number of bulk
...
species handled in \verb!B2! or \verb!B2.5!
...
transferred from \verb!B2! or \verb!B2.5! (\verb!DG! specification "Surface special")
...
\verb!Eirene! strata
\item[DEF\_NSTS]\index{DEF\_NSTS}\index{NSTS} the maximum number of non
default standard surfaces to be handled by \verb!Eirene!
...
\VerbatimInput[fontsize=\footnotesize]{B2.Makefile}

and, as a prototypical example, the \verb!compiler.linux.Fujitsu! in
the \verb!config! subdirectory: \index{B2.5, compiling options}

\VerbatimInput[fontsize=\footnotesize]{B2.compiler.linux.Fujitsu}
...
%%grep #if $SOLPSTOP/src/b2/current/*/*.F | awk '{print $2}' | sort | uniq

\begin{description}
\item[AIX] \index{AIX} Specific to AIX compilers
...
\item[EASY] \index{EASY} Uses an ``easier'' model for chord integration
\item[Fortran\_90] index{Fortran\_90} Specific to \verb!FORTRAN 90! compilers
...
\item[NO\_B2\_CHEM\_SPUT] \index{NO\_B2\_CHEM\_SPUT} Used to indicate that chemical sputtering is specified through \verb!Eirene! arrays instead of \verb!B2! or \verb!B2.5!.
...
\item[USE\_EIRENE] \index{USE\_EIRENE} Compiles a version coupled to \verb!Eirene!
\item[USE\_EIRENE\_NOT\_YET\_READY] \index{USE\_EIRENE\_NOT\_YET\_READY} Obsolete
...
\item[WANT\_THIS] \index{WANT\_THIS} Adds some boundary condition treatments
...
%%grep #if $SOLPSTOP/src/b2/current/*/*.F | awk '{print $2,$1}' | sort | uniq
...
COMPLICATED & sources/b2stbc.F \\
DEF\_MAXCHORDS & b2plot/process\_chords.F \\
EASY & b2plot/chord.F \\
Fortran\_90 & user/b2file.F \\
...
USE\_EIRENE & b2plot/b2plot.F \\
USE\_EIRENE & convert/b2mds.F \\
USE\_EIRENE & driver/b2mndr.F \\
USE\_EIRENE & driver/b2mwti.F \\
USE\_EIRENE & postprocessing/b2ytdr.F \\
USE\_EIRENE & sources/b2stbr.F \\
USE\_EIRENE\_NOT\_YET\_READY & b2plot/b2plot.F \\
...
%%grep #if $SOLPSTOP/src/Eirene/current/*/*.F | awk '{print $2}' | sort | uniq

\begin{description}
\item[NAG\_RENAME] \index{NAG\_RENAME} Used with quadruple-precision compilers or to replace machine-dependent NAG calls with FORTRAN intrinsics
\item[NO\_B2\_CHEM\_SPUT] \index{NO\_B2\_CHEM\_SPUT} Used to indicate that chemical sputtering is specified through \verb!Eirene! arrays instead of \verb!B2! or \verb!B2.5!.
\item[NO\_FORT10] \index{NO\_FORT10} Deactivates output in the \verb!fort.10! file
\item[NO\_NAG] \index{NO\_NAG} No \verb!NAG! libraries available
\item[NO\_SRFPRVSL] \index{NO\_SRFPRVSL}
\item[TRIANG] \index{TRIANG} Toggles the use of coupling routines with the triangulated far SOL mesh option
\end{description}

%%grep #if $SOLPSTOP/src/Eirene/current/*/*.F | awk '{print $2,$1}' | sort | uniq
...
CELL\_CENTRED & couple/infcop.F \\
DBG & eirene/sputer.F \\
DBG & user/srfprvsl.F \\
NO\_B2\_CHEM\_SPUT & couple\_B2\_new/infcop.F \\
NO\_B2\_CHEM\_SPUT & couple\_Tria\_new/infcop.F \\
NO\_FORT10 & couple/infcop.F \\
NO\_FORT10 & couple\_B2\_new/wneutral.F \\
NO\_FORT10 & couple\_Tria\_new/wneutral.F \\
NO\_FORT10 & eirene/input.F \\
NO\_FORT10 & eirene/mcarlo.F \\
NO\_FORT10 & eirene/outeir.F \\
NO\_NAG & nag-dummy/nagsubst.F \\
NO\_SRFPRVSL & main-routines/mcarlo.F \\
TRIANG & couple/eirsrt.F \\
TRIANG & couple/if0prm.F \\
TRIANG & couple/infcop.F \\
TRIANG & couple/save\_tallies.F \\
TRIANG & couple/uptcop.F \\
TRIANG & couple/wneutral.F \\
    \hline
...
This only works at the moment using the \verb!linux.Fujitsu! compiler,
and then only at Garching!
...
The \$\verb!SOLPSTOP/src/DivGeo/dg/equtrn!
...
\$\verb!SOLPSTOP/src/DivGeo/dg/equtrn/doxygen/refman.pdf!.
...
\section{solps5.1}
\index{subversion repository log, solps5.1}

\VerbatimInput[fontsize=\footnotesize]{solps5.1_svn.log}

\section{solps\_data}
\index{subversion repository log, solps\_data}

\VerbatimInput[fontsize=\footnotesize]{solps_data_svn.log}
