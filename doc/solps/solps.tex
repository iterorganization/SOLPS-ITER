...
\item S.~Voskoboynikov, for the \SOLPSITER model equations (Appendix \ref{cha:equations})
\end{itemize}
...
\ref{cha:standaloneb2} describes the stand-alone version of {\tt B2.5}.  This
is followed by a description of the stand-alone version of {\tt Eirene} in
...
\ref{cha:dgmanual} is the {\tt DG} manual, and lastly
\ref{cha:coursenotes} includes a listing of the programs and files included
in the {\tt B2.5} part of the \SOLPSITER package.
...
When referring to \SOLPSITER in scientific papers, please use \cite{WiesenPSI2014,BONNINPET15} and references therein. You will also find these references in the \${\tt \{SOLPSTOP\}/doc} directory.
...
({\tt Xavier.Bonnin@iter.org}) at ITER, the code Responsible Officer (RO).

The first formality will then be for the prospective users to obtain an IDM (ITER Document Management) account, in case they do not have one already. This account request will be done by the RO, and the requesting user will receive an automated electronic message asking for confirmation of the provided e-mail address. With this account, the user will have access to the \SOLPSITER GIT repository (\href{https://git.iter.org/projects/BND/repos/solps-iter/browse}{https://git.iter.org/projects/BND/repos/solps-iter/browse}) maintained at the IO, the \SOLPSITER SharePoint web site hosted by ITER (\href{https://portal.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}{https://portal.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}, works best with Internet Explorer). Links to the IDM-controlled documents related to \SOLPSITER can be found in a centralized folder (\href{https://user.iter.org/?uid=Q92BAQ}{https://user.iter.org/?uid=Q92BAQ}). The account request will also serve for registration of the user to the \SOLPSITER mailing list ({\tt SOLPS-ITER@iter.org}). Lastly, there is a Confluence Wiki page located at \href{https://confluence.iter.org/display/IMP/SOLPS-ITER}{https://confluence.iter.org/display/IMP/SOLPS-ITER} for the purpose of collecting user contributions to the body of knowledge about the code.

The \SOLPSITER Git repository also includes the repositories for {\tt B2.5}, {\tt EIRENE}, {\tt Carre} and {\tt DivGeo} individually as submodules. Some service libraries (such as the GGD, MSCL and GR service libraries), can be found in the repository in the IMAS Extra and Libraries section.
...
It is best afterwards to position oneself on the master braches, which is achieved by:
...
For later updates, it is recommended you use the {\tt solps-iter\_update} script. If you are planning to do code development, your starting point should be the develop branches, which you can upload by using the {\tt solps-iter\_update\_develop} script instead.

Please note that because of the way some of the \SOLPSITER utility scripts function, the path to the \SOLPSITER top directory must not contain any of the strings {\tt .debug}, {\tt .mpi}, {\tt .openmp}, or {\tt .ig}. 
...
Users all have write access to these repositories although not to the {\tt develop}, {\tt master} or {\tt svn/*} branches.  This basically means one should create {\tt feature/[topic]}, {\tt bugfix/[topic]}, or {\tt hotfix/[topic]} branches and raise a "Pull Request" (\href{https://git.iter.org/projects/BND/repos/solps-iter/pull-requests}{https://git.iter.org/projects/BND/repos/solps-iter/pull-requests}) to have them merged into the {\tt develop} branch after pushing them back to the ITER repository.
...
The Git \SOLPSITER repository is structured as a self-contained whole, but some effort was also devoted to be able to check out, as individual stand-alone modules, the main pieces of the SOLPS suite, i.e. {\tt DivGeo}, {\tt Carre}, {\tt Eirene} and {\tt B2.5}, although this is not recommended.
...
\item[NCAR] plotting library (available from \href{http://library.ucar.edu}{http://library.ucar.edu})
\item[GR] plotting library (optional, developed at FZ-J{\"u}lich, available from the IO Git repository at \href{https://git.iter.org/projects/LIB/repos/gr-software/browse}{https://git.iter.org/projects/LIB/repos/gr-software/browse})
...
\item[NetCDF]\index{netcdf} storage/archival library (optional) (available from \href{http://www.unidata.ucar.edu/netcdf/}{http://www.unidata.ucar.edu/netcdf/})
\item[MSCL] utility library (available from the IO Git repository at \href{https://git.iter.org/projects/LIB/repos/mscl/browse}{https://git.iter.org/projects/LIB/repos/mscl/browse})
...
\item[python] scripting language (\SOLPSITER needs at least version 2.7)
...
\${\tt \{SOLPSTOP\}/lib/}\${\tt \{HOST\_NAME\}.\allowbreak}\${\tt \{COMPILER\}} directory.
...
These environment variables are found in files config.\${\tt \{HOST\_NAME\}}.\${\tt \{COMPILER\}}.
...
\begin{description}\index{Hostnames}
  ...
  \item[IPPITM] ITM cluster hosted at IPP-Garching: *.itm.rzg.mpg.de
  ...
  \item[ITM, MARCONI] ITM Gateway machine: *.efda-itm.eu, *.marconi.cineca.it
  \item[JET] JET domain: *.jet.uk
  \item[KEIO] Keio University of Yokohama: *.keio.ac.jp
  \item[LEUVEN] KU-Leuven: *.mech.kuleuven.be
  ...
  \item[AU\_UNKNOWN] All other domain names from Australia
  \item[CA\_UNKNOWN] All other domain names from Canada
  \item[CN\_UNKNOWN] All other domain names from China
  \item[EU\_UNKNOWN] All other domain names from EURATOM Member countries
  ...
  \item[RU\_UNKNOWN] All other domain names from the Russian Federation
  \item[US\_UNKNOWN] All other domain names from the United States: *.edu, *.gov, *.us
  \item[UNKNOWN] everywhere else
\end{description}

If your specific domain name is not on this list, please add it to {\tt whereami}. In cases where the {\tt hostname} and {\tt nslookup} commands used by {\tt whereami} do not provide the desired \${\tt HOST\_NAME} value, you can specify it explicitly by means of a \${\tt SOLPSTOP/SETUP/setup.csh.HOST\_NAME.local} file, which should contain:
...
\begin{description}\index{Platforms}
  ...
  \item[g95] The g95 compiler, in use on the Marconi Gateway
\end{description}

For each hostname domain, a default compiler can be defined by means of the {\tt default\_compiler}
file. This default can be overridden by giving the desired compiler name as an argument following
the {\tt setup.csh} command, i.e.:
\begin{verbatim}
...
please edit the {\tt default\_compiler} script accordingly.
...
programs, additional sets of config files can be found in the {\tt config} directory in each module. The
...
{\tt config.}\${\tt \{HOST\_NAME\}.}\${\tt \{COMPILER\}\allowbreak.local} which the {\tt Makefile} will load and use to
overwrite the more general settings from the distributed {\tt config} file.

Some Windows\index{Windows} implementations have been attempted, and require an additional {\tt -DWINDOWS} compiler switch, but are not currently being supported.
...
\${\tt SOLPSTOP/modules/B2.5/src/include/DIMENSIONS.F}. If the dimensions are not right for you,
...
\end{verbatim}
and edit the file {\tt include.local/DIMENSIONS.F} so that the
dimensions (mainly used for the namelists in {\tt B2.5}) are
...
{\tt gmake all\_mpi(\_debug)} as needed.
...
\begin{description}\index{Compiling the code}\index{B2.5, compiling}\index{SOLPS-ITER, compiling}\index{Eirene, compiling}\index{DivGeo, compiling}\index{Carre, compiling}
  \item[solps] (Default) Compiles the necessary components for a full \SOLPSITER implementation, including:
    \begin{description}
      ...
      \item[carre] The {\tt Carre} grid generator and associated translators
  ...
  \item[all] The same as {\bf solps} above, but in addition:
    \begin{description}
      \item[b25] The {\tt B2.5} stand-alone plasma fluid solver and accompanying programs
      \item[eirene] The {\tt Eirene} stand-alone Monte-Carlo neutral transport code
    \end{description}
  \item[nox] The same as {\bf solps} above, but without linking to any graphical libraries:
    \begin{description}
      \item[carre\_nox] The {\tt Carre} grid generator will not generate graphs
  ...
  \item[all\_nox] The same as {\bf nox} above, but in addition:
    \begin{description}
      \item[b25\_nox] The {\tt B2.5} stand-alone plasma fluid solver and accompanying programs
      \item[eirene\_nox] The {\tt Eirene} stand-alone Monte-Carlo neutral transport code
    \end{description}
  \item[nox\_build] The same as {\bf nox} above and used for compilation tests on the Continous Integration server:
    ...
    \end{description}
  \item[carre] The {\tt Carre} grid generator and associated translators only
  \item[divgeo] The {\tt DivGeo} graphical input file builder and accompanying equilibrium and template converters only
  \item[divgeo\_nox] Equilibrium and template converters for {\tt DivGeo}
  \item[b25] The {\tt B2.5} stand-alone plasma fluid solver and accompanying programs only
  \item[b25\_nox] The {\tt B2.5} stand-alone plasma fluid solver and accompanying programs, not using NCAR Graphics
  \item[b25\_all] The {\tt B2.5} stand-alone plasma fluid solver and accompanying programs only, plus {\tt b2xd} the \SOLPSfive to \SOLPSfour converter
  \item[eirene] The {\tt Eirene} stand-alone Monte-Carlo neutral transport code only
  \item[eirene\_nox] The {\tt Eirene} stand-alone Monte-Carlo neutral transport code only, without graphics
...
Adding the {\tt \_mpi} suffix at the end of any of build targets will execute the command
with MPI parallelization options, when relevant, and put the executables in a separate
{\tt builds/}\${\tt \{TOOL\_CHAIN\}.mpi} directory.

Adding the {\tt \_openmp} suffix at the end of any of build targets will execute the command
with OpenMP parallelization options, when relevant, and put the executables in a separate
{\tt builds/}\${\tt \{TOOL\_CHAIN\}.openmp} directory.

Adding both the {\tt \_openmp} and {\tt \_mpi} suffixes, in that order, i.e. {\tt \_openmp\_mpi}, at the end of any of build targets will execute the command
with both OpenMP and MPI parallelization options, when relevant, and put the executables in a separate
{\tt builds/}\${\tt \{TOOL\_CHAIN\}.openmp.mpi} directory.

Adding the {\tt \_debug} suffix at the end of any of build targets will execute the command
in debug mode, and put the executables in a separate
{\tt builds/}\${\tt \{TOOL\_CHAIN\}.debug} directory.
...
\begin{description}
  ...
  \item[ssc] Goes to \${\tt \{SOLPSTOP\}/modules/Carre}, the {\tt Carre} top directory
  ...
  \item[slib] Goes to \${\tt \{SOLPSTOP\}/lib/}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}}, the directory where libraries are stored
...
\section{Examples}
\label{examples}\index{DIMENSIONS.F}

On the \SOLPSITER web site (\href{https://portal.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}{https://portal.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}), in the folder {\em Examples}, one can find a few example cases for \SOLPSITER runs, in both {\tt B2.5} stand-alone or {\tt B2.5-Eirene} coupled modes.

There is also a dedicated directory \${\tt SOLPSTOP/runs/examples} which contains links to a series of sample cases. These are actually stored in the {\tt /work/imas/shared/external/assets/solps-iter/MD5} directory hosted at the IO, and the means to retrieve these cases is detailed in the \${\tt SOLPSTOP/runs/examples/README.md} file. Following the instructions therein will extract an archived file in gzipped tar format containing a {\tt baserun} and one or more run directories and associated files. In some cases, a {\tt DIMENSIONS.F} file is also provided, which shows the dimensions necessary to run those cases (see Appendix \ref{sec:DIMENSIONS} for full details about the contents of such files).

The cases stored for retrieval include:
\begin{description}
  \item[test\_slab\_ortho\_standalone] An orthogonal slab test case for {\tt B2.5} standalone
  \item[AUG\_16151\_D] ASDEX-Upgrade D-only benchmark cases, including {\tt B2.5} standalone, coupled and single {\tt Eirene} call run directories
  \item[AUG\_16151\_D+C+He] ASDEX-Upgrade multi-fluid benchmark cases, including {\tt B2.5} standalone, coupled and single {\tt Eirene} call run directories
  \item[ITER\_2171\_D+He+Be+Ne] ITER full performance case with Ne injection
  \item[ITER\_2264\_0.6Ne\_100MW] Another ITER full perfomance case with Ne injection
  \item[ITER\_2275\_Donly\_20MW] ITER low power case at low density (D-fuelled)
  ...
  \item[ITER\_2297\_Honly\_20MW] Same as above, continuing scan
  \item[ITER\_2298\_Honly\_20MW] Same as above, continuing scan
  \item[ITER\_2299\_Honly\_20MW] Same as above, continuing scan, reaching detachment
  \item[ITER\_2308\_Honly\_20MW] Same as above, continuing scan, detached conditions
  \item[ITER\_2360\_0.95He+0.05H\_40MW] ITER Helium plasma case at mid-power
  \item[ITER\_2588\_D+He+N] ITER full power case with N injection and gas puff feedback on SOL D particle content
  \item[ITER\_2588\_drifts] ITER {\tt B2.5} stand-alone D-only demonstration case with and without drifts
...
\end{description}
as well as additional partial cases used in tutorials when learning how to build a case using the {\tt DivGeo} and {\tt SOLPS-GUI} utilities.
...
\end{verbatim}
This will set \${\tt SOLPSWORK}, create the appropriate {\tt SOLPSTOP} file, and load a {\tt setup.csh.local} for each user if it exists. The scripts such as {\tt dg}, {\tt carre}, and {\tt triang} then check if {\tt SOLPS\_CENTRAL} is set and will create the meshes, etc under \${\tt \{DEVICE\}/}\${\tt \{USER\}} instead of just \${\tt \{DEVICE\}}. It is therefore important that the {\tt DivGeo/device}, {\tt Carre/meshes} and {\tt Triang/triangles} and each \${\tt \{DEVICE\}} subdirectory have the permissions set such that each user can write to it. This could be by having a {\tt solps} group, for example, or by providing write access for all users. To avoid issues where the first user to create a \${\tt \{DEVICE\}} directory will do so without the correct permissions, a {\tt add\_central\_device} script has been provided which will create the appropriate directories with write access for all users. The administrator should run this and add each {\tt DEVICE} that will be used by the group.

Each user will then {\tt cd} to the central installation directory, run {\tt tcsh; source setup.csh}, and the rest of the usage would be standard.
...
The {\tt setup.csh} file and its subsidiaries \${\tt \{SOLPSTOP\}/SETUP/setup.csh.}\${\tt \{HOST\_NAME\}.}\${\tt \{COMPILER\}} and \${\tt \{SOLPSTOP\}/SETUP/config.}\${\tt \{HOST\_NAME\}.}\${\tt \{COMPILER\}} are used to load any modules packages useful for the local \SOLPSITER run environment and the following environment variables:
\begin{enumerate}
\item {\tt B25\_DEFINES}\index{B25\_DEFINES}, an environment variable containing the additional
  "defines" to use during compilation of {\tt B2.5}
\item {\tt DBX}\index{DBX}, the name of the debugger to be used
\item {\tt DEVICE}\index{DEVICE} the name of the fusion device most likely to be simulated for this {\tt HOST\_NAME} (see \ref{cha:divgeo} for available choices)
\item {\tt EIRENE\_DEFINES}\index{EIRENE\_DEFINES}, an environment variable containing the additional
  "defines" to use during compilation of {\tt Eirene}
\item {\tt GKS\_ROOT}\index{GKS\_ROOT}, the root of the {\tt GKS} tree, and
  {\tt LD\_GKS}, the appropriate loader instruction to load the {\tt GKS} library
\item {\tt GLI\_HOME}\index{GLI\_HOME}, the location of the {\tt gksfont.dat} file (this definition must appear in both the {\tt setup.csh.*.*} and {\tt config.*.*} files as it used both during compilation and at runtime
\item {\tt GR\_ROOT}\index{GR\_ROOT}, the root of the {\tt GR} tree, and
  {\tt LD\_GR}, the appropriate loader instruction to load the {\tt GR} library (a graphics package produced by FZ-J{\"u}lich and used by {\tt Eirene})
...
\item {\tt MDSPLUS\_DIR}\index{MDSPLUS\_DIR}, the root of the {\tt MDSplus} tree, and
  {\tt LD\_MDSPLUS}, the appropriate loader instruction to load {\tt MDSplus}
\item {\tt MSCL\_ROOT}\index{MSCL\_ROOT}, the root of the {\tt mscl} tree, and
  {\tt LD\_MSCL}, the appropriate loader instruction to load the {\tt mscl} library
\item {\tt NAGDIR}\index{NAGDIR}, the root of the {\tt NAG} tree, and
  {\tt LD\_NAG}, the appropriate loader instruction to load the {\tt NAG} library
\item {\tt NCARG\_ROOT}\index{NCARG\_ROOT}, the root of the {\tt NCAR} tree, and
  {\tt LD\_NCARG}, the appropriate loader instruction to load {\tt NCAR}
\item {\tt NCDIR}\index{NCDIR}, the root of the {\tt NetCDF} tree, and
  {\tt LD\_NETCDF}, the appropriate loader instruction to load {\tt NetCDF}
\item {\tt PERFMON}\index{PERFMON}, an environment variable used by
...
\item some aliases to move around the tree ({\tt sbb}, {\tt sei},
  {\it etc.})
\item some aliases for plotting ({\tt xyplot}, etc...)
\item two aliases to swap between the debug and no\_debug versions of the code ({\tt set\_debug} and {\tt set\_nodebug})
\item two aliases to swap between the MPI and no\_MPI versions of the code ({\tt set\_mpi} and {\tt set\_nompi})
\item two aliases to swap between the OpenMP and no\_OpenMP versions of the code ({\tt set\_openmp} and {\tt set\_noopenmp})
\end{enumerate}
Of course, any additional desired definitions can be added. The preferred place for those is, for those variables that need not be shared by other users, the \${\tt \{SOLPSTOP\}/SETUP/setup.csh.}\${\tt \{HOST\_NAME\}.}\${\tt \{COMPILER\}.local} and \${\tt \{SOLPSTOP\}/SETUP/config.}\${\tt \{HOST\_NAME\}.}\${\tt \{COMPILER\}.local} files. Variables that are needed at compilation time need to appear in the {\tt config.*.*(.local)} file, while variables needed at runtime must appear in {\tt setup.csh.*.*(.local)} file.

However, the \SOLPSITER GIT server
holds several examples in the \${\tt \{SOLPSTOP\}/SETUP/} directory.
...
for {\tt B2.5}; {\tt modules/Eirene} for the Monte-Carlo code, {\tt modules/Carre} and
{\tt modules/fxdr}
...
{\tt modules/amds} for the automated generator of atomic physics blocks for {\tt Eirene},
{\tt modules/solps4-5} for the {\tt b2sxdr} converter from
\SOLPSfour to \SOLPSITERns, and {\tt modules/Sonnet-light} for the {\tt Sonnet} library in use
for \SOLPSfour grids.
...
\item[src] \index{src}\index{B2.5, src}\label{b2.5_src}
  contains the {\tt B2.5} source code files
...
of a {\tt config.}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}.local} file in the
{\tt config} subdirectory is recommended. Since the {\tt Makefile}
reads those after the {\tt config.}\${\tt {HOST\_NAME}.\allowbreak}\${\tt {COMPILER}} file,
...
\begin{description}
\item[ionization\_potentials] \index{ionization\_potentials} is a {\tt B2} and {\tt B2.5}
  input file containing ionization potentials
\item[incatm] \index{incatm} is a {\tt B2.5} input file containing data for
  atomic mass, surface binding energy and density of all elements
\item[ratadas.filelist] \index{ratadas.filelist}\index{b2ar}\index{ADAS} is a {\tt B2.5} input file
  containing information used to index into the ADAS files when the
  "adas" option is used in {\tt b2ar} (see section \ref{sec:b2ar})
\item[stra.dat] \index{stra.dat}\index{b2ar}\index{baserun}\index{STRAHL} is a file containing {\tt STRAHL}
  atomic data results for use by {\tt b2ar} when building {\tt b2frates} files where the 'strahl' data
  option is chosen. Must be copied to the local {\tt baserun} directory.
\item[weis.dat] \index{weis.dat}\index{b2ar}\index{baserun} is a file containing atomic data
  results for hydrogen by Weisheit {\it et al.} for use by {\tt b2ar} when building {\tt b2frates} files.
  Must be copied to the local {\tt baserun} directory.
\end{description}
...
Because {\tt Eirene} is distributed as a stand-alone submodule and is being maintained
...
the {\tt interfaces/couple\_SOLPS-ITER} and {\tt user-routines/user\_iter} subdirectories.
The other are included for compatibility with the FZ-J{\"u}lich master {\tt Eirene}
repository.
...
The {\tt modules/adas} subdirectory contains files which are used to tell various programs
which {\tt ADAS} files to use.

{\tt source.location} indicates which class of lines to use ("pjr"
...
\begin{verbatim}
    slib
\end{verbatim}
will take you to \${\tt \{SOLPSTOP\}/lib/}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}}.
...
The number of scripts developed for various {\bf SOLPS} versions is very large. Efforts are currently underway to translate all the relevant ones for use within \SOLPSITERns. Any of these scripts can be copied to the \${\tt \{SOLPSTOP\}/scripts.local} directory for local user modifications as desired. A non-exhaustive listing of these scripts, with some explanation as to their purpose, follows:
\begin{itemize}
\item the residuals can be plotted with
  ...
  \end{description}
all of the above can take "-m \#" to plot the residuals {\it modulo} "\#" occurrences
and/or "-l \#" to plot the last "\#" residuals
and/or "-a \#" to plot the average residual over "\#" occurrences.
...
\item power/particle losses (see section \ref{cha:regions} and Appendix \ref{lab:b2:numerics:parameters} for the region definitions)
  ...
  \item[rsamoreg \#1]\index{rsamoreg} ionization parallel momentum for region \#1 (default 0)
  ...
  \item[rcxmoreg\_region \#1]\index{rcxmoreg\_region} charge exchange
    parallel momentum, by region, for species  \#1 (default 0)
  ...
  \item[rrahireg\_region \#1]\index{rrahireg\_region} recombination
    energy, by region, for species  \#1 (default 0)
  \item[rramoreg\_region \#1]\index{rramoreg\_region} recombination
    parallel momentum, by region, for species  \#1 (default 0)
  ...
  \item[rsahireg\_region \#1]\index{rsahireg\_region} ionization energy,
    by region, for species  \#1 (default 0)
  \item[rsamoreg\_region \#1]\index{rsamoreg\_region} ionization
    parallel momentum, by region, for species  \#1 (default 0)
  \item[rsanareg\_region \#1]\index{rsanareg\_region} ionization rate, by
    region, for species  \#1 (default 0)
  \end{description}
these require the {\tt SOLPS\_FLUIDS} and {\tt NREG} variables to be set
correctly (see section \ref{lab:b2:numerics:parameters} and Appendix \ref{cha:regions}
for the definitions of the regions)\index{SOLPS\_FLUIDS}\index{NREG}. If
the {\tt SOLPS\_FLUIDS} or {\tt NREG} files are not found, the necessary
information is extracted from the {\tt run.log} file.
\item plotting
  \begin{description}
  \item[plot]\index{plot} The various {\tt xyplot}\#, {\tt lxyplot}\#, {\tt xlyplot}\# and
    {\tt lxlyplot}\# call {\tt plot} passing their name as the 1$^{st}$ parameter; {\tt plot}
    parses the 1$^{st}$ parameter to determine if $\log x$ ({\tt lx}) or $\log y$ ({\tt ly})
    ...
    a header. The system normally uses {\tt gnuplot.cmd} for the
    {\tt gnuplot} commands, and {\tt gnuplot.data} for the data.  A
...
rm GNUPLOT_COUNT gnuplot.*data
    \end{verbatim}
    where the {\tt 2dt} could be any series of commands that use
    {\tt plot} for their graphics.  See the script {\tt 2d\_plots}
    ...
    {\tt gnuplot.cmd} to the filename specified by this environment
    variable
    ...
    {\tt GNUPLOT\_MAX\_POINTS} (defaults to 2)
    ...
    \item[GNUPLOT\_POSTSCRIPT\_OPTIONS]\index{GNUPLOT\_POSTSCRIPT\_OPTIONS} {\tt gnuplot} options for the
    PostScript driver
    \item[GNUPLOT\_TERM]\index{GNUPLOT\_TERM} provides the "term" information for {\tt gnuplot}
    \item[GNUPLOT\_TMP]\index{GNUPLOT\_TMP} causes the {\tt gnuplot.cmd},
    {\tt gnuplot.data} and other temporary files to be written to the directory
    specified by this environment variable. Works for most scripts.
    Also moves the location of the {\tt GNUPLOT\_EXTRA} and
    {\tt GNUPLOT\_COUNT} files to being located in {\tt GNUPLOT\_TMP}.
    ...
    \end{description}
    If the file {\tt GNUPLOT\_COUNT} exists, then an internal counter
    {\tt GNUPLOT\_COUNT} is set by reading the number from the file;
    incremented; and then saved to the file; instead of using
    {\tt gnuplot.data} for the data, the file
    {\tt gnuplot.}\${\tt GNUPLOT\_COUNT.data} is used and the commands for
    plotting are concatenated to {\tt gnuplot.cmd} allowing for
...
negative quantities on the same scale and "M" the absolute values of negative quantities in a separate plot)
  ...
  \item[2d]\index{2d} plots time dependent quantities from
    {\tt b2time.nc} (Appendix \ref{cha:b2time}), 2$^{nd}$, 3$^{rd}$ {\it etc.\ versus} the 1$^{st}$ argument. 
  \item[2da]\index{2da} prints time dependent quantities from
    {\tt b2time.nc} (Appendix \ref{cha:b2time}), 2$^{nd}$, 3$^{rd}$ {\it etc.\ versus} the 1$^{st}$ argument.
  \item[2dt]\index{2dt} plots time dependent quantities from
    {\tt b2time.nc} (Appendix \ref{cha:b2time}) versus time. A python version of this script also exists (Python 2.7 and 3.5 compatible).
  \item[2dt\_av]\index{2dt\_av} plots averaged time dependent quantities from
    {\tt b2time.nc} (Appendix \ref{cha:b2time}) versus time.
  ...
  \item[2d\_plots]\label{script:2dplots}\index{2d\_plots} prepares {\tt b2time2d.ps} from
    {\tt b2time.nc} (Appendix \ref{cha:b2time}).  This uses the batch
    feature of the {\tt plot} command.
  \item[2d\_plots\_av]\index{2d\_plots\_av} prepares {\tt b2time2d\_av.ps} from
    averaged values in {\tt b2time.nc} (Appendix \ref{cha:b2time}).  This uses the batch
    feature of the {\tt plot} command.
  \item[2d\_profiles]\index{2d\_profiles} creates {\tt *.last10} files for plotting.
    When processing fluxes, divides them by the area of contact to the neighbouring cell.
  \item[2d\_profiles\_extended]\index{2d\_profiles\_extended} creates {\tt *.last10} files for plotting.
    When processing fluxes, divides them by the area of contact to the neighbouring cell.
    The flux files ending in {\tt P.last10} are divided by the perpendicular area of the cell face.
  ...
  \item[display\_tallies]\index{display\_tallies} Displays the {\tt B2.5} tallies in an easy-to-read format (argument "H" gives the first instance, and "L" the last instance)
  ...
  \item[fhexreg]\index{fhexreg} Plots the electron energy flow across important $x$-directed surfaces in {\tt B2.5}.
  \item[fheyreg]\index{fheyreg} Plots the electron energy flow across important $y$-directed surfaces in {\tt B2.5}.
  \item[fhixreg]\index{fhixreg} Plots the ion energy flow across important $x$-directed surfaces in {\tt B2.5}.
  \item[fhiyreg]\index{fhiyreg} Plots the ion energy flow across important $y$-directed surfaces in {\tt B2.5}.
  \item[fhmxreg]\index{fhmxreg} Plots the parallel kinetic energy flow across important $x$-directed surfaces in {\tt B2.5}.
  \item[fhmyreg]\index{fhmyreg} Plots the parallel kinetic energy flow across important $y$-directed surfaces in {\tt B2.5}.
  \item[fhpxreg]\index{fhpxreg} Plots the ionization energy flow across important $x$-directed surfaces in {\tt B2.5}.
  \item[fhpyreg]\index{fhpyreg} Plots the ionization energy flow across important $y$-directed surfaces in {\tt B2.5}.
  \item[fhjxreg]\index{fhjxreg} Plots the electrostatic energy flow across important $x$-directed surfaces in {\tt B2.5}.
  \item[fhjyreg]\index{fhjyreg} Plots the electrostatic energy flow across important $y$-directed surfaces in {\tt B2.5}.
  \item[fhtxreg]\index{fhtxreg} Plots the total energy flow across important $x$-directed surfaces in {\tt B2.5}.
  \item[fhtyreg]\index{fhtyreg} Plots the total energy flow across important $y$-directed surfaces in {\tt B2.5}.
  ...
  \item[fluxt \#1]\index{fluxt}\index{DIMENSIONS.F} Gives the recycling fluxes for the first \#1 {\tt Eirene} strata. The default value is all the strata, as read from the {\tt b2.neutrals.parameters} file, or if not present, the \${\tt SOLPSTOP/modules/B2.5/src\allowbreak/include.local/DIMENSIONS.F} file, or if not present, the \${\tt SOLPSTOP/modules/B2.5/src\allowbreak/include/DIMENSIONS.F} file.
  ...
  \item[tflux \#1]\index{tflux}\index{DIMENSIONS.F} Gives the recycling fluxes for the first \#1 strata from {\tt b2.neutrals.parameters}. The default value is all the strata, as read from the {\tt b2.neutrals.parameters} file, or if not present, the \${\tt SOLPSTOP\allowbreak/modules/B2.5/src/include.local/DIMENSIONS.F} file, or if not present, the \${\tt SOLPSTOP\allowbreak/modules/B2.5/src/include/DIMENSIONS.F} file.
  ...
  \end{description}
many of the above require the {\tt SOLPS\_FLUIDS} and {\tt NREG} variables to be set
correctly (see section \ref{lab:b2:numerics:parameters} and Appendix \ref{cha:regions}
for the definitions of the regions and surfaces)\index{SOLPS\_FLUIDS}\index{NREG}. If
the {\tt SOLPS\_FLUIDS} or {\tt NREG} files is not found, the necessary information is
extracted from the {\tt run.log} file.
\item Utility
  \begin{description}
  ...
  \item[convert\_from\_solps4.3 \#1]\index{convert\_from\_solps4.3} Converts run number \#1 from \${\tt DEVICE} from the SOLPS4.3 ITER archive to a \SOLPSITER run in directory \${\tt SOLPSTOP/runs/}\${\tt DEVICE/}\#1 .
  ...
  \item[plot\_cps \#1]\index{plot\_cps} Copies {\tt gnuplot.cmd} to
    {\tt gnuplot\_cps.cmd}, modifying it so that colour plotting occurs to \#1
    or by default {\tt out.ps}; executes {\tt gnuplot} on new file.
    The location of these files can be modified by use of the {\tt GNUPLOT\_TMP} variable.
  \item[plot\_it]\index{plot\_it} Utility script used to produce complex plots.
  \item[plot\_ps \#1]\index{plot\_ps} Copies {\tt gnuplot.cmd} to
    {\tt gnuplot\_ps.cmd}, modifying it so that black-and-white plotting
    occurs to \#1 or by default {\tt out.ps}; executes {\tt gnuplot} on new file.
    The location of these files can be modified by use of the {\tt GNUPLOT\_TMP} variable.
...
\item Others
  ...
  \item[b2favere\_OK]\index{b2favere\_OK} Checks that the {\tt b2favere} file
    is of correct size and content.
  \item[b2fstate\_OK]\index{b2fstate\_OK} Checks that the {\tt b2fstate} file
    is of correct size and content.
  ...
  \item[b2plot]\index{b2plot} Shortcut to \btwoplotns. A "-n" option allows for dry-run tests.A "-d" option allows to run within the chosen debugger. A "-s" option allows to run \btwoplot with coupling to {\tt Eirene} data. A "-h" option prints a help description.
  ...
  \item[b2run]\index{b2run}\index{b2co}\index{b2fstate} Shortcut to the {\tt Makefile} launching {\tt B2.5}. A "-n" option allows for dry-run tests. A "-d" option allows to run within the chosen debugger. A "-s" option allows to run {\tt B2.5} in stand-alone mode. A "-m" option allows to run with MPI options. A "-o" option adds any other desired run options. The options can be grouped together inside quotes or given individually with multiple "-o" instances. A "-t \#" option allows to request an OpenMP run with \# threads. A "-b" option can be used when invoking the {\tt b2co} program to obtain a provisional {\tt b2fstate} file while the main code is running. A "-h" option prints a help description. (See \ref{b2run} for fuller details)
  \item[b2run\_hgw\_cluster]\index{b2run\_hgw\_clus} Shortcut to the {\tt Makefile} launching {\tt B2.5} for the IPP--Greifswald cluster. A "-n" option allows for dry-run tests.
  \item[b2sxdr]\index{b2sxdr} Shortcut to the \SOLPSfour to \SOLPSfive
    converter. A "-n" option allows for dry-run tests. A "-d" option allows to run within the chosen debugger. A "-s" option allows to specify the run to convert was not coupled to {\tt Eirene}. A "-o" option adds any other desired run options. The options can be grouped together inside quotes or given individually with multiple "-o" instances. A "-h" option prints a help description.
  ...
  \item[correct\_baserun\_timestamps]\index{correct\_baserun\_timestamps}
    Makes sure the timestamps for the start-up files in the {\tt baserun}
    directory are in the right chronological order.
  \item[correct\_b2yt\_timestamps]\index{correct\_b2yt\_timestamps}\index{b2yt}
    Makes sure the timestamps for the start-up files in the current
    directory are in the right chronological order for the {\tt b2yt} converter.
  ...
  \item[first\_setup]\index{first\_setup} Creates the {\tt config.}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}} files in each of the \SOLPSITER submodules.
  ...
  \item[gtfl]\index{gtfl} Replaces the specified links by the files they are linked to.
  \item[imasdb\_solps-iter]\index{imasdb\_solps-iter}\index{IMAS} Script to load the IMAS ITER scenario database and the \SOLPSITER IMAS database archives
  \item[import\_files\_from\_convert]\index{import\_files\_from\_convert}\index{b2yt} Imports and renames the files produced by {\tt b2yt} in a {\tt convert} directory at the same level as the current directory to prepare the new converted case for a run
  ...
  \item[ITER\_divertor\_HTM]\index{ITER\_divertor\_HTM} Performs the thermal calculation of the divertor target monoblocks and reflector plate hypervapotron channels. It requires {\tt b2ymb.dat} and {\tt b2yrp.dat} to be present in the run directory. It runs \btwoplot to produce the heat load profile data of the targets and reflector plates. Then it runs {\tt b2ymb} to produce the input files (see \ref{sec:b2ymb}) for the {\tt FreeFem++}\cite{MR3043640} heat transfer model of the monoblock {\tt Monoblock\_HTM.edp} for the outer target. Afterwards, {\tt b2ymb} is ran again to produce the input files to run {\tt Monoblock\_HTM.edp} for the inner target. When the inner target is completed, {\tt b2yrp} (see \ref{sec:b2yrp}) is run to produce the input files for the inner reflector plate heat transfer model {\tt ReflectorPlate\_HTM.edp}. Finally, {\tt b2yrp} is executed one last time to produce the input files for the thermal analysis of the outer reflector plate.
  \item[itersubmit]\index{itersubmit} Command file to launch runs on the ITER HPC cluster. See Section \ref{b2run} for details.
  \item[itmsubmit]\index{itmsubmit} Command file to launch runs on the Marconi ITM Gateway machine. See Section \ref{b2run} for details.
  \item[ippitmsubmit]\index{ippitmsubmit} Command file to launch runs on the IPP ITM cluster. See Section \ref{b2run} for details.
  ...
  \item[move\_optional\_files]\index{move\_optional\_files} Script used by the run Makefile to check for the existence of optional output files and, upon succesful termination of a run, move these files to the run directory.
  ...
  \item[QSUB.postprocess]\index{QSUB.postprocess} List of instructions appended to the {\tt B2.5} stand-alone submission scripts for run post-processing
  ...
  \item[resave\_mds]\index{resave\_mds} Update an archived {\tt MDSplus} case. A "-n" option allows for dry-run tests. A "-d" option allows to run within the chosen debugger. A "-s" option allows to update a {\tt B2.5} stand-alone case.
  ...
  \item[save\_mds]\index{save\_mds} Archive an {\tt MDSplus} case. A "-n" option allows for dry-run tests. A "-d" option allows to run within the chosen debugger. A "-s" option allows to archive a {\tt B2.5} stand-alone case.
  \item[setup\_baserun\_eirene\_links]\index{setup\_baserun\_eirene\_links} Prepares the run directory before a {\tt B2.5-Eirene} run by settings up links to the proper data files.
  ...
  \item[str2ogr]\index{str2ogr} Converts a .str structure file into a {\tt DG} .ogr template file.
...
\begin{description}
  ...
  \item[nareg\_species.py \#1]\index{nareg\_species.py}  Average ion
    densities for region \#1 summed over charge states for each species.
  ...
  \item[rranareg.py \#1]\index{rranareg.py} Recombination rates for
    region \#1.
  \item[rranareg\_per\_rsanareg.py \#1]\index{[rranareg\_per\_rsanareg.py}
    Recombination rates divided by ionization rates for region \#1
    (summed over charge states).
...
\${\tt \{SOLPSTOP\}/modules} directory. Files that were needed by the
post-processing or analysis scripts are now placed within the
\${\tt \{SOLPSTOP\}/scripts} directory. Please refer to the appropriate sections
...
If one is running {\tt B2.5} in stand-alone mode, it is recommended to set the environment variable:
\begin{verbatim}
...
running within the \${tt \{SOLPSTOP\}/runs} tree, the
...
\begin{description}
  ...
  \item[-m $\backslash$"MPI-launcher + options$\backslash$"] Allows to launch {\tt b2XX} in MPI
    mode with the provided options (example: [the syntax is important, both
      the slashes and the quotes are needed] \verb!b2run -m \"mpiexec -np 2\" b2mn!)
  \item[-s] Specifies that one wishes to run {\tt B2.5} in stand-alone mode, i.e. without any coupling to {\tt Eirene}.
  \item[-o "run options"] Allows to launch {\tt b2XX} with additional run options. The options can be grouped together inside quotes or given individually with multiple "-o" instances.
...
If the file named {\tt SOLPSTOP} is missing, the script {\tt QSUB.*} will construct \${\tt \{SOLPSTOP\}} by
...
in the \${\tt \{SOLPSTOP\}/runs} directory ({\tt QSUB.*} and {\tt SLURM.*} files). Currently, the following commands
are provided:
\begin{description}
  \item{ippitmsubmit}\index{ippitmsubmit} for submissions on the IPP ITM cluster
  \item{itersubmit}\index{itersubmit} for submissions on the ITER HPC cluster
  \item{itmsubmit}\index{itmsubmit} for submissions on the Marconi ITM Gateway machine
...
\end{description}
Users on other systems are encouraged to recopy these command scripts (found in \${\tt \{SOLPSTOP\}/scripts})
and adapt them to their local environment, along
with the needed modifications to the \${\tt \{SOLPSTOP\}/runs/QSUB.*} or \${\tt \{SOLPSTOP\}/runs/SLURM.*} actual submission scripts.
Each submission command file can handle five possible cases:
\begin{description}\index{QSUB files}
  \item{QSUB.*, SLURM.*} Runs a stand-alone {\tt B2.5} case.
  \item{QSUB.*-gz, SLURM.*-gz} Runs a stand-alone {\tt B2.5} case where the output log file has been compressed (activated with the "-z" option).
  \item{QSUB.*\_coupled, SLURM.*\_coupled} Runs a coupled \SOLPSITER case.
  \item{QSUB.*\_coupled-gz, SLURM.*\_coupled-gz} Runs a coupled \SOLPSITER case where the output log file has been compressed (activated with the "-z" option).
  \item{ } Any otherwise named file specified by the user and given as an argument to the submission command. The specified file must be located in the \${\tt \{SOLPSTOP\}/runs} directory, or in the actual run directory on one above it.
\end{description}
where "*" stands for "iter", "itm", "jet", "lsf", or "sge", referring to the machine system on which the
submission script is to be used. The "\_coupled" scripts are used when the presence of an {\tt Eirene}
{\tt input.dat} input file is detected in the run directory.
All the submission scripts make use of {\tt b2run} so the same possibility of submitting runs from
outside the \${\tt SOLPSTOP} file space is available.

If, for instance, one is running on one of the ITER clusters, then, the instructions to follow would be to run the script
\begin{verbatim}
itersubmit
\end{verbatim}
or
...
\begin{verbatim}
qsub -S /usr/bin/tcsh $NAME
\end{verbatim}
or
\begin{verbatim}
sbatch $NAME
\end{verbatim}
depending on which cluster you are on, or stops if the submission file is not found, where \${\tt NAME} stands for the name of the submission file used.

If {\tt <<Submission\_file>>} is not specified, {\tt itersubmit} looks for {\tt QSUB.iter\_coupled} (or {\tt SLURM.iter\_coupled}) or
for {\tt QSUB.iter} (or {\tt SLURM.iter}) depending on whether a coupled or a standalone run is required (and to which cluster you are submitting). To request a stand-alone run explicitly, the user can use the "-s" option. Otherwise, the script will assume a coupled run is requested if an {\tt Eirene} {\tt input.dat} file is present in the run directory, and a stand-alone run otherwise.

In the case one wishes to run the code with MPI and needs to pass MPI options to the launching program, this is achieved by means of the "-m" option, followed by the desired options inside double quotes, as follows:
\begin{verbatim}
...
If the user wishes to submit an OpenMP job, this is done by use of the "-t" option, followed by the number of desired CPU threads.
...
The {\tt QSUB.iter} and {\tt SLURM.iter} scripts first of all
change directory to \${\tt \{SOLPSTOP\}}
with the help of {\tt SOLPSTOP} file
and execute {\tt setup.csh} there. This is necessary
...
Then the {\tt QSUB.iter} and {\tt SLURM.iter} scripts return back to the
current run directory and execute the script {\tt b2run}.

The examples presented below are for a pure D plasma.

\newpage\section{b2ag, setting up the geometry}
\index{b2fgmtry}\label{sec:b2ag}\index{b2ag}\index{fort.30}\index{b2run}
...
or
\begin{verbatim}
    gmake -f $SOLPSTOP/runs/Makefile fort.30
\end{verbatim}
%% $
or, best:
\begin{verbatim}
...
{\tt b2ag} will also prepare the grid information in the format usable by {\tt Eirene},
i.e. a {\tt fort.30} file.
...
referred to the code ({\tt b2stbc\_bas}) for the definitive meaning,
...
and the "north" boundary in two. However, the {\tt b2ah} format does
...
{\tt b2.boundary.\allowbreak parameters} (the recycling coefficients can be
...
\${\tt \{SOLPSTOP\}/doc/SOLPS\_2002\_Course/atomic\_physics.pdf} .
...
If {\tt 'b2stbc\_boundary\_namelist'} is set to 1 in {\tt b2mn.dat}
...
\item[BCCHAR]\index{BCCHAR} {\tt character array, length NBC}, specifying the nature of
...
\item[BCSTART]\index{BCSTART} {\tt integer array, length NBC}, for North,
  ...
  index
\item[BCEND]\index{BCEND} {\tt integer array, length NBC}, for North, South
...
\item[BCCON]\index{BCCON} {\tt integer array, length NS * NBC}, specifying
  ...
  \item[1] prescribe the value of the density, {\tt CONPAR(,,1)}
  specifies the required density in $m^{-3}$
  \item[2] prescribe the gradient of the density, {\tt CONPAR(,,1)}
  specifies the required density gradient in $m^{-4}$
  \item[3] sheath conditions, {\tt CONPAR(,,1)} not used (zero
  gradient is used)
  \item[4] prescribe the value of the density, weakly a mixed boundary
  condition, {\tt CONPAR(,,1)} specifies the required density in
  $m^{-3}$ and {\tt CONPAR(,,2)} specifies the "strength" of the
  boundary condition
  \item[5] prescribe the particle flux per unit area,
  {\tt CONPAR(,,1)} specifies the required particle flux density in
  $m^{-2} s^{-1}$
  \item[6] prescribe the total particle flux for a constant density,
  {\tt CONPAR(,,1)} specifies the particle flux in
  $s^{-1}$
  ...
  density, {\tt CONPAR(,,1)} specifies the particle flux in
  $s^{-1}$
  ...
  {\tt CONPAR(,,1)} not used, derived from {\tt CBSNA(0,IS,IREG)}.
  The species used must be declared using the {\tt b2stbc\_isfeedback} switch.
  \item[12] particle density feedback boundary condition, as above,
  {\tt CONPAR(,,1)} not used, derived from {\tt CBSNA(0,IS,IREG)}.
  The species used must be declared using the {\tt b2stbc\_isfeedback} switch.
  \item[13] particle density to achieve specified total flux,
  {\tt CONPAR(,,1)} is the specified radial flux crossing the surface
  {\tt b2stbc\_type13\_ref} steps away from the boundary, to which is added
  the returning flux of ionised neutrals that crossed into the plasma core
  ({\it via} {\tt CONPAR(,,3)}) when running with {\tt Eirene} and the
  'ionising\_core' switch is used,
  {\tt CONPAR(,,2)} is the strength of the feedback.
  \item[14] sound speed velocity flux,
  {\tt CONPAR(,,1)} is a multiplier to the outgoing sound speed $C_{s, \mbox{collective}}$.
  ...
  {\tt CONPAR(,,1)} specifies the desired density in $m^{-3}$.
  {\tt CONPAR(,,2)} is the strength of the feedback
  \item[25] Feedback boundary condition: prescribe the average value of the density and add a density perturbation from neighbouring radial cell. This boundary condition is suitable for any species of a multi-species plasma (i.e. in the case when the condition used in {\tt BCCON=21} fails). It is recommended to use this boundary condition together with the corresponding condition on ion temperature ({\tt BCENI=26,27}).
  {\tt CONPAR(,,1)} specifies the desired average density in $m^{-3}$.
  \item[26] Feedback boundary condition: prescribe the total ion flux and find the average density. A density perturbation is taken from the neighbouring radial cell. This boundary condition is suitable for any species of a multi-species plasma (i.e. in the case when the condition used in {\tt BCCON=22} fails). It is recommended to use this boundary condition together with a corresponding condition on the ion temperature ({\tt BCENI=26,27}). The total desired flux is summed over all {\tt BCCON=26} core boundaries.
  {\tt CONPAR(,,1)} specifies the desired particle flux in $s^{-1}$.
  \item[27] Feedback boundary condition: prescribe the particle flux sum for all neutrals and ions belonging to a given isonuclear sequence and find the average density of the highest ionization stage. A density perturbation is taken from the neighbouring radial cell. It is recommended to use this boundary condition together with corresponding condition on ion temperature ({\tt BCENI=25,26}). The total desired flux is summed over all {\tt BCCON=27} core boundaries.
  {\tt CONPAR(,,1)} specifies the desired particle flux in $s^{-1}$.
  \end{description}
\item[BCMOM]\index{BCMOM} {\tt integer array, length NS * NBC}, specifying
  ...
  velocity is set to be at least {\tt MOMPAR(,,1)} $\times
  C_{s,\mbox{species}}$
  \item[4] prescribe the value of the velocity, weakly a mixed
  boundary condition, {\tt MOMPAR(,,1)} specifies the parallel
  velocity in $m.s^{-1}$ and {\tt MOMPAR(,,2)} specifies the
  ...
  [[[Eventually intended to have {\tt MOMPAR(,,1)} specify the
  gradient of the Mach number in $m^{-1}$]]]
  \item[9] prescribe the total parallel momentum flux with constant flux
  density, {\tt MOMPAR(,,1)} specifies the parallel momentum flux in $N$
  ...
  \item[13] sheath boundary condition from {\tt b2stbc\_spb} for the parallel momentum
...
\item[BCENE]\index{BCENE} {\tt integer array, length NBC}, specifying
  ...
  difference [the sound speed used depends on settings of {\tt MOMPAR(,ISMAIN,2)}]
  \item[4] prescribe the value of the electron temperature, weakly a
  mixed boundary condition, {\tt ENEPAR(,1)} specifies the
  temperature in $eV$ and {\tt ENEPAR(,2)} specifies the
  "strength" of the boundary condition
  ...
  electron temperature, {\tt ENEPAR(,1)} specifies the energy flux
  ...
  density, {\tt ENEPAR(,1)} specifies the energy flux in $W$
  ...
  \item[10] feedback option for core, {\tt ENEPAR(,1)} not used,
  derived from {\tt cbshe(0,coreregno)}
  \item[11] not used
  ...
  proportional to temperature, {\tt ENEPAR(,1)} specifies the energy
  ...
  \item[15] sheath boundary condition, from {\tt b2stbc\_spb}, recommended when using drifts (see Section~\ref{sec:bcene} for details). Linked to using {\tt BCCON=14 }for all ion species.
  \item[16] Feedback boundary condition with constant temperature,
    {\tt ENEPAR(,1)} specifies the power flux in $W$ across the flux surface
    with index {\tt b2stbc\_type16\_ref} (default=-1), {\tt ENEPAR(,2)}
    should be something like {\tt 0.1} and specifies the strength of the
    feedback. Also see type [17] below. Available if {\tt bcene\_16\_style=0} (default).
    If {\tt bcene\_16\_style=1}, integrated electron heat flux with constant $T_e$,
    summed over all core boundaries with {\tt BCENE=16}.
  ...
  as per type [16] but with {\tt type20\_} switches and a different
  feedback scheme.
  \item[21] constant temperature feedback scaled by temperature on the ring
   {\tt bc\_type21\_ref} away. {\tt ENEPAR(,1)} specifies the desired electron temperature in $eV$. {\tt ENEPAR(,2)} is the strength of the feedback.
...
\item[BCENI]\index{BCENI} {\tt integer array, length NBC}, specifying the
  ...
  {\tt ENIPAR(,1)} specifies the contribution to the energy
  transmission coefficient [the sound speed used depends on settings
  of {\tt MOMPAR(,ISMAIN,2)}]
  \item[4] prescribe the value of the ion temperature, weakly a mixed
  boundary condition, {\tt ENIPAR(,1)} specifies the temperature in
  $eV$ and {\tt ENIPAR(,2)} specifies the "strength" of the
  boundary condition
  ...
  temperature, {\tt ENIPAR(,1)} specifies the energy flux in $W$
  ...
  density, {\tt ENIPAR(,1)} specifies the energy flux in $W$
  ...
  \item[10] feedback option for core, {\tt ENIPAR(,1)} not used,
  derived from {\tt cbshi(0,ISMAIN,coreregno)}
  ...
  to temperature, {\tt ENIPAR(,1)} specifies the energy flux density
  ...
  \item[15] sheath boundary condition, from {\tt b2stbc\_spb}, recommended when using drifts (see Section~\ref{sec:bceni} for details). Linked to using {\tt BCCON=14} for all ion species.
  \item[16] Feedback boundary condition with constant temperature,
    {\tt ENIPAR(,1)} specifies the power flux in $W$ across the flux surface
    with index {\tt b2stbc\_type16\_ref} (default=-1), {\tt ENIPAR(,2)}
    should be something like {\tt 0.1} and specifies the strength of the
    feedback. Also see type [17] below. Available if {\tt bceni\_16\_style=0} (default).
  ...
  {\tt ENIPAR(,,2)} is the strength of the feedback
...
\item[BCPOT]\index{BCPOT} {\tt integer array, length NBC}, specifying the
  ...
  \item[1] prescribe the value of the potential, {\tt POTPAR(,1)}
  specifies the potential in $V$
  \item[2] prescribe the gradient of the potential, {\tt POTPAR(,1)}
  ...
  boundary condition, {\tt POTPAR(,1)} specifies the potential in
  $V$ and {\tt POTPAR(,2)} specifies the "strength" of the
  boundary condition
  \item[5] prescribe the current flux density per unit area,
    {\tt POTPAR(,1)} specifies the electric current flux density in
    $A.m^{-2}$
  ...
  density, {\tt POTPAR(,1)} specifies the electric current in $A$
  ...
  on using {\tt cbsch(0,coreregno)})
  \item[11] sheath conditions, electron energy transmission from {\tt b2stbc\_spb}
  {\tt POTPAR(,2)} specifies the bias potential in $V$
  \item[12] Imposes the currents due to drifts for the South core boundary. Must be used in conjunction with the switch setting {\tt \allowbreak istyle\_cur\_contr\_on\_S\_and\_N=2}
  \item[13] Imposes the currents due to drifts for the South private flux and North boundaries. Must be used in conjunction with {\tt \allowbreak istyle\_cur\_contr\_on\_S\_and\_N=2}
  ...
  {\tt POTPAR(,,2)} is the strength of the feedback.
  \end{description}
...
\item[LBNDUSR]\index{LBNDUSR} {\tt logical}, if true will also
...
If {\tt 'b2tqna\_transport\_namelist'} is set to 1 in {\tt b2mn.dat}
...
\item[CFLAL]\index{CFLAL} {\tt real},
  multiplier to the thermo-electric coefficient flux limit
\item[CFLAB]\index{CFLAB} {\tt real},
...
\item[CORR\_CORE\_DN]\index{CORR\_CORE\_DN} {\tt real array, length NS}, damping parameter for speed-up of the density equations in the core when drifts are activated (does not apply to neutral species)
\item[CORR\_CORE\_DT]\index{CORR\_CORE\_DT} {\tt real}, damping parameter for speed-up of the temperature equations in the core when drifts are activated
...
If {\tt 'b2tqna\_inputfile'} is set to 1 in {\tt b2mn.dat}
...
\begin{description}
  ...
  \item[sdata(1,m,i,j)]\index{sdata} are the radial positions (measured in metres) of the
     ...
     where $m$ runs from $1$ to {\tt nsdata(1,i,j)}. By default, the radial positions are
     measured from the separatrix outward, along the outer midplane, but this can be changed by
     making use of the $ixref$ and $iyref$ switches, respectively, to modify the poloidal
     location of the profile and the radial position from which the distances are measured.
  ...
  \item[xdata(1,m,i,j)]\index{xdata} are the axial positions (normalised to the total length)
     ...
     where $m$ runs from $1$ to {\tt nxdata(1,i,j)}. By default, the connection length
...
\begin{description}
  ...
  \item['cppr'] Plot the plasma pressure
...
be followed for a succesful conversion is:
\begin{enumerate}
  ...
  \item[4] Check and correct as necessary the relative ages of the input files.
  \item[5] Run the {\tt b2yt} converter
  \item[6] Displace the converted case input files to the converted case {\tt baserun} and run directories
\end{enumerate}
...
activated with the 'b2ytdr\_non\_commensurate' option set to '1' in {\tt b2yt.dat}. This option should also be used when the overall grid size remains the same but the location and/or depth of the cuts has changed.

The conversion of the {\tt Eirene} input file additionally requires for the converter program to know the original version of {\tt Eirene} with which the input file was used, and this is provided by means of the 'eirene\_format' switch. Its possible values (case-insensitive) are:
\begin{description}
...
\end{description}
The {\tt input.dat.2} file that the converter will create will follow the \SOLPSITER format.

The conversion of the {\tt Eirene} output file ({\tt fort.44}) additionally requires for the converter program to know the original version of {\tt SOLPS} with which the output file was created, and this is provided by means of the 'solps\_version' switch. Its possible values (case-insensitive) are:
\begin{description}
  ...
  \item["5.2"] for {\tt fort.44} output files from {\bf SOLPS5.2} runs
...
been produced and is available in the run directory as {\tt b2fgmtry}.
The old geometry file should also remain available in
the run directory as {\tt b2fgmtry.old}.
...
to produce a new {\tt input.dat.2} file. In order to
...
was run is located at the same level.

At this time, not all cases involving reshuffling of bundles have been tested, and the
user is encouraged to check the {\tt b2yt} output files visually after execution to
...
\label{sec:b2co}\index{b2co}\index{plasmastate}\index{B2.5, run recovery}\index{b2fstate}\index{recover\_initial\_files}\index{tracing}\index{b2movies.nc}\index{b2tallies.nc}\index{b2wall.nc}\index{b2time.nc}\index{b2run}
...
This procedure will produce the files necessary to re-start the run from the point at which the {\tt plasmastate} file was written. In cases with feedback schemes active, the feedback values will be grabbed from the relevant {\tt b2.*\_save.parameters.XXXX} files and a new set of {\tt b2.*\_save.parameters} files will be created. Optional output files (i.e the NetCDF files {\tt b2time.nc}, {\tt b2tallies.nc}, {\tt b2wall.nc}, and {\tt b2movies.nc}, as well as the {\tt *.trc} files in the {\tt tracing} directory), if present, will be truncated to be synchronized with the recovered plasma state file.
...
In {\tt X\_tg[i-o].dat} is stored the value of the poloidal coordinate of the center of each tile along the monoblock axis (in meters).
...
In {\tt X\_[i-o]rp.dat} is stored the value of the poloidal coordinate of the center of each tile along the RPs axis (in meters).
...
This routine saves the final output state of a run into an IDS (Interface Data Structure).

It reads the {\tt b2mndr\_id*} switch group, defined in {\tt b2mn.dat}:
\begin{itemize}
  \item {\tt b2mndr\_user} defines the user name and it is set by default to be the same as the \${\tt USER} environment variable.
  \item {\tt b2mndr\_device} defines the device name and it is set by default to be the same as the \${\tt DEVICE} environment variable. If {\tt DEVICE} has been left undefined, the device will be noted as {\tt 'solps-iter'}.
  \item {\tt b2mndr\_shot\_number} defines the shot number
  \item {\tt b2mndr\_run\_number} defines the run number
\end{itemize}
These switches are used as identification parameters for the IDS. Afterwards, a
{\tt b2mn} run is being initialized but {\em not} started, so that the final
...
\begin{verbatim}
LSYMET  LBALAN  LCOARSE NLTRIMESH
NFLA    NCUTB   NCUTL   IMF   NTRFRM   NFULL   IBRAD   IBPOL   IBTOR
\end{verbatim}
...
DO 30, IT=1,NTARGI
   DO 33, IPRT=1,NTGPRT(IT)
      I NDT    NINCT  NIXY   NTIN   NTEN   NIFLG   NPTC   NPTCM
        NSPZI  NSPZE  NEMOD
   33 CONTINUE
30 CONTINUE
\end{verbatim}
...
\begin{description}
  \item[LCOARSE] Coarse mesh switch ???
  \item[NLTRIMESH] When {\tt .TRUE.}, {\tt Eirene} is only run to obtain the contours necessary to build the triangular grid, which are then written in {\tt fort.78}. This is used as by the {\tt triang} script toolchain. No particle trajectories are followed.
  \item[NCUTB] For \SOLPSITER couplings, should be 0
  \item[NCUTL] For \SOLPSITER couplings, should be 1
  \item[IMF] {\tt fort.30} format switch, should be 0
  \item[NTRFRM] {\tt fort.33} format switch, should be 0
  ...
  \item[NEMOD] This corresponds to the {\tt NEMODS} flag in input block 7, and is needed only if {\tt INDSRC=6}, i.e., if the source is specified by data from block 14 alone. As for {\tt NPTC}, only the value from the first stratum segment is used.
...
This input block is used to adjust the edges of the additional walls in the {\tt Eirene} description of the vacuum vessel walls to match exactly the location of the grid corners from the plasma solver in {\tt B2.5}. This block may begin with an optional text line containing either {\tt GENERAL} or {\tt BIASED\_GARCHING}, which changes the format below. The \SOLPSITER default is to not include this text line. By default, the grid corners at the targets will be adjusted to the additional surfaces specified, and there should be one such additional surface per grid corner. Each additional surface can only serve for one grid corner adjustment, no more.

{\it ***15. Data for interfacing routine "geousr"}

If there is no preceding text line, the reading format is:
\begin{verbatim}
NADMOD   NASMOD
DO I=1,NADMOD
   NRS   IPUNKT
   IF (IPUNKT.LT.0) XPOLPOS   YPOLPOS
ENDDO
DO I=1,NASMOD
   NAS   IPUNKT   NSSIR   NSSIP
ENDDO
\end{verbatim}
...
\begin{description}
  \item[NADMOD] Number of additional surfaces to be modified (should be 2 per divertor target, and 2 for a limiter geometry case)
  \item[NASMOD] Number of standard surfaces to be modified. Should be 0 for standard \SOLPSITER couplings.
  \item[NORMOD] Number of cell faces whose normal is to inverted. Should be 0 for standard \SOLPSITER couplings.
  \item[NRS] Index of the additional surface being modified
  \item[IPUNKT] {\tt ABS(IPUNKT)} is the index of the vertex of that additional surface being modified
  \item[XCOOR] New X-coordinate of the point being modified (in cm). Not necessary in standard \SOLPSITER couplings.
  \item[YCOOR] New Y-coordinate of the point being modified (in cm). Not necessary in standard \SOLPSITER couplings.
  \item[ZCOOR] New Z-coordinate of the point being modified (in cm). Not necessary in standard \SOLPSITER couplings.
  \item[XPOLPOS] X-index (poloidal) of the grid cell to which the additional surface vertex must be matched
  \item[YPOLPOS] Y-index (radial) of the grid cell to which the additional surface vertex must be matched
  \item[NAS] Index of the standard surface to be modified
  \item[NSSIR] Radial position of the standard surface to be modified
  \item[NSSIP] Poloidal position of the standard surface to be modified
  \item[IDIR] Direction of the face whose normal is being inverted (1 = radial, 2=poloidal)
  \item[IR] Radial index of the cell where the normal is being inverted
  \item[IP] Poloidal index of the cell where the normal is being inverted
\end{description}

\subsection{Information for MPI}

There is a final optional block in the {\tt Eirene} input file that can be used to choose the MPI parallelization strategy to be applied. Its format is:
...
There are currently four possible choices supported. They are:
\begin{description}
  \item[BALANCED] A balanced strategy where the work is attempted to be divided evenly among processors (currently the \SOLPSITER default). If the initial setup for the {\bf BALANCED} strategy fails, the code then attempts the {\bf APCAS} strategy.
  \item[APCAS] meaning {\it All Processors Compute All Strata}, where the work of each stratum is divided among all processors. Usually involves more communication between processors.
  \item[ORIGINAL] The original strategy implemented in versions of {\tt Eirene} prior to \SOLPSITER version 3.0.5. In that strategy, each processor is handed either part of a large stratum or one or several full smaller strata. This strategy works well if the number of processors is larger than the number of strata, but is not recommended if the number of strata is larger than the number of processors.
  \item[AUTOMATIC] tells the code to use the current default, whatever it may be.
\end{description}
The format to read the strategy is free-form. The input file is scanned for the words describing the chosen strategy after the title line {\tt *** INFORMATION\_FOR\_MPI} is found. The {\em first} match to an (uppercase) strategy name will be the strategy followed by the code. If the title line is missing or no match is found to any of the strategy names, the default {\bf BALANCED} strategy is used. See the {\tt Eirene\_balanced\_MPI} document on the \SOLPSITER web site {\tt Manuals and Documentation} folder for more details on the performance and algorithm corresponding to each strategy.
...
to establish the links {\tt Eirene} needs to the relevant atomic data files,
...
\href{https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS/trunk/solps\_examples/solps5.1/bench\_mark/AUG\_16151\_D+C+He/}{https://solps-mdsplus.aug.ipp.mpg.de/repos/SOLPS/trunk/solps\_examples/solps5.1/bench\_mark/AUG\_16151\_D+C+He/}. Another version of
these files can be found in the {\em Examples} folder in the \SOLPSITER SharePoint
web site
(\href{https://portal.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}{https://portal.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}).
...
In order to attempt to improve convergence of coupled \SOLPSITER runs, currently three
averaging schemes are available to reduce the impact of the Monte-Carlo noise intrinsic
to the {\tt Eirene} sources. The user may want to activate one of these schemes,
...
\begin{description}
  ...
  \item[eirene\_underrelax] The neutral sources are multiplied by an under-relaxation factor of $1/eirene\_underrelax$ and added to the sources from the previous timestep (under-relaxation scheme).
...
between the
{\tt B2.5} and {\tt Eirene} parts of the code. Please see Appendix
...
Second, the array sizes of {\tt chemical\_sputter\_yield}, is used, now need to match the
...
{\tt input.dat} file used for the same case) and to the non-standard surfaces (block 3A) corresponding to the targets. Note that
...
boundary". If the {\tt Eirene} input file is being produced by use of the
...
deductible from the {\tt DG} model is present.

Third, in \SOLPSITERns, the North and South boundaries of the {\tt B2.5} grid are
...
\begin{description}
  \item[NDX] Number of poloidal cells in the {\tt Eirene} computational grid
  \item[NDY] Number of radial cells in the {\tt Eirene} computational grid
  ...
  \item[NPLS] Number of plasma background species declared in the {\tt Eirene} input file
...
\begin{description}
  \item[dab2] Atom density ($m^{-3}$), dimension ({\tt NDX},{\tt NDY},{\tt NATM})
  \item[tab2] Atom temperature ($eV$), dimension ({\tt NDX},{\tt NDY},{\tt NATM})
  \item[dmb2] Molecule density ($m^{-3}$), dimension ({\tt NDX},{\tt NDY},{\tt NMOL})
  \item[tmb2] Molecule temperature ($eV$), dimension ({\tt NDX},{\tt NDY},{\tt NMOL})
  \item[dib2] Test ion density ($m^{-3}$), dimension ({\tt NDX},{\tt NDY},{\tt NION})
  \item[tib2] Test ion temperature ($eV$), dimension ({\tt NDX},{\tt NDY},{\tt NION})
  \item[rfluxa] Radial flux density of atoms ($m^{-2}.s^{-1}$), dimension ({\tt NDX},{\tt NDY},{\tt NATM})
  \item[rfluxm] Radial flux density of molecules ($m^{-2}.s^{-1}$), dimension ({\tt NDX},{\tt NDY},{\tt NMOL})
  \item[pfluxa] Poloidal flux density of atoms ($m^{-2}.s^{-1}$), dimension ({\tt NDX},{\tt NDY},{\tt NATM})
  \item[pfluxm] Poloidal flux density of molecules ($m^{-2}.s^{-1}$), dimension ({\tt NDX},{\tt NDY},{\tt NMOL})
  \item[refluxa] Radial energy flux density carried by atoms ($W.m^{-2}$), dimension ({\tt NDX},{\tt NDY},{\tt NATM})
  \item[refluxm] Radial energy flux density carried by molecules ($W.m^{-2}$), dimension ({\tt NDX},{\tt NDY},{\tt NMOL})
  \item[pefluxa] Poloidal energy flux density carried by atoms ($W.m^{-2}$), dimension ({\tt NDX},{\tt NDY},{\tt NATM})
  \item[pefluxm] Poloidal energy flux density carried by molecules ($W.m^{-2}$), dimension ({\tt NDX},{\tt NDY},{\tt NMOL})
  \item[emiss] $H_\alpha$ emissivity due to atoms ($photons.m^{-3}.s^{-1}$), dimension ({\tt NDX},{\tt NDY})
  \item[emissmol] $H_\alpha$ emissivity due to molecules and molecular ions ($photons.m^{-3}.s^{-1}$), dimension ({\tt NDX},{\tt NDY})
  \item[srcml] Molecule particle source ($A$), dimension ({\tt NDX},{\tt NDY},{\tt NMOL})
  \item[edissml] Energy spent for dissociating hydrogenic molecules ($W$), dimension ({\tt NDX},{\tt NDY},{\tt NMOL})
  \item[wldnek] Heat transferred by neutrals ($W$), total over strata, dimension ({\tt NLIM}+{\tt NSTS})
  \item[wldnep] Potential energy released by neutrals ($W$), total over strata, dimension ({\tt NLIM}+{\tt NSTS})
  \item[wldna] Flux of atoms impinging on surface ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
  \item[ewlda] Average energy of impinging atoms on surface ($eV$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
  \item[wldnm] Flux of molecules impinging on surface ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
  ...
  \item[wldra] Flux of reflected atoms from surface ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
  \item[wldrm] Flux of reflected molecules from surface ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
  \item[For each stratum $i=1,nstra$] The above data, resolved per stratum:
  \begin{description}
    \item[wldnek(i)] Heat transferred by neutrals ($W$), dimension ({\tt NLIM}+{\tt NSTS})
    \item[wldnep(i)] Potential energy released by neutrals ($W$), dimension ({\tt NLIM}+{\tt NSTS})
    \item[wldna(i)] Flux of atoms impinging on surface ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
    \item[ewlda(i)] Average energy of impinging atoms on surface ($eV$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
    \item[wldnm(i)] Flux of molecules impinging on surface ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
    \item[ewldm(i)] Average energy of impinging molecules on surface ($eV$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
    \item[wldra(i)] Flux of reflected atoms from surface ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
    \item[wldrm(i)] Flux of reflected molecules from surface ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
  \end{description}
  \item[wldpp] Flux of plasma ions impinging on surface ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NPLS})
  \item[wldpa] Net flux of atoms emitted from surface ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
  \item[wldpm] Net flux of molecules emitted from surface ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
  \item[wldpeb] Power carried by particles emitted from surface ($W$), total over strata, dimension ({\tt NLIM}+{\tt NSTS})
  \item[wldspt] Flux of sputtered wall material ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS})
  \item[wldspta] Flux of sputtered wall material per atom ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
  \item[wldsptm] Flux of sputtered wall material per molecule ($A$), total over strata, dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
  \item[For each stratum $i=1,nstra$] The above data, resolved per stratum:
  \begin{description}
    \item[wldpp(i)] Flux of plasma ions impinging on surface ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NPLS})
    \item[wldpa(i)] Net flux of atoms emitted from surface ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
    \item[wldpm(i)] Net flux of molecules emitted from surface ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
    \item[wldpeb(i)] Power carried by particles emitted from surface ($W$), dimension ({\tt NLIM}+{\tt NSTS})
    \item[wldspt(i)] Flux of sputtered wall material ($A$), dimension ({\tt NLIM}+{\tt NSTS})
  \item[wldspta(m)] Flux of sputtered wall material per atom ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NATM})
  \item[wldsptm(m)] Flux of sputtered wall material per molecule ($A$), dimension ({\tt NLIM}+{\tt NSTS},{\tt NMOL})
  \end{description}
  ...
  \item[wlpump(A)] Pumped flux per atom ($A$), dimension ({\tt NATM}, {\tt NLIM}+{\tt NSTS})
  \item[wlpump(M)] Pumped flux per molecule ($A$), dimension ({\tt NMOL}, {\tt NLIM}+{\tt NSTS})
  \item[wlpump(I)] Pumped flux per test ion ($A$), dimension ({\tt NION}, {\tt NLIM}+{\tt NSTS})
  \item[wlpump(P)] Pumped flux per plasma ion ($A$), dimension ({\tt NPLS}, {\tt NLIM}+{\tt NSTS})
  \item[eneutrad] Radiation rate due to atoms ($W$), dimension ({\tt NDX},{\tt NDY},{\tt NATM})
  \item[eirdiag\_nds] Indices for segments on resolved non-standard surfaces, dimension ($5 \times$ {\tt NSTS} $+ 1$)
  \item[sarea\_res] Surface area of surface segment ($m^2$), dimension ({\tt NCL})
  \item[wldna\_res] Flux of atoms impinging on surface segment ($A$), dimension ({\tt NATM}, {\tt NCL})
  \item[wldnm\_res] Flux of molecules impinging on surface segment ($A$), dimension ({\tt NMOL}, {\tt NCL})
  \item[ewlda\_res] Average energy of impinging atoms on surface segment ($eV$), dimension ({\tt NATM}, {\tt NCL})
  \item[ewldm\_res] Average energy of impinging molecules on surface segment ($eV$), dimension ({\tt NMOL}, {\tt NCL})
  \item[ewldea\_res] Energy flux carried by emitted atoms from surface segment ($W$), dimension ({\tt NATM}, {\tt NCL})
  \item[ewldem\_res] Energy flux carried by emitted molecules from surface segment ($W$), dimension ({\tt NMOL}, {\tt NCL})
  \item[ewldrp\_res] Total energy flux carried by emitted particles from surface segment ($W$), dimension ({\tt NCL})
  \item[ewldmr\_res] Flux of emitted molecules from recycling atoms ($A$), dimension ({\tt NMOL}, {\tt NCL})
  \item[wldspt\_res] Flux of sputtered wall material ($A$), dimension ({\tt NCL})
  \item[wldspta\_res] Flux of sputtered wall material per atom ($A$), dimension ({\tt NCL}, {\tt NATM})
  \item[wldsptm\_res] Flux of sputtered wall material per molecule ($A$), dimension ({\tt NCL}, {\tt NMOL})
  \item[wlpump\_res(A)] Pumped flux per atom ($A$), dimension ({\tt NCL}, {\tt NATM})
  \item[wlpump\_res(M)] Pumped flux per molecule ($A$), dimension ({\tt NCL}, {\tt NMOL})
  \item[wlpump\_res(I)] Pumped flux per test ion ($A$), dimension ({\tt NCL}, {\tt NION})
  \item[wlpump\_res(P)] Pumped flux per plasma ion ($A$), dimension ({\tt NCL}, {\tt NPLS})
...
\begin{description}
  \item[PDENA] Atom particle density (in $cm^{-3}$), dimension ({\tt NTRII},{\tt NATM})
  \item[PDENM] Molecule particle density (in $cm^{-3}$), dimension ({\tt NTRII},{\tt NMOL})
  \item[PDENI] Test ion particle density (in $cm^{-3}$), dimension ({\tt NTRII},{\tt NION})
  \item[EDENA] Energy density carried by atoms (in $eV.cm^{-3}$), dimension ({\tt NTRII},{\tt NATM})
  \item[EDENM] Energy density carried by molecules (in $eV.cm^{-3}$), dimension ({\tt NTRII},{\tt NMOL})
  \item[EDENI] Energy density carried by test ions (in $eV.cm^{-3}$), dimension ({\tt NTRII},{\tt NION})
  \item[VXDENA] $X$-directed momentum density carried by atoms (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NATM})
  \item[VXDENM] $X$-directed momentum density carried by molecules (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NMOL})
  \item[VXDENI] $X$-directed momentum density carried by test ions (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NION})
  \item[VYDENA] $Y$-directed momentum density carried by atoms (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NATM})
  \item[VYDENM] $Y$-directed momentum density carried by molecules (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NMOL})
  \item[VYDENI] $Y$-directed momentum density carried by test ions (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NION})
  \item[VZDENA] $Z$-directed momentum density carried by atoms (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NATM})
  \item[VZDENM] $Z$-directed momentum density carried by molecules (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NMOL})
  \item[VZDENI] $Z$-directed momentum density carried by test ions (in $g.cm^{-2}.s^{-1}$), dimension ({\tt NTRII},{\tt NION})
  \item[VOL] Triangle volumes (in $cm^3$), dimension ({\tt NTRII})
  \item[PUX] $X$-component of the poloidal unit vector at the triangle center
  \item[PUY] $Y$-component of the poloidal unit vector at the triangle center
  \item[PVX] $X$-component of the radial unit vector at the triangle center
  \item[PVY] $Y$-component of the radial unit vector at the triangle center
\end{description}

Please note that, for the momentum densities, the coordinate system used by {\tt Eirene} is Cartesian, i.e. independent of the local magnetic field, and that this data is given in CGS units.
...
When running a coupled \SOLPSITER case to obtain a steady-state solution, it is recommended to use the source
...
{\tt MDSplus} documentation. Both of these variables should be set in your
\${\tt \{SOLPSTOP\}/SETUP/config.}\${\tt \{HOST\_NAME\}.}\${\tt \{COMPILER\}(.local)} file.
...
[At IPP on the TOK Linux cluster, add the following to {\tt .login}:
\begin{verbatim}
...
The {\tt MDSplus} server name is {\tt solps-mdsplus.aug.ipp.mpg.de:8001}.
...
\item {\tt MDS+} option
\item {\tt MDS+ server:} = {\tt solps-mdsplus.aug.ipp.mpg.de:8001}
...
\else
    \centerline{\includegraphics[origin=bl,angle=+90,height=8cm]{MDSPLUS/jetdsp_mdsplus_example.eps}}
\fi
...
"solps-mdsplus.aug.ipp.mpg.de:8001" contained 95977 cases consisting of
...
\item[hl2m] HL-2M (SWIP, Chengdu, China)
...
files named {\tt dg.*} in the {\tt Carre} run directory.
...
To run {\tt Carre}, assuming that you are working on a case from {\tt DG},
you would then need to do
\begin{verbatim}
  carre -
\end{verbatim}
and then either type {\tt return} to accept the default, or a {\tt y} if
asked to accept {\tt Carre}'s choices.
...
Lastly, there is a "hidden" language variable {\tt sellan}\index{sellan} (default value 'english') that can be changed
to its original 'francais'.

Once all the settings have been chosen, it is often convenient to save them, so as to not have to re-enter them on the next {\tt Carre} meshing attempt. This is achieved by means of the '(S)ave' option within the {\tt carre} script, provided that the user does not start the next meshing attempt with the '(P)repare' option, but rather jumps directly to '(G)rid'.

After running {\tt Carre}, it is recommended to have a look at the mesh and decide if one wants to modify it before
...
In some cases, the {\tt carre -} script will fail. In that case, one can still
run {\tt Carre} in "standalone" mode by invoking:
\begin{verbatim}
  ssc
...
An extensive review of {\tt B2-Eirene} has been published as \cite{SOLPS2006}\index{SOLPS review}.
...
below, can be found as part of the {\tt SOLPS} package at \${\tt \{SOLPSTOP\}/doc/CPP\_2006.pdf}.
...
\begin{gather}
\left\langle V_{iN} \sigma_{cx} \right\rangle = \frac{32 \sigma _{cx}}{3 \sqrt{\pi}}
\sqrt{\frac{T_i}{m_i}}
\frac{m^3}{s}
~.
\end{gather}.
...
\hfill        \tilde{q}_{ex} = \frac{3}{2} n T_e \left(- \frac{b_z}{B h_y}
     ...
     - \frac{3}{2} \frac{\partial T_e}{\partial x} \right)
     - D^{(AN)}_n \frac{1}{h_x n} \frac{\partial n}{\partial x}
     - D^{(AN)}_p \frac{1}{h_x n} \frac{\partial p_i}{\partial x} \right)
     + n V_{e\perp}^{(AN)}
      \hfill \\
...
\hfill  \tilde{q}_{ey} = \frac{3}{2} n T_e \left( \frac{B_z}{B^2} \frac{1}{h_x}
     ...
     - \frac{3}{2} \frac{\partial T_e}{\partial y} \right)
     - D^{(AN)}_n \frac{1}{h_y n} \frac{\partial n}{\partial y}
     - D^{(AN)}_p \frac{1}{h_y n} \frac{\partial p_i}{\partial y} \right)
     + n V_{ey}^{(AN)}
         \hfill \\
...
or in \${\tt \{SOLPSTOP\}/doc/CPP\_2006.pdf}) heat
...
\subsection{Derivation of parallel viscosity term in momentum equation}
...
a simpler and faster neutral fluid model is also available as {\tt B2.5} stand-alone. The surface sources of neutrals are
...
\begin{center}
    ...
    \hline quantity & functional dependence & \multicolumn{3}{|c|}{numerical prefactor} \\
    \hline          &                       & Braginskii & fits to Balescu & fits to Zhdanov \\
    ...
    \hline \endfirsthead
    \hline
    \multicolumn{5}{|l|}{\small\sl continued on next page} \\
    \hline \endfoot
    \endlastfoot
$\kappa _{e \parallel}$ & $\frac{T_e^{5/2} n_e}{\ln \Lambda \sum \limits_a Z_a^2 n_a}$ & 3.16 & $\frac{5}{2} \frac{2.16 Z_{eff}}{1 + 0.27 Z_{eff}}$ & $\frac{\left( 3.9 +\frac{2.3}{Z_{eff}} \right)}{0.31+\frac{1.2}{Z_{eff}}+\frac{0.41}{Z_{eff}^2}}$ \\ \hline
$\kappa _{i \parallel}$ & $\frac{T_i^{5/2}}{\ln \Lambda}\sum \limits_a \frac{n_a}{Z_a^2}$ & $\frac{3.9}{\mathlarger{\sum \limits_b} Z_b^2 n_b \sqrt{\frac{2 m_a m_b}{m_a + m_b}}}$ & \multicolumn{2}{|c|}{$\frac{3.95}{\sqrt{m_a} \sum \limits_b Z_b^2 n_b}$} \\ \hline
$\eta _{a \parallel}$ & $\frac{T_i^{5/2}}{\ln \Lambda}\frac{n_a}{Z_a^2}$ & $\frac{1.37}{\mathlarger{\sum \limits_b} \frac{Z_b^2 n_b}{\sqrt{\frac{m_a + m_b}{2}}}}$ & \multicolumn{2}{|c|}{$\frac{\sqrt{2} \; 0.96 \sqrt{m_a}}{\sum \limits_b Z_b^2 n_b}$} \\ \hline
...
The version of the equations shown in this Appendix represents the best available description of the standard recommended settings for the \SOLPSITER physics model, built upon the original work on {\bf SOLPS5.2} from the St. Petersburg group \cite{ROZHANSKY2009}. This Appendix is not meant as an exhaustive registry of the capabilities of the code, for which the reader is encouraged to peruse Chapters \ref{cha:standaloneb2} and \ref{cha:coupledb2eirene} as well as the list of switches in Appendix \ref{lis:b2cdci} and of namelist variables in Appendix \ref{lis:b2cdcn}.

\section{Sign conventions}\index{sign conventions}\label{sec:signs}
...
\begin{equation}
\begin{matrix}
\widetilde{ \Gamma}_{ax} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + V_{ax}^{\left( AN \right)} + V_a^{corr\_dpc} \right) n_a + 0 \cdot \frac{\delta_{1,a}}{e} \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} \right) \\
&- D_{n,a} \frac{1}{h_x} \frac{\partial n_a}{\partial x} - \widetilde{D}_{p,ax} \frac{1}{h_x} \frac{\partial p_a}{\partial x} - D_{axy} \frac{1}{h_y} \frac{\partial n_a T_i}{\partial y}
\end{matrix}
\end{equation}

\begin{equation}
\begin{matrix}
\widetilde{ \Gamma}_{ay} =& \left( V_{ay}^{\left( E \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} \right) \\
& - D_{n,a} \frac{1}{h_y} \frac{\partial n_a}{\partial y} - \widetilde{D}_{p,ay} \frac{1}{h_y} \frac{\partial p_a}{\partial y} + D_{ayx} \frac{1}{h_x} \frac{\partial n_a T_i}{\partial x}
\end{matrix}
\end{equation}

\begin{equation}
\begin{matrix}
\Gamma_{ax} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + \widetilde{V}_{ax}^{\left( dia \right)} + V_{ax}^{\left( AN \right)} + V_a^{corr\_dpc} \right) n_a + \\
& 0 \cdot \frac{\delta_{1,a}}{e} \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_x} \frac{\partial n_a}{\partial x} - \widetilde{D}_{p,ax} \frac{1}{h_x} \frac{\partial p_a}{\partial x}
\end{matrix}
\end{equation}

\begin{equation}
\begin{matrix}
\Gamma_{ay} =& \left( V_{ay}^{\left( E \right)} + \widetilde{V}_{ay}^{\left( dia \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} \right) \\
...
\begin{equation}
\begin{matrix}
\Gamma_{ax}^{Cor} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + 2 \widetilde{V}_{ax}^{\left( dia \right)} + V_{ax}^{\left( AN \right)} + V_a^{corr\_dpc} \right) n_a + \\
& 0 \cdot \frac{\delta_{1,a}}{e} \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_x} \frac{\partial n_a}{\partial x} - \widetilde{D}_{p,ax} \frac{1}{h_x} \frac{\partial p_a}{\partial x}
\end{matrix}
\end{equation}
...
\begin{equation}
\begin{matrix}
\Gamma_{ay}^{Cor} =& \left( V_{ay}^{\left( E \right)} + 2 \widetilde{V}_{ay}^{\left( dia \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \\
& \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_y} \frac{\partial n_a}{\partial y} - \widetilde{D}_{p,ay} \frac{1}{h_y} \frac{\partial p_a}{\partial y}
\end{matrix}
\end{equation}
...
\begin{equation}
\begin{matrix}
\Gamma_{ax}^{\left( Eir \right)} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + V_{ax}^{\left( dia \right)} + V_{ax}^{\left( AN \right)} + V_a^{corr\_dpc} \right) n_a + \\
& 0 \cdot \frac{\delta_{1,a}}{e} \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_x} \frac{\partial n_a}{\partial x}
\end{matrix}
\end{equation}

\begin{equation}
\begin{matrix}
\Gamma_{ay}^{\left( Eir \right)} =& \left( V_{ay}^{\left( E \right)} + V_{ya}^{\left( dia \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \\
& \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_y} \frac{\partial n_a}{\partial y}
\end{matrix}
\end{equation}
...
and $\delta_{a,b}$ is the Kronecker delta (1 if $a=b$, 0 otherwise). Here, it is used as shortcut to identify the one hydrogenic species with $z_a = 1$ and $z_{n,a} = 1$. If the code contains multiple hydrogenic species, an error will be reported to avoid double-counting of the current contributions.
...
\begin{equation}
r_{CX_{ab}} = \exp \left(
	r_{0,CX_{ab}} + r_{1,CX_{ab}} \ln  \left(
		\frac{m_p T_i} {m_b e}
	\right)
\right)
\end{equation}
...
& \left\{
\begin{matrix}
-c_{E\times B}   \frac{B_z}{B^2}   \frac 1{h_y}  \frac{\partial \Phi }{\partial y}, & \ z_a \neq 0 \\
0, & \ z_a = 0
\end{matrix} \right.
\end{matrix}
\end{equation}
...
& \left\{
\begin{matrix}
c_{E\times B}   \frac{B_z}{B^2}   \frac 1{h_x}   \frac{\partial \Phi }{\partial x}, & \ z_a \neq 0 \\
0, & \ z_a = 0
\end{matrix} \right.
\end{matrix}
\end{equation}
...
\begin{equation*}
V_{ay}^{(AN)} = c_{V_a^{(AN)},0} + c_{V_a^{(AN)},1} \frac{10^{20}}{n_e} + c_{V_a^{(AN)},2} D_{f,a} + c_{V_a^{(AN)},3} \frac{T_e}{16 e|B|}
\end{equation*}
...
\begin{equation}
D^{(AN)}_{n,a}=\left( c_{D_{n,a},0}+c_{D_{n,a},1}\frac{10^{20}}{n_e}+c_{D_{n,a},2}D_{f,a}+c_{D_{n,a},3}\frac{T_e}{16 e|B|}\right)f_D(x,y),
\end{equation}
...
\alpha_{3,D}, & \ x,y\in \text{PFR} \cup \text{SOL}
\end{matrix} \right.
\end{matrix}
\end{equation}
...
$D_{f,a}=c_{D_{f,a},0} + c_{D_{f,a},1} \frac{10^{20}}{n_i}
+ \frac{
	c_{D_{f,a},2} \sqrt{\frac{T_i}{m_a}}
}{
	c_{D_{f,a},3} n_i + c_{D_{f,a},4} n_e
},$
...
\begin{matrix}
\frac{D_{p,a}}
{\left(1+\left(\frac{\frac{\sqrt{g}}{h_x} D_{p,a} \left| \frac{1}{h_x} \frac{\partial p_a}{\partial x} \right| }{\frac{\sqrt{g}}{h_x} \mathit{\alpha n}_a \frac{V_{T_i}} 4} \right)^{\gamma _1} \right)^{\frac 1{\gamma_1}}},
...
\begin{matrix}
\frac{D_{p,a}}
{\left(1+\left(\frac{\frac{\sqrt{g}}{h_y} D_{p,a} \left| \frac 1{h_y}\frac{\partial p_a}{\partial y} \right| }{\frac{\sqrt{g}}{h_x} \mathit{\alpha n}_a\frac{V_{T_i}} 4}\right)^{\gamma _1}\right)^{\frac 1{\gamma_1}}},
& \ V_{T_i}=\sqrt{\frac{8T_i}{\mathit{\pi m}_a}}, & \ z_a = 0 \\
D_{p,a}, & \ z_a \neq 0 \\
\end{matrix} \right.
\end{matrix}
...
\begin{equation*}
D^{(AN)}_{p,a}=\frac 1{z_aT_e+T_i} \left( c_{D_{p,a},0} + c_{D_{p,a},1} \frac{10^{20}}{n_e} + c_{D_{p,a},2} D_{f,a} + c_{D_{p,a},3} \frac{T_e}{16 e|B|}\right),
\end{equation*}
...
= S_{a \parallel}^m + S_{CF_a}^m + S_{fr_a}^m &+ S_{Therm_a}^{m} + S_{I_a}^m + S_{R_a}^m + S_{CX_a}^m + S_{AN_a}^m
\end{align}
...
with hydrogenic ion mixtures or non-hydrogenic plasmas, the use of Eq.
(\ref{eq:mom_old}) is preferred.

%-------- end of 4th page -------------

\subsection{Sources for parallel momentum equations}
...
$S_{AN_a}^m$ is a correcting source due to the radial anomalous current (here assumed to be carried by hydrogenic ions only):

\begin{equation*}
S_{AN_a}^m = \left\{
\begin{matrix}
0, & z_a \ne 1 \mbox{ or } z_{n,a} \ne 1 \\
- j_y^{(AN)} \frac{B B_x}{B_z}, & z_a = 1 \mbox{ and } z_{n,a} = 1 \\
\end{matrix}
\right.
\end{equation*}
...
& \left\{
\begin{matrix}
-r_{R_a} n_e n_a \left( \frac{m_a+m_{a-1}}2 \right) V_{\parallel a}, & \ a=0,\ldots,ns-1, & \ z_{a-1} < z_a \text{ and } z_{n,a-1}= z_{n,a}, \\
0, \\
\end{matrix} \right.
\end{matrix}
\end{equation}
...
$S_{Therm,ea}^m = - \alpha_{ex} c_e^{(1)} \frac{n_a z_a^2 m_e}{b_x e \zeta_e} \frac{1}{c_{\sigma^{(CL)}_{\parallel}}^{(Luciani)}} \frac{\partial T_e}{h_x \partial x}, ~ z_a \ne 0$
...
$S_{Therm,ia}^m = \sum \limits_{\substack{b=0 \\ b \ne a}}^{ns - 1} c_{\alpha_{ab}}^{(Flim)} z_a^2 z_b^2 \frac{m_p}{\zeta_p e} \frac{n_a n_b}{n_a + n_b} \alpha_{ab} \frac{\partial T_i}{h_x \partial x}, ~z_a \ne 0$
...
\frac{1}{1+\frac{\left| \alpha_{ab} \frac{\partial T_i}{h_x \partial x} \right|}{c_{flab} j_{ab, \max}}}, & \mbox{outside the separatrix}
\end{matrix}
\right.
...
\max \{ 5; 17.3 - 0.5 \cdot \ln \left( \frac{n_e}{10^{20}} \right) + 1.5 \cdot \ln \left( \frac{T_e}{e \cdot 10^3} \right)  \} & \mbox{\scriptsize{Wesson formula}} &\mbox{\scriptsize{for ion-ion collisions}}\\
\end{matrix} \right.
\end{matrix}
\end{equation}
...
\eta_{ax} \frac{\partial V_{\parallel a}}{h_x \partial x}, & z_a = 1~\&~z_{n,a} = 1 \\
m_a V_{\parallel a} \Gamma_{ax}^{Cor} -
...
\subsection{Parallel momentum transport coefficients}
...
c_{\eta _a,3} \frac{T_e}{16 e|B|}\right)
...
\alpha_{3,\eta _a}, & \ x,y\in \text{PFR} \cup \text{SOL}
\end{matrix} \right.
\end{matrix}
\end{equation}
...
\frac 1
{1 + \frac{
\frac{\sqrt g}{h_x} \widetilde{\eta}^{(CL)}_{ax}  \left| \frac{\partial V_{\parallel a}}{h_x \partial x} \right|
}
{V_{m_a}^{(max)}}
}, &
...
$V_{m_a}^{(max)} = c_{flim_2} \frac{\sqrt g}{h_x} |b_x| n_a T_i$, ~ $c_{flim_2} = \frac{3}{8}$

$\widetilde{\eta}^{(CL)}_{ax} = c_{\eta_{ax}^{(CL)}}^{Luciani} \eta^{(CL)}_{ax}$
...
, & \text{for hydrogenic ions on inner flux surfaces,} \\
...
$c \left( y \right) = \mathlarger{\oint} h_x |b_x|^{-1} dx$, \ \ \ \  $\varepsilon = \frac
{\left( \mathlarger{\oint} h_x dx \right)^2}
...
\left( \frac{4 \pi \varepsilon_0}{e^2} \right)^2$, where the Coulomb logarithm $\ln \Lambda $
can be chosen as
...
$\eta_{ay} = \eta^{(AN)}_a + \eta^{(CL)}_{ay}$

$\widetilde{q}_a^{(0)} = q^{(1)} \arctan \left( \frac{q_a^{(0)}}{q^{(1)}} \right), \; q^{(1)} = c_{q1} n_i T_i \sqrt{\frac{T_i + T_e}{m_i}}$
...
\begin{matrix}
q_a^{(0)} = q_{a\parallel}^{(0)} +
\frac B{B_x} q_{ax}^{(dia)} & =
& \left\{
\begin{matrix}
0 & \text{outside the separatrix} \\
-\frac 5 2 \frac{B_z B}{B_x \left\langle B^2 \right\rangle} \frac{n_a T_i}e \frac{\partial T_i}{h_y \partial y}
...
e\right)^2}{c(y) \sum \limits^{ns-1}_{a=0} z_a^2 n_a}}
\cdot
\frac 1 {1 +
\frac{15.12 \cdot 10^{16} \cdot \varepsilon^{3/2} \left( \frac{T_i} e \right)^2}
...
When Eq. (\ref{eq:mom_new}) is used ({\tt 'b2sigp\_style'} set to {\tt '2'}), we have:
...
Otherwise, with Eq. (\ref{eq:mom_old}):
...
(T_e - T_i),\;\;\; c_{eqp}=4.8 \cdot 10^{-15},\;\;$ where $\ln \Lambda $ can be chosen as
...
\max \{ 5; 15.2 - 0.5 \cdot \ln \left( \frac{n_e}{10^{20}} \right) + \ln \left( \frac{T_e}{e \cdot 10^3} \right)  \} && \text{Wesson formula for electron-ion collisions} \\
\end{matrix} \right.
\end{matrix}
$

$Q_R = \sum \limits_{a=0}^{ns-1} r_{Q_a} n_e n_a$ from b2stel

$Q_{Fei} = V_{e \parallel} \sum \limits_{\substack{a=0 \\ z_a \neq 0}}^{ns - 1}
...
\begin{equation*}
\widetilde{q}_{ex} = \widetilde{f}_{ex} T_e - \kappa_{ex} \frac 1{h_x} \frac{\partial T_e} {\partial x} - q_{ex}^{P.Sch}
\end{equation*}

\begin{equation*}
\widetilde{f}_{ex} = \frac 3 2 \Gamma_{ex} + V_{ex}^{(str)} n_e - \widetilde{c_e}^{(2)} \frac{j^{ \left( \parallel \right) }_x}e
\end{equation*}
...
q_{ex} =
\left[
	\frac 3 2 \Gamma_{ex} - \widetilde{c_e}^{(2)} \frac{j^{\left( \parallel \right) }_x} e
	-
...
\Gamma_{ax}^{(he)} =
\left(
	b_x V_{a \parallel} + V_{ax}^{(E)} + \frac 5 3 V_a^{(AN)} + V_a^{corr\_dpc}
\right) n_a -
\frac 5 3 \left[
	D_{n,a} \frac 1{h_x} \frac{\partial n_a}{\partial x} + \widetilde{D}_{p,ax} \frac{1}{h_x} \frac{\partial p_a}{\partial x}
\right]
\end{equation*}
...
\left[
	...
	- \widetilde{c_e}^{(2)} b_x \frac{\widetilde j_{\parallel}} e
\right] T_e -
\kappa_{ex} \frac 1{h_x} \frac{\partial T_e}{\partial x}
\end{equation*}
...
\right) n_e -
D_{n,a} \frac 1{h_x} \frac{\partial n_a}{\partial x}
\end{equation*}
...
\right) n_e -
D_{n,a} \frac 1{h_y} \frac{\partial n_a}{\partial y}
\end{equation*}
...
when {\tt 'b2sigp\_style'} is set to {\tt '2'} (default) or to its limiting
...
\alpha_{3,h_e}, & \ x,y\in \text{PFR} \cup \text{SOL}
\end{matrix} \right.
\end{matrix}
$

$\widetilde{\widetilde{\kappa}}_{ex}^{(CL)} = c_{\kappa_{ex}}^{(Flim)} \widetilde{\kappa}_{ex}^{(CL)}$
...
\max \{ 5; 23.4 - 1.15 \cdot \log_{10} \left( \frac{n_e}{10^6} \right) + 3.45 \cdot \log_{10} \left( \frac{T_e}e \right)  \}, & \frac{T_e}{e} \leq 50 & \text{Braginsky formula} \\
\max \{ 5; 25.3 - 1.15 \cdot \log_{10} \left( \frac{n_e}{10^6} \right) + 2.30 \cdot \log_{10} \left( \frac{T_e}e \right)  \}, & \frac{T_e}{e} > 50  \\
\max \{ 5; 14.9 - 0.5 \cdot \ln \left( \frac{n_e}{10^{20}} \right) + \ln \left( \frac{T_e}{e \cdot 10^3} \right)  \} && \text{Wesson formula for electron-electron collisions} \\
\end{matrix} \right.
\end{matrix}
$
...
$\alpha^{(CL)}_{x} = - b_x^2 \frac{e}{m_e} \frac{\widetilde{c_e}^{(2)}}{c_e^{(1)}} n_e \tau_e$
when {\tt 'b2sigp\_style'} is set to {\tt '2'} (Zhdanov formulation, default), and
$\alpha^{(CL)}_{x} = b_x^2 \frac{e}{m_e} \sqrt{\frac 5 2} f_{\alpha} \left(z_{eff} \right) n_e \tau _e$ otherwise (Balescu formulation), and
...
\begin{equation*}
\widetilde{\widetilde{\alpha}}^{(CL)}_{x} = c_{\alpha_{ex}}^{(F\text{lim})} \widetilde{\alpha}^{(CL)}_{x}
\end{equation*}
\begin{equation*}
\widetilde{\alpha}^{(CL)}_{x} = c_{\kappa_{ex}^{(CL)}}^{(Luciani)} \alpha^{(CL)}_{x}
\end{equation*}
\begin{equation*}
c_{\alpha_{ex}}^{(F\text{lim})} = \left\{
\begin{matrix}
...
\begin{equation*}
\widetilde q_{ey} = \widetilde f_{ey} T_e - \kappa_{ey} \frac 1{h_y} \frac{\partial T_e}{\partial y} - q_{ey}^{P.Sch}
\end{equation*}
...
\Gamma_{ay}^{(he)} = \left( \frac 5 3 V_a^{(AN)} + V_{ay}^{(E)} \right) n_a - \frac 5 3 \left[
	D_{n,a} \frac 1{h_y} \frac{\partial n_a}{\partial y} +
	\widetilde D_{p,ay} \frac 1{h_y} \frac{\partial	p_a}{\partial y}
\right]
\end{equation*}
...
$Q_R^{(i)} = \sum \limits_{a=0}^{ns-1} Q_{R_a}^{(i)}$,
...
\max \{ 5; 23.4 - 1.15 \cdot \log_{10} \left( \frac{n_e}{10^6} \right) + 3.45 \cdot \log_{10} \left( \frac{T_e}e \right) \}, & \frac{T_e}{e} \leq 50 & \text{Braginsky formula} \\
\max \{ 5; 25.3 - 1.15 \cdot \log_{10} \left( \frac{n_e}{10^6} \right) + 2.30 \cdot \log_{10} \left( \frac{T_e}e \right) \}, & \frac{T_e}{e} > 50  \\
\max \{ 5; 17.3 - 0.5 \cdot \ln \left( \frac{n_e}{10^{20}} \right) + 1.5 \cdot \ln \left( \frac{T_e}{e \cdot 10^3} \right) \} && \text{Wesson formula for ion-ion collisions} \\
\end{matrix} \right.
\end{matrix}
$
...
\begin{equation*}
\widetilde f_{ix} = \frac 3 2 \Gamma_{ix} + V_{ix}^{(str)} n_i + 0 \cdot \frac{3}{2} \left( \frac{j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)}}{e} \right)
\end{equation*}
...
q_{ix} = \left[
	...
	V_{ix}^{(str)} n_i +
        0 \cdot \frac{3}{2} \left( \frac{j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)}}{e} \right)
\right] T_i
...
\alpha_{3,h_a}, & \ x,y\in \text{PFR} \cup \text{SOL}
\end{matrix} \right.
\end{matrix}
$

$\widetilde{\widetilde{\kappa}}_{ix}^{(CL)} = c_{\kappa_{ix}}^{(flim)} \widetilde{\kappa}_{ix}^{(CL)}$
...
$c_{flim_1}$ is the ion heat flux limiter coefficient.

$\tilde{\kappa}_{ix}^{(CL)} = c_{\kappa_{ix}^{(CL)}}^{(Luciani)} \kappa_{ix}^{(CL)}$,
...
For the integral modeling ({\tt 'b2trcl\_integral\_modeling'} set to {\tt '1'}) in the region of the overlap with {\tt ASTRA} in the inner part of the core:
...
here $\left\langle B^2 \right\rangle = \frac{\mathlarger{\oint} B^2 \sqrt{g} dx}{\mathlarger{\oint} \sqrt g dx}$

from {\tt ASTRA} we get $T_i/e$ and $\oint q_i^{NEO} ds$ as a functions of $R_{\text{(outer midplane)}}$ and calculate

$K_{ASTRA} = \frac{-1}{\oint q_i^{NEO} ds} \frac{d(T_i/e)}{dR_{\text{(outer midplane)}}}$ as a function of the flux surface in {\tt B2.5}
...
\widetilde f_{iy} = \frac 3 2 \Gamma_{iy} + V_{iy}^{(str)} n_i + \frac 3 2 \frac{\left(
	j_y^{(AN)} + \widetilde j_y^{(vis \parallel)} + \widetilde j_y^{(vis \perp)} + \widetilde j_y^{(visq)} + j_y^{(in)}
\right)} e
\end{equation*}
...
$j_x^{(AN)} = - \sigma^{(AN)} \frac{\partial \Phi}{h_x \partial x}$
...
\begin{equation}
\widetilde j_x^{(visq)} =
- 0.24 \sum \limits^{ns-1}_{\substack{a = 0 \\ z_a = 1~\&~z_{n,a} = 1 }}
...
\left\{
\begin{matrix}
\frac{m_a n_a \left\langle V_{aa_0} \sigma_{ex} \right\rangle n_{a_0}}{2 B^2}, &
...
\begin{equation}
\left\langle V_{aa_0} \sigma_{ex} \right\rangle = 3.2 \cdot 10^{-15} \sqrt{\frac{T_i}{0.026e}}
\end{equation}
...
$j_y = j_y^{(AN)} + \widetilde j_y^{(dia)} + j_y^{(in)} + \widetilde j_y^{(vis \parallel)} + \widetilde j_y^{(vis \perp)}
+ \widetilde j_y^{(visq)} + \widetilde j_y^{(s)} + j_y^{(ST)}$,

$j_y^{(AN)} = - \sigma^{(AN)} \frac{\partial \Phi}{h_y \partial y} $
...
\begin{equation}
j_y^{(in)} = - \frac{B_z}2 \frac{\partial}{h_x \partial x}
\left( \frac{1}{B^2} \right)
\sum \limits^{ns-1}_{\substack{a = 0 \\ z_a \neq 0}} m_a n_a V^2_{a \parallel},
\end{equation}
...
$V_{a_{0}x}=\frac{\Gamma_{a_{0}x}}{n_{a_0}}$

$\sigma_{aa_0}$ see above
...
%-------- end of 20th page -------------
\section{Typical boundary conditions}

In this Section, the boundaries are denoted as $S_0$ for the PFR boundaries, $S_1$ for the core boundaries, $E$ for the East boundaries (right targets), $W$ for the West boundaries (left targets) and $N$ for the outer main wall boundary.
...
$c_{nsr}$ --- neutral sources rescale coefficient, $1$ --- when working with B2.5 stand-alone and $10^{-10}$ when coupling to Eirene
...
\left. \left[
	...
	\right) n_a
	+
	\frac{\sqrt g}{h_y} D_{ayx} \frac1{h_x} \frac{\partial n_a T_i}{\partial x}
	+
	\frac{\sqrt g}{h_y} V_{ay}^{(E)} n_a
\right] \right|_{S_0}, & mdf\_fnb = 1
\end{matrix} \right.
\end{matrix}
\end{equation}
...
{\em Imposing a sound speed outgoing flux:} {\tt BCCON = 14}
...
\sqrt{\frac{\sum \limits_{b=0}^{ns-1} p_b}{\sum \limits_{b=0}^{ns-1} m_b n_b}} n_a
-
...
c_{m,a,3} h_z \left. \frac{\sqrt g}{h_y} m_a n_a V_{\parallel a} \right|_{S_1} + \left. \left[
...
{\em Prescribe the value of the parallel velocity as a function of poloidal coordinate:} {\tt BCMOM = 15}
...
\frac{\mathlarger{\oint} B \sqrt{g} \, dx}{\mathlarger{\oint} \sqrt{g} \, dx}
$, the integration is done over inner magnetic surfaces

$v_{big} \approx 10^{12}$, $n_{e0} = 10^{20}$, $p_{m,a,1}$ is a given value of the average parallel velocity, in $m.s^{-1}$.
...
{\em Imposing the {\tt b2stbc\_spb} style boundary condition on the velocity:} {\tt BCMOM = 13}
...
\sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_a}
{\sum \limits_{a=0}^{ns-1} m_a n_a}}$
...
$p_{m,a,1}=0$ is the imposed velocity in $m.s^{-1}$, $u_{big} = 10^{18}$, $n_{e00} = 10^{20}$
...
$c_s = \sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_a}{\sum \limits_{a=0}^{ns-1} m_a n_a}}$
...
{\em Imposing the {\tt b2stbc\_spb} style boundary condition for parallel velocity:} {\tt BCMOM = 13}
...
$p_{m,a,1}=0$ is the imposed parallel velocity in $m.s^{-1}$, $u_{big} = 10^{18}$, $n_{e00} = 10^{20}$
...
\left(
	\widetilde j_y^{(dia)} + j_y^{(in)} + \widetilde j_y^{(vis \parallel)} + j_y^{(vis \perp)}
...
\frac{\partial}{h_x \partial x}
\left( \frac{1}{B^2} \right)
\sum \limits^{ns-1}_{\substack{a = 1 \\
...
\left. - \frac{\sqrt{g}}{h_y}
\left(
	\widetilde j_y^{(dia)} + j_y^{(in)} +
        \widetilde j_y^{(vis \parallel)} + j_y^{(vis \perp)} +
	\widetilde j_y^{(visq)} + \widetilde j_y^{(s)}
\right) \right|_N
, & facdrift \neq 0 \\
\end{matrix} \right.
...
{\em Imposing a sound speed velocity flux (to be used in connection with {\tt BCCON = 14}):} {\tt BCPOT = 11}

\begin{small}
$\left. \frac{\sqrt g}{h_x} j_x \right|_W =
\frac{\sqrt g}{h_x} e \left. \left[
	- \sum \limits_{b=0}^{ns-1} z_a \left(
		p_{n,a,1} |b_x| \sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_b}{\sum \limits_{b=0}^{ns-1} m_b n_b}} n_a
	\right) +
...
if {\tt 'b2stbc\_istyle\_fchi'} is {\tt '1'}, or
...
if {\tt 'b2stbc\_istyle\_fchi'} is {\tt '0'}.
...
$p_{h_e,1} = 8 \cdot 10^5$ is the imposed total electron energy flux, in $W$.
...
$\left. -\frac{\sqrt g}{h_y} \widetilde q_{ey} \right|_N = \left. \left[
	\left(
		p_{h_e,2} \frac{\sqrt g}{h_y} \sqrt{\frac{T_e}{m_e}}
	\right) T_e n_e
	+
	p_{h_e,1} \max \left(
		0, \frac{\sqrt g}{h_y} \widetilde f_{ey}
...
$p_{h_e,1} = 10^{-2}$ is the radial electron temperature decay length, in $m$.
...
If {\tt 'bcene\_15\_style'} is set to {\tt '0'} (to be used in conjunction with {\tt BCCON = 14}) :
...
If {\tt 'bcene\_15\_style'} is set to {\tt '1'}:

$\left. \frac{\sqrt g}{h_x} \widetilde q_{ex} \right|_W = \left. \left[
	-\frac{\sqrt g}{h_x} |b_x| \left(
		\left( 1 + \gamma_e \right) T_e +
		\left( 1 - \gamma_e \right) e \Phi
...
$p_{h_e,1} = 0.9$, $p_{\Phi,2} = 0$ is the applied electric potential at the target, in $V$.
...
$p_{h_i,1} = 10^{-2}$ is the radial ion temperature decay length, in $m$.
...
$\left. \frac{\sqrt g}{h_y} \widetilde q_{iy} \right|_{S_1} = \left. \left[ p_{h_i,1} \frac{
	...
	{\sum \limits_{core} \left( \frac{\sqrt g}{h_y} \right)}
        - q_{iy}^{P.Sch}
        + \frac{3}{2} \frac{\left( j_y^{(AN)} + j_y^{(in)} + \widetilde{j}_y^{(vis \parallel)} + \widetilde{j}_y^{(visq)} + 0 \cdot j_y^{(vis \perp)} \right)}{e} T_i
\right] \right|_{S_1}$

$p_{h_i,1} = 8 \cdot 10^5$ is the imposed total ion energy flux, in $W$.
...
\begin{equation*}
\begin{matrix}
\left. \frac{\sqrt g}{h_y} \widetilde q_{iy} \right|_{N} = &
& \left\{
\begin{matrix}
\begin{matrix}
\left[ \frac 5 2 \sum \limits_{a=0}^{ns-1} S_{n_a,0}^{no\_mdf} T_i
...
\begin{equation*}
\begin{matrix}
\left. -\frac{\sqrt g}{h_y} q_{iy} \right|_N = &
& \left. \left[
\begin{matrix}
\frac 5 2 \sum \limits_{a=0}^{ns-1} S_{n_a,0} T_i
-
\frac{\sqrt g}{h_y} \frac{\kappa_i^{(AN)}}{p_{h_i,1}} T_i
+
\frac 5 2 \sum \limits_{a=0}^{ns-1} S_{n_a,1} n_a T_i + \\
...
{\em Boundary condition from {\tt b2stbc\_spb}:} {\tt BCENI = 15} (to be used in association with {\tt BCCON = 14})

$\left. \frac{\sqrt g}{h_x} \widetilde q_{ix} \right|_W = \left.
- p_{h_i,1} \frac{\sqrt g}{h_x} |b_x|
\sqrt{\frac{\sum \limits_{b=0}^{ns-1} p_b}{\sum \limits_{b=0}^{ns-1} m_b n_b}} T_i
\sum \limits^{ns-1}_{\substack{a = 0 \\ z_a \neq 0}} p_{n,a,1} n_a
-
...
\left. \left[
	\left(
		c_{h_i,a,7}
		\frac{\sqrt g}{h_x} |b_x|
		\sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_a}{\sum \limits_{a=0}^{ns-1} m_a n_a}}
		\frac{n_a}{n_i}
	\right) n_i T_i
	-
	\frac{\sqrt g}{h_x} q_{ix}^{P.Sch}
...
\right\}
\end{matrix} \right] \right|_W ,
\end{matrix}
\end{equation*}
\end{small}

$u_ {a,x}^{(dia)} = V_{ax}^{(E)} + \widetilde V_{xa}^{(dia)}$

$u_{p_{out}} = \max \left\{
	0, \frac{u_{u_{out}}}{b_x} + \frac{u_{1,x}^{(dia)}}{b_x}
\right\}$

$u_{u_{out}} = p_{m,a_{main},1} c_s |b_x|$, $c_s = \sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_a}{\sum \limits_{a=0}^{ns-1} m_a n_a}}$, \\\\ $p_{m,a_{main},1} = 1.0$
...
{\em Imposing a {\tt b2stbc\_spb} style boundary condition:} {\tt BCENI = 15} (to be used in association with {\tt BCCON = 14})
...
$u_{a,x}^{(dia)} = V_{ax}^{(E)} + \widetilde V_{ax}^{(dia)}$
...
-\frac 4 3 b_x B^{\frac 3 2} \frac{\partial}{h_x \partial x} \left(
	\frac{
		c_{\eta_{ax}^{(CL)}} c_{\eta_{ax}^{(CL)}}^{(Luciani)} \eta^{(CL)}_{ax} b_x
	}{B^2}
	\frac{
		\partial \left(
			B^{\frac 1 2} \left(
...
-\frac 4 3 b_x B^{\frac 3 2} & \frac{\partial}{h_x \partial x} \left(
	\frac{c_{\eta_{ax}^{(CL)}} c_{\eta_{ax}^{(CL)}}^{(Luciani)} \eta^{(CL)}_{ax} b_x}{B^2}
...
\right)-
-\frac{\partial}{h_z \sqrt g \partial x} \left(
	\frac{h_z \sqrt g}{h_x} \frac 4 3 \eta _{ax} \frac{\partial \ln \left( h_z B^{\frac 1 2} \right)}
...
\frac{
	\sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \left| r_{n_a,i,j} \right|
}{
	\sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \sqrt{g_{i,j}} n_{a,i,j}
}
\end{equation}
...
\frac{
	\sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \left| r_{u_a,i,j} \right|
}{
	\sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \sqrt{g_{i,j}} n_{a,i,j} \sqrt{m_a T_i}
}
\end{equation}
...
\frac{
	\sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \left| r_{h_e,i,j} \right|
}{
	\left( \frac{3}{2} + BoRiS \right) \sum \limits_{a=0}^{ns-1} \sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \sqrt{g_{i,j}} n_{a,i,j}
...
\frac{
	\sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \left| r_{h_i,i,j} \right|
}{
	\left( \frac{3}{2} + BoRiS \right) \sum \limits_{a=0}^{ns-1} \sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \sqrt{g_{i,j}} n_{a,i,j}
	\left( z_a T_e + T_i \right)
}
...
\frac{
	\sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \left| r_{po,i,j} \right|
}{
	\sum \limits_{i=0}^{n_x-1} \sum \limits_{j=0}^{n_y-1} \sqrt{g_{i,j}} e n_{e,i,j}
}
\end{equation}
...
\begin{equation}\begin{split}
S_{hi3}=\sum_{\alpha}\Bigl[ ~ & C_{hi2\alpha}t_{\alpha}c_{s\alpha}S_{0}+C_{hi3\alpha}
t_{\alpha}c_{s\alpha}S_{1}+C_{hi4\alpha}t_{\alpha}c_{st}S_{0}+  \\
~ & C_{hi5\alpha}t_{\alpha}c_{st}S_{1}+C_{hi7\alpha}
\frac{\max\left(0,-\Gamma_{\alpha y}\right) }{n_{i}}\Bigr]   \label{33}
\end{split}\end{equation}
...
t_{\alpha}c_{s\alpha}S_{1}+C_{hi4\alpha}t_{\alpha}c_{st}S_{0}+ \\
~ & C_{hi5\alpha}t_{\alpha}c_{st}S_{1}+C_{hi7\alpha}
\frac{\max\left(0,\Gamma_{\alpha y}\right) }{n_{i}}\Bigr]   \label{56}
\end{split}\end{equation}
...
\begin{equation}\begin{split}
S_{hi3}=\sum_{\alpha}\Bigl[ ~ & C_{hi2\alpha}t_{\alpha}c_{sD\alpha}S_{0}+C_{hi3\alpha}
t_{\alpha}c_{sD\alpha}S_{1}+C_{hi4\alpha}t_{\alpha}c_{stD}S_{0}+ \\
~ & C_{hi5\alpha}t_{\alpha}c_{stD}S_{1}+C_{hi7\alpha}
\frac{\max\left(0,-\Gamma_{\alpha x}\right) }{n_{i}}\Bigr]   \label{79}
\end{split}\end{equation}
...
t_{\alpha}c_{sD\alpha}S_{1}+C_{hi4\alpha}t_{\alpha}c_{stD}S_{0}+ \\
~ & C_{hi5\alpha}t_{\alpha}c_{stD}S_{1}+C_{hi7\alpha}
\frac{\max\left(0,\Gamma_{\alpha x}\right) }{n_{i}}\Bigr]   \label{102}
\end{split}\end{equation}
...
(and $jxi$\index{jxi} for double-null runs) by assigning it an explicit value in the {\tt b2mn.dat} file.
...
fnixip \index{fnixip}\index{b2time.nc, fnixip} & integrated poloidal main ion particle flux, inboard divertor & $s^{-1}$ \\
...
fnixap \index{fnixap}\index{b2time.nc, fnixap} & integrated poloidal main ion particle flux, outboard divertor & $s^{-1}$ \\
...
dnsepm \index{dnsepm}\index{b2time.nc, dnsepm} & main ion density diffusion coefficient, outer midplane & $m^2.s^{-1}$ \\
dpsepm \index{dpsepm}\index{b2time.nc, dpsepm} & main neutral pressure diffusion coefficient, outer midplane & $m^2.s^{-1}$ \\
...
vxsepm \index{vxsepm}\index{b2time.nc, vxsepm} & main ion poloidal pinch velocity, outer midplane & $m.s^{-1}$ \\
vysepm \index{vysepm}\index{b2time.nc, vysepm} & main ion radial pinch velocity, outer midplane & $m.s^{-1}$ \\
vssepm \index{vssepm}\index{b2time.nc, vssepm} & main ion particle viscosity, outer midplane & $m^2.s^{-1}$ \\
...
fetyip \index{fetyip}\index{b2time.nc, fetyip} & integrated radial total energy flux, main chamber & $W $ \\
...
fetyap \index{fetyap}\index{b2time.nc, fetyap} & integrated radial total energy flux, divertor region & $W $ \\
...
fnisip \index{fnisip}\index{b2time.nc, fnisip} & poloidal main ion particle flux, into inboard separatrix throat & $s^{-1}$ \\
...
fnisap \index{fnisap}\index{b2time.nc, fnisap} & poloidal main ion particle flux, into outboard separatrix throat & $s^{-1}$ \\
...
{\em ix} at the inner target ({\tt l}), inner midplane ({\tt i}),
outer midplane ({\tt a}) and outer target ({\tt r}).  The
positions of the inner and outer midplanes can be selected by
{\tt b2mwti\_jxi} and {\tt b2mwti\_jxa}, respectively, in
{\tt b2mn.dat}.
...
\begin{center}
    ...
    \hline \endfoot
    \endlastfoot
fn3dl \index{fn3dl}\index{b2time.nc, fn3dl} & poloidal particle flux of main ion, inboard divertor & $s^{-1}.m^{-2}$ \\
fl3dl \index{fl3dl}\index{b2time.nc, fl3dl} & poloidal electron flux, inboard divertor & $s^{-1}.m^{-2}$ \\
fo3dl \index{fo3dl}\index{b2time.nc, fo3dl} & poloidal total ion flux, inboard divertor & $s^{-1}.m^{-2}$ \\
fe3dl \index{fe3dl}\index{b2time.nc, fe3dl} & poloidal electron energy flux, inboard divertor & $W.m^{-2}$ \\
fi3dl \index{fi3dl}\index{b2time.nc, fi3dl} & poloidal ion energy flux, inboard divertor & $W.m^{-2}$ \\
ft3dl \index{ft3dl}\index{b2time.nc, ft3dl} & poloidal total energy flux, inboard divertor & $W.m^{-2}$ \\
fc3dl \index{fc3dl}\index{b2time.nc, fc3dl} & poloidal current, inboard divertor  & $A.m^{-2}$ \\
...
an3dl \index{an3dl}\index{b2time.nc, an3dl} & atom density of main neutral, inboard divertor & $m^{-3}$ \\
mn3dl \index{mn3dl}\index{b2time.nc, mn3dl} & molecule density of $1^{st}$ molecule, inboard divertor & $m^{-3}$ \\
...
vs3di \index{vs3di}\index{b2time.nc, vs3di} & main ion viscosity, inboard midplane & $m^2.s^{-1}$ \\
vx3di \index{vx3di}\index{b2time.nc, vx3di} & main ion poloidal pinch velocity, inboard midplane & $m.s^{-1}$ \\
vy3di \index{vy3di}\index{b2time.nc, vy3di} & main ion radial pinch velocity, inboard midplane & $m.s^{-1}$ \\
...
vs3da \index{vs3da}\index{b2time.nc, vs3da} & main ion viscosity, outboard midplane & $m^2.s^{-1}$ \\
vx3da \index{vx3da}\index{b2time.nc, vx3da} & main ion poloidal pinch velocity, outboard midplane & $m.s^{-1}$ \\
vy3da \index{vy3da}\index{b2time.nc, vy3da} & main ion radial pinch velocity, outboard midplane & $m.s^{-1}$ \\
...
an3dr \index{an3dr}\index{b2time.nc, an3dr} & atom density of main neutral, outboard divertor & $m^{-3}$ \\
mn3dr \index{mn3dr}\index{b2time.nc, mn3dr} & molecule density of $1^{st}$ molecule, outboard divertor & $m^{-3}$ \\
fn3dr \index{fn3dr}\index{b2time.nc, fn3dr} & poloidal particle flux of main ion, outboard divertor & $s^{-1}.m^{-2}$ \\
fe3dr \index{fe3dr}\index{b2time.nc, fe3dr} & poloidal electron energy flux, outboard divertor & $W.m^{-2}$ \\
fi3dr \index{fi3dr}\index{b2time.nc, fi3dr} & poloidal ion energy flux, outboard divertor & $W.m^{-2}$ \\
ft3dr \index{ft3dr}\index{b2time.nc, ft3dr} & poloidal total energy flux, outboard divertor & $W.m^{-2}$ \\
fc3dr \index{fc3dr}\index{b2time.nc, fc3dr} & poloidal current, outboard divertor & $A.m^{-2}$ \\
fl3dr \index{fl3dr}\index{b2time.nc, fl3dr} & poloidal electron flux, outboard divertor & $s^{-1}.m^{-2}$ \\
fo3dr \index{fo3dr}\index{b2time.nc, fo3dr} & poloidal total ion flux, outboard divertor & $s^{-1}.m^{-2}$ \\
    \hline
...
fn3dtl \index{fn3dtl}\index{b2time.nc, fn3dtl} & poloidal particle flux of main ion, inboard top divertor & $s^{-1}$ \\
...
fn3dtr \index{fn3dtr}\index{b2time.nc, fn3dtr} & poloidal particle flux of main ion, outboard top divertor & $s^{-1}$ \\
...
Additional multi-dimensional writes to {\tt b2time.nc} can be made using the ``b2mwti\_2dwrite'' switch.
If set to ``1'' then a few 2D arrays ($n_e$, $T_e$, $T_i$) are output with each write to {\tt b2time.nc}. If set
to ``2'' then additional quantities such as fluxes are added, see below Table
\ref{tab:5db2time}.
...
\begin{center}
    ...
    \hline \endfoot
    \endlastfoot
    \multicolumn{3}{|c|}{If b2mwti\_2dwrite $\ge$ 1} \\
    \hline
    ...
    \hline
    \multicolumn{3}{|c|}{and if b2mwti\_2dwrite = 2} \\
    \hline
    ...
    rsahi2d \index{rsahi2d}\index{b2time.nc, rsahi2d} & ionization energy source/sink & $W$ \\
    rsana2d \index{rsana2d}\index{b2time.nc, rsana2d} & ionization rate & $s^{-1}$ \\
...
nesepi\_av \index{nesepi\_av}\index{b2time.nc, nesepi\_av} & averaged separatrix electron density, inboard divertor & $m^{-3}$ \\
tesepi\_av \index{tesepi\_av}\index{b2time.nc, tesepi\_av} & averaged separatrix electron temperature, inboard divertor & $eV$ \\
tisepi\_av \index{tisepi\_av}\index{b2time.nc, tisepi\_av} & averaged separatrix ion temperature, inboard divertor & $eV$ \\
posepi\_av \index{posepi\_av}\index{b2time.nc, posepi\_av} & averaged separatrix potential, inboard divertor & $V $ \\
...
nesepa\_av \index{nesepa\_av}\index{b2time.nc, nesepa\_av} & averaged separatrix electron density, outboard divertor & $m^{-3}$ \\
tesepa\_av \index{tesepa\_av}\index{b2time.nc, tesepa\_av} & averaged separatrix electron temperature, outboard divertor & $eV$ \\
tisepa\_av \index{tisepa\_av}\index{b2time.nc, tisepa\_av} & averaged separatrix ion temperature, outboard divertor & $eV$ \\
posepa\_av \index{posepa\_av}\index{b2time.nc, posepa\_av} & averaged separatrix potential, outboard divertor max & $V $ \\
nemxip\_av \index{nemxip\_av}\index{b2time.nc, nemxip\_av} & averaged maximum electron density, inboard divertor & $m^{-3}$ \\
temxip\_av \index{temxip\_av}\index{b2time.nc, temxip\_av} & averaged maximum electron temperature, inboard divertor & $eV$ \\
timxip\_av \index{timxip\_av}\index{b2time.nc, timxip\_av} & averaged maximum ion temperature, inboard divertor & $eV$ \\
pomxip\_av \index{pomxip\_av}\index{b2time.nc, pomxip\_av} & averaged maximum potential, inboard divertor & $V $ \\
nemxap\_av \index{nemxap\_av}\index{b2time.nc, nemxap\_av} & averaged maximum electron density, outboard divertor & $m^{-3}$ \\
temxap\_av \index{temxap\_av}\index{b2time.nc, temxap\_av} & averaged maximum electron temperature, outboard divertor & $eV$ \\
timxap\_av \index{timxap\_av}\index{b2time.nc, timxap\_av} & averaged maximum ion temperature, outboard divertor  & $eV$ \\
pomxap\_av \index{pomxap\_av}\index{b2time.nc, pomxap\_av} & averaged maximum potential, outboard divertor & $V $ \\
nesepi\_std \index{nesepi\_std}\index{b2time.nc, nesepi\_std} & averaged separatrix electron density, inboard divertor & $m^{-3}$ \\
tesepi\_std \index{tesepi\_std}\index{b2time.nc, tesepi\_std} & averaged separatrix electron temperature, inboard divertor & $eV$ \\
tisepi\_std \index{tisepi\_std}\index{b2time.nc, tisepi\_std} & averaged separatrix ion temperature, inboard divertor & $eV$ \\
posepi\_std \index{posepi\_std}\index{b2time.nc, posepi\_std} & averaged separatrix potential, inboard divertor & $V $ \\
...
nesepa\_std \index{nesepa\_std}\index{b2time.nc, nesepa\_std} & variance for separatrix electron density, outboard divertor & $m^{-3}$ \\
tesepa\_std \index{tesepa\_std}\index{b2time.nc, tesepa\_std} & variance for separatrix electron temperature, outboard divertor & $eV$ \\
tisepa\_std \index{tisepa\_std}\index{b2time.nc, tisepa\_std} & variance for separatrix ion temperature, outboard divertor & $eV$ \\
posepa\_std \index{posepa\_std}\index{b2time.nc, posepa\_std} & variance for separatrix potential, outboard divertor max & $V $ \\
nemxip\_std \index{nemxip\_std}\index{b2time.nc, nemxip\_std} & variance for maximum electron density, inboard divertor & $m^{-3}$ \\
temxip\_std \index{temxip\_std}\index{b2time.nc, temxip\_std} & variance for maximum electron temperature, inboard divertor & $eV$ \\
timxip\_std \index{timxip\_std}\index{b2time.nc, timxip\_std} & variance for maximum ion temperature, inboard divertor & $eV$ \\
pomxip\_std \index{pomxip\_std}\index{b2time.nc, pomxip\_std} & variance for maximum potential, inboard divertor & $V $ \\
nemxap\_std \index{nemxap\_std}\index{b2time.nc, nemxap\_std} & variance for maximum electron density, outboard divertor & $m^{-3}$ \\
temxap\_std \index{temxap\_std}\index{b2time.nc, temxap\_std} & variance for maximum electron temperature, outboard divertor & $eV$ \\
timxap\_std \index{timxap\_std}\index{b2time.nc, timxap\_std} & variance for maximum ion temperature, outboard divertor  & $eV$ \\
pomxap\_std \index{pomxap\_std}\index{b2time.nc, pomxap\_std} & variance for maximum potential, outboard divertor & $V $ \\
    \hline
...
rdneureg \index{rdenureg}\index{b2tallies.nc, rdneureg}
 & vreg, time     & $W$           & Total radiation from {\tt Eirene} neutrals \\
...
rdneureg \index{rdneureg}\index{b2tallies.nc, rdneureg}                          & vreg, time     & $W$           & Radiation rate from {\tt Eirene} neutrals \\
...
Three types of regions are defined in {\tt b2agfs.F}: volume,
$x$-directed and $y$-directed.  The number of volume regions should also
be stored in the {\tt NREG} environment variable (or through the presence
of an {\tt NREG} ASCII file containing only the number of regions) as it is
...
For this geometry, {\tt NREG} should be set to $1$.
...
In cases where the problem of interest has only one target and a plasma entry surface, the target should be located to the right of the domain and the plasma should enter from the left.

\clearpage
\section{Regions for limiter cases}
\index{regions, limiter cases}

For this geometry, {\tt NREG} should be set to $2$.
...
For this geometry, {\tt NREG} should be set to $4$ (default).
...
For this geometry, {\tt NREG} should be set to $8$.
...
For both cases, {\tt NREG} should be set to $8$.
...
For this geometry, {\tt NREG} should be set to $5$.
...
(for \SOLPSfive and \SOLPSITER runs) and {\tt fort.44} (when running
with {\tt Eirene}) files are read in, and then
...
\begin{center}\index{b2plot, plasma quantities}
  \begin{longtable}{|l|l|}
    ...
    brch     & \index{b2plot, brch} Recycling charge source {\em b2stbr\_sch} $(A)$ (including any {\tt Eirene} contributions) \\
    brhe     & \index{b2plot, brhe} Recycling electron heat source {\em b2stbr\_she} $(W)$ (including any {\tt Eirene} contributions) \\
    brhi     & \index{b2plot, brhi} Recycling ion heat source {\em b2stbr\_shi} $(W)$ (including any {\tt Eirene} contributions) \\
    brmo     & \index{b2plot, brmo} Recycling parallel momentum source {\em b2stbr\_smo} $(N)$ (including any {\tt Eirene} contributions) \\
    brna     & \index{b2plot, brna} Recycling particle source {\em b2stbr\_sna} $(particles.s^{-1})$ (including any {\tt Eirene} contributions) \\
    ...
    rsam     & \index{b2plot, rsam} Ionization ion momentum losses ({\em rsamo}) $(N)$ \\
    ...
    veexb.x  & \index{b2plot, veexb.x} Poloidal component of electron ${\bf E} \times {\bf B}$ drift velocity $(m.s^{-1})$ \\
    veexb.y  & \index{b2plot, veexb.y} Radial component of electron ${\bf E} \times {\bf B}$ drift velocity $(m.s^{-1})$ \\
    vexb.x   & \index{b2plot, vexb.x} Poloidal component of ion ${\bf E} \times {\bf B}$ drift velocity $(m.s^{-1})$ \\
    vexb.y   & \index{b2plot, vexb.y} Radial component of ion ${\bf E} \times {\bf B}$ drift velocity $(m.s^{-1})$ \\
...
argument $\langle$ireg$\rangle$. If $\langle$ireg$\rangle$=0, the arrays will contain the region
number of each cell. If $\langle$ireg$\rangle$ is greater than 0, the cells in region with index $\langle$ireg$\rangle$ will contain 1.0, and the rest will contain zero. If $\langle$ireg$\rangle$ is less than 0, the cells in region with index --$\langle$ireg$\rangle$ will contain zero, and the rest will contain 1.0.

The source arrays {\bf sna}, {\bf smo}, {\bf smq}, {\bf she}, {\bf shi}, and
{\bf sch} can take an optional integer argument $\langle$isel$\rangle$, varying from 0 to 3 (0 to 1 for {\bf sna}). Then the data
...
also all require an integer argument $\langle$i$\rangle$,
...
\begin{center}\index{b2plot, stack commands}
  \begin{longtable}{|c|c|c|c|c|c|c|c|c|l|}
    ...
    \hline \endfirsthead
    \hline
    \multicolumn{10}{|l|}{\small\sl continued on next page} \\
    \hline \endfoot
    \endlastfoot
    ...
    intx \index{b2plot, intx}      & 0 & 0 & 0 & 1 & 0 & 0 & 0 & \ref{sec:intx}  & Integrate along $x$ \\
    ...
    mdims \index{b2plot, mdims}    & 0 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:mdims} & Prints the dimensions and contents of the matrix stack \\
    ...
    over \index{b2plot, over}      & 0 & 0 & 0 & 2 & 0 & 0 & 0 & \ref{sec:over}  & Copy over second top element of matrix stack \\
    ...
    psxperp \index{b2plot, psxperp} & 0 & 0 & 0 & 1 & 0 & 0 & 0 & \ref{sec:psxperp}   & Divide by area perpendicular to flux tube\\
    ...
    rigz \index{b2plot, rigz}      & 0 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:rigz}  & Toggles use of ignorable cells (\SOLPSfour runs) \\
    ...
    stoe \index{b2plot, stoe}      & 0 & 0 & 0 & 1 & 0 & 0 & 0 & \ref{sec:stoe}  & Copy top of matrix stack to Eirene stack \\
    ...
    tese \index{b2plot, tese}      & 0 & 0 & 0 & 1 & 0 & 0 & 0 & \ref{sec:tese}  & Copy top of stack onto electron temperature \\
    traj \index{b2plot, traj}      & 7+* & 0 & 0 & 1 & 0 & 0 & 0 & \ref{sec:traj}  & Trajectories \\
    ...
    wtos \index{b2plot, wtos}      & 0 & 0 & 0 & 0 & 0 & 1 & 0 & \ref{sec:wtos}  & Copy wall stack array onto matrix stack \\
    ...
    ytow \index{b2plot, ytow}      & 1 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:ytow}  & Copy $Y$-column of matrix onto wall stack \\
...
\begin{center}\index{b2plot, plot commands}
  \begin{longtable}{|c|c|c|c|c|c|c|c|c|l|}
    ...
    expl \index{b2plot, expl}      & 0 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:expl}   & Toggle line or scatter experimental data plot \\
    ...
    idup \index{b2plot, idup}      & 1 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:idup}   & Duplicate top element from the integer stack \\
    imp \index{b2plot, imp}        & 1 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:imp}    & Set $jxi$ (inner midplane coordinate) \\
    iover \index{b2plot, iover}    & 2 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:iover}  & Copy over second element from the integer stack to top \\
    iprt \index{b2plot, iprt}      & 1 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:iprt}   & Outputs the integer stack \\
    iswap \index{b2plot, iswap}    & 2 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:iswap}  & Swap top two elements from the integer stack \\
    ...
    omp \index{b2plot, omp}        & 1 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:omp}    & Set $jxa$ (outer midplane coordinate) \\
    ...
    rover \index{b2plot, rover}    & 0 & 2 & 0 & 0 & 0 & 0 & 0 & \ref{sec:rover}  & Copy over second element from the real stack to top \\
    rprt \index{b2plot, rprt}      & 0 & 1 & 0 & 0 & 0 & 0 & 0 & \ref{sec:rprt}   & Outputs the real stack \\
    rswap \index{b2plot, rswap}    & 0 & 2 & 0 & 0 & 0 & 0 & 0 & \ref{sec:rswap}  & Swap top two elements from the real stack \\
    ...
    xpar \index{b2plot, xpar}      & 0 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:xpar}   & Set $X$ coordinate to parallel length of fieldline \\
    ...
    ytra \index{b2plot, ytra}      & 1 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:ytra}   & Transform $Y$ coordinate for comparison to experiment \\
...
\begin{center}\index{b2plot, character commands}
  \begin{longtable}{|c|c|c|c|c|c|c|c|c|l|}
    ...
    chor \index{b2plot, chor}    & 0 & 0 & 1 & 0 & 0 & 0 & 0 & \ref{sec:chor}     & Filename containing chords (for integration) \\
    ...
    echbx \index{b2plot, echbx}  & 0 & 0 & 1 & 0 & 1 & 0 & 0 & \ref{sec:echbx}    & Boxcar average of chords for Eirene stack data \\
    echmp \index{b2plot, echmp}  & 0 & 0 & 1 & 0 & 1 & 0 & 0 & \ref{sec:echmp}    & Chord contributions to the mesh element from Eirene stack data \\
    echor \index{b2plot, echor}  & 0 & 0 & 1 & 0 & 1 & 0 & 0 & \ref{sec:echor}    & Filename containing chords (for integration of Eirene stack data) \\
    ...
    mtab \index{b2plot, mtab}    & 4 & 0 & 1 & 1 & 0 & 0 & 0 & \ref{sec:mtab}     & Output part of the matrix on top of the stack to a file \\
    ...
    outp \index{b2plot, outp}    & 2 & 0 & 1 & 1 & 0 & 0 & 0 & \ref{sec:outp}     & Prints value of specific element \\
    pdev \index{b2plot, pdev}    & 0 & 0 & 1 & 0 & 0 & 0 & 0 & \ref{sec:pdev}     & {\tt PLPLOT} device name \\
    saw \index{b2plot, saw}      & 2 & 0 & 1 & 0 & 0 & 0 & 0 & \ref{sec:saw}      & Read external data onto the matrix stack \\
    saws \index{b2plot, saws}    & 3 & 0 & 1 & 0 & 0 & 0 & 0 & \ref{sec:saws}     & Read external fluid species data onto the matrix stack \\
    sawx \index{b2plot, sawx}    & 3 & 0 & 1 & 0 & 0 & 0 & 0 & \ref{sec:sawx}     & Read external additional species data onto the matrix stack \\
    ...
    sover \index{b2plot, sover}  & 0 & 0 & 2 & 0 & 0 & 0 & 0 & \ref{sec:sover}    & Copy over the second element of the string stack \\
    sprt \index{b2plot, sprt}    & 0 & 0 & 1 & 0 & 0 & 0 & 0 & \ref{sec:sprt}     & Outputs the string stack \\
    sswap \index{b2plot, sswap}  & 0 & 0 & 2 & 0 & 0 & 0 & 0 & \ref{sec:sswap}    & Swap the top two elements of the string stack \\
    ...
    trgtab \index{b2plot, trgtab} & 4 & 0 & 1 & 0 & 0 & 0 & 1 & \ref{sec:trgtab}     & Output part of the top of the target stack to a file \\
    ...
    wtab \index{b2plot, wtab}    & 2 & 0 & 1 & 0 & 0 & 1 & 0 & \ref{sec:wtab}     & Output part of the top of the wall stack to a file \\
    \hline
...
\begin{center}\index{b2plot, Eirene quantities}
  \begin{longtable}{|l|l|}
    ...
    vxa \index{b2plot, vxa} & Atoms $x$-directed velocity $(m.s^{-1})$ \\
    vxdena \index{b2plot, vxdena} & Atoms $x$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vxdeni \index{b2plot, vxdeni} & Test ions $x$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vxdenm \index{b2plot, vxdenm} & Molecules $x$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vxi \index{b2plot, vxi} & Test ions $x$-directed velocity $(m.s^{-1})$ \\
    vxm \index{b2plot, vxm} & Molecules $x$-directed velocity $(m.s^{-1})$ \\
    vya \index{b2plot, vya} & Atoms $y$-directed velocity $(m.s^{-1})$ \\
    vydena \index{b2plot, vydena} & Atoms $y$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vydeni \index{b2plot, vydeni} & Test ions $y$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vydenm \index{b2plot, vydenm} & Molecules $y$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vyi \index{b2plot, vyi} & Test ions $y$-directed velocity $(m.s^{-1})$ \\
    vym \index{b2plot, vym} & Molecules $y$-directed velocity $(m.s^{-1})$ \\
    vza \index{b2plot, vza} & Atoms $z$-directed velocity $(m.s^{-1})$ \\
    vzdena \index{b2plot, vzdena} & Atoms $z$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vzdeni \index{b2plot, vzdeni} & Test ions $z$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vzdenm \index{b2plot, vzdenm} & Molecules $z$-directed momentum density $(kg.m^{-2}.s^{-1})$ \\
    vzi \index{b2plot, vzi} & Test ions $z$-directed velocity $(m.s^{-1})$ \\
    vzm \index{b2plot, vzm} & Molecules $z$-directed velocity $(m.s^{-1})$ \\
    \hline
...
\begin{center}\index{b2plot, Eirene stack commands}
  \begin{longtable}{|c|c|c|c|c|c|c|c|c|l|}
    ...
    eavg \index{b2plot, eavg}    & 0 & 8 & 0 & 0 & 1 & 0 & 0 & \ref{sec:eavg}  & Perform a volume average of Eirene data over a rectangular region \\
    ...
    edims \index{b2plot, edims}    & 0 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:edims}    & Prints the dimensions and contents of the Eirene stack \\
    ...
    emge \index{b2plot, emge}      & 0 & 1 & 0 & 0 & 1 & 0 & 0 & \ref{sec:emge}     & Eirene matrix element is greater than or equal to \\
    ...
    emmax \index{b2plot, emmax}    & 0 & 0 & 0 & 0 & 2 & 0 & 0 & \ref{sec:emmax}    & Maximum of top two elements on the Eirene stack \\
    emmin \index{b2plot, emmin}    & 0 & 0 & 0 & 0 & 2 & 0 & 0 & \ref{sec:emmin}    & Minimum of top two elements on the Eirene stack \\
    ...
    eover \index{b2plot, eover}    & 0 & 0 & 0 & 0 & 2 & 0 & 0 & \ref{sec:eover}    & Copy over the second element of the Eirene stack \\
...
\begin{center}\index{b2plot, wall quantities}
  \begin{longtable}{|l|l|}
    ...
    c\_cp\_b3 \index{b2plot, c\_cp\_b3}  & Coating heat capacity $b_3$ factor (either $(J.kg^{-1}.K)$ [old format] or $(J.kg^{-1}.K^{-3})$ [new format]) \\
...
argument $\langle$iseg$\rangle$. If $\langle$iseg$\rangle$=0, the arrays will contain the wall segment
number of each element. If $\langle$iseg$\rangle$ is larger than 0, the elements in segment
with index $\langle$iseg$\rangle$ will contain 1.0, and the rest will contain zero.
If $\langle$iseg$\rangle$ is less than 0, the elements in segment
with index --$\langle$iseg$\rangle$ will contain zero, and the rest will contain 1.0.

The arrays {\bf iwlt}, {\bf iwle}, {\bf iweq}, {\bf iwne}, {\bf iwge}, {\bf iwgt},
{\bf iswlt}, {\bf iswle}, {\bf isweq}, {\bf iswne}, {\bf iswge}, and {\bf iswgt}
all require an integer argument $\langle$i$\rangle$,
with which the logical comparison is done. If the comparison test is true, the arrays
will contain 1.0, otherwise they will contain zero.
...
\begin{center}\index{b2plot, wall commands}
  \begin{longtable}{|c|c|c|c|c|c|c|c|c|l|}
    ...
    wmge \index{b2plot, wmge}     & 0 & 1 & 0 & 0 & 0 & 1 & 0 & \ref{sec:wmge}     & Wall matrix element is greater than or equal to \\
    ...
    wmmax \index{b2plot, wmmax}   & 0 & 0 & 0 & 0 & 0 & 2 & 0 & \ref{sec:wmmax}    & Maximum of top two elements on the wall stack \\
    wmmin \index{b2plot, wmmin}   & 0 & 0 & 0 & 0 & 0 & 2 & 0 & \ref{sec:wmmin}    & Minimum of top two elements on the wall stack \\
    ...
    wover \index{b2plot, wover}   & 0 & 0 & 0 & 0 & 0 & 2 & 0 & \ref{sec:wover}    & Copy over the second element of the wall stack \\
...
\begin{center}\index{b2plot, target quantities}
  \begin{longtable}{|c|l|}
    ...
    trgl \index{b2plot, trgl}      & ANK target loading (obsolete, use {\em 100 wlld} instead) \\
...
The {\bf trgseg} command requires an integer argument $\langle$iseg$\rangle$.
If $\langle$iseg$\rangle$=0, the arrays will contain the wall segment
number of each element. If $\langle$iseg$\rangle$ is larger than 0,
the elements in segment with index $\langle$iseg$\rangle$
will contain 1.0, and the rest will contain zero.
If $\langle$iseg$\rangle$ is less than 0,
the elements in segment with index --$\langle$iseg$\rangle$
...
\begin{center}\index{b2plot, target commands}
  \begin{longtable}{|c|c|c|c|c|c|c|c|c|l|}
    ...
    rtrgm/ \index{b2plot, rtrgm/}   & 0 & 1 & 0 & 0 & 0 & 0 & 1 & \ref{sec:rtrgm/}   & Divide the target data by a real \\
    rtrgmma \index{b2plot, rtrgmma}  & 0 & 1 & 0 & 0 & 0 & 0 & 1 & \ref{sec:rtrgmma}  & Take the maximum of the target data and a real \\
    rtrgmmi \index{b2plot, rtrgmmi}  & 0 & 1 & 0 & 0 & 0 & 0 & 1 & \ref{sec:rtrgmmi}  & Take the minimum of the target data and a real \\
    ...
    trgddp \index{b2plot, trgddp}   & 0 & 0 & 0 & 0 & 0 & 0 & 1 & \ref{sec:trgddp}   & Take derivative of target data along perpendicular direction \\
    trgddw \index{b2plot, trgddw}   & 0 & 0 & 0 & 0 & 0 & 0 & 1 & \ref{sec:trgddw}   & Take derivative of target data along wall coordinate \\
    trgddz \index{b2plot, trgddz}   & 0 & 0 & 0 & 0 & 0 & 0 & 1 & \ref{sec:trgddz}   & Take derivative of target data along target depth \\
    trgdrop \index{b2plot, trgdrop}  & 0 & 0 & 0 & 0 & 0 & 0 & 1 & \ref{sec:trgdrop}  & Drop the top element from the target stack \\
    trgdup \index{b2plot, trgdup}   & 0 & 0 & 0 & 0 & 0 & 0 & 1 & \ref{sec:trgdup}   & Duplicate the top element from the target stack \\
    ...
    trgover \index{b2plot, trgover}  & 0 & 0 & 0 & 0 & 0 & 0 & 2 & \ref{sec:trgover}  & Copy over second element on top of target stack \\
...
\begin{center}\index{b2plot, constants}
  \begin{longtable}{|c|l|}
    ...
    jsep \index{b2plot, jsep}  & Radial index of outermost closed flux surface (just inside the separatrix) \\
...
The {\bf coro} command, with integer argument $\langle$is$\rangle$ and real argument $\langle$T$\rangle$
prints to {\tt stdout} the coronal radiation
rates for the homonuclear sequence of species $\langle$is$\rangle$ at temperature $\langle$T$\rangle$ (in $eV$).

The {\bf debu} command, with integer argument $\langle$level$\rangle$,
sets up the level of debugging information to $\langle$level$\rangle$ (default 0).
The higher the value of $\langle$level$\rangle$, the more output is produced.
{\bf debug} is an alias for {\bf debu}.

The {\bf wlld} command requires an integer argument $\langle$ig$\rangle$. Let
$\langle$ig$\rangle$ = JKLM. If M \textgreater 0, then the file {\tt radsrc.b2e} is produced,
containing the radiation sources (in $MW/m^3$). If L \textgreater 0, then the
file {\em wlld.dat} is produced, which contains the power loading
from the plasma onto the Eirene walls (this description does not include
splitting of the targets by the grid and is obsolete). If K \textgreater 0, then a
...
plot zone. If LM = 99 and J \textgreater 0, additional diagnostics are produced that indicate the radiation split by isonuclear sequence. If LM = 98 and J \textgreater 0, additional diagnostics are produced that indicate the radiation coming from the core additional sources. \\
If the user wants to use the heat transfer model for the monoblock and/or the reflector plates, then he must define in the DG model the inner/outer target, the reflector plates and dome plot zones using the label 'trg[i-o]', '[i-o]rp' and 'dome' respectively. The {\tt wlld\_*.dat} produced should then be postprocessed by b2ymb (see \ref{sec:b2ymb}), b2yrp (\ref{sec:b2yrp}) and b2ydm (\ref{sec:b2ydm}).
...
Plots the 2-D graph of the matrix data at $iy$ = $\langle$pos$\rangle$ versus $x$.
...
Plots the 2-D graph of the matrix data at $ix$ = $\langle$pos$\rangle$ versus $y$.
...
Plots the 2-D graph of the matrix data at $iy$ = $\langle$pos$\rangle$ versus $\log(x)$.
...
Plots the 2-D graph of the matrix data at $ix$ = $\langle$pos$\rangle$ versus $\log(y)$.
...
Plots the 2-D graph of target data at $iz$ = $\langle$pos$\rangle$ versus wall coordinate.
Vertical lines will be added to the plot to delimit
the various wall segments.

\subsubsection{$<$pos,{\bf I}$>$ trg.z}\index{b2plot, trg.z}\label{sec:trg.z}
Plots the 2-D graph of target data at element $i$ = $\langle$pos$\rangle$ versus $z$.

\subsubsection{$<$n1,{\bf I}$>$ line}\index{b2plot, line}\label{sec:line}
Chooses between plotting against $x$-coordinate (default, $\langle$n1$\rangle$=1)
or $y$-coordinate ($\langle$n1$\rangle$=2). Not sure this works.
...
Sums the matrix data over the $z$-direction from $\langle$is\textsubscript{start}$\rangle$ to $\langle$is\textsubscript{end}$\rangle$, and then copies the
sum to the first $z$ position. If $\langle$is\textsubscript{start}$\rangle$=$\langle$is\textsubscript{end}$\rangle$=0, then the sum is done over all species.

\subsubsection{$<$is\textsubscript{start},{\bf I}$>$ $<$is\textsubscript{end},{\bf I}$>$ esumz}\index{b2plot, esumz}\label{sec:esumz}
Sums the Eirene data over the $z$-direction from $\langle$is\textsubscript{start}$\rangle$ to $\langle$is\textsubscript{end}$\rangle$, and then copies the
sum to the first $z$ position. If $\langle$is\textsubscript{start}$\rangle$=$\langle$is\textsubscript{end}$\rangle$=0, then the sum is done over all species.

\subsubsection{$<$is\textsubscript{start},{\bf I}$>$ $<$is\textsubscript{end},{\bf I}$>$ wsumz}\index{b2plot, wsumz}\label{sec:wsumz}
Sums the wall data over the $z$-direction from $\langle$is\textsubscript{start}$\rangle$ to $\langle$is\textsubscript{end}$\rangle$, and then copies the
sum to the first $z$ position. If $\langle$is\textsubscript{start}$\rangle$=$\langle$is\textsubscript{end}$\rangle$=0, then the sum is done over all species.

\subsubsection{$<$is\textsubscript{start},{\bf I}$>$ $<$is\textsubscript{end},{\bf I}$>$ zrng}\index{b2plot, zrng}\label{sec:zrng}
Selects the subset over species extended from species index $\langle$is\textsubscript{start}$\rangle$ to
species index $\langle$is\textsubscript{end}$\rangle$ from the matrix data.

\subsubsection{$<$is\textsubscript{start},{\bf I}$>$ $<$is\textsubscript{end},{\bf I}$>$ ezrng}\index{b2plot, ezrng}\label{sec:ezrng}
Selects the subset over species extended from species index $\langle$is\textsubscript{start}$\rangle$ to
species index $\langle$is\textsubscript{end}$\rangle$ from the Eirene data.

\subsubsection{$<$is\textsubscript{start},{\bf I}$>$ $<$is\textsubscript{end},{\bf I}$>$ wzrng}\index{b2plot, wzrng}\label{sec:wzrng}
Selects the subset over species extended from species index $\langle$is\textsubscript{start}$\rangle$ to
species index $\langle$is\textsubscript{end}$\rangle$ from the wall data.

\subsubsection{$<$i,{\bf I}$>$ esel}\index{b2plot, esel}\label{sec:esel}
Propagates the Eirene data value from element $\langle$i$\rangle$ to all other elements.

\subsubsection{$<$i,{\bf I}$>$ wsel}\index{b2plot, wsel}\label{sec:wsel}
Propagates the wall data value from element $\langle$i$\rangle$ to all other elements.

\subsubsection{$<$ix,{\bf I}$>$ xsel}\index{b2plot, xsel}\label{sec:xsel}
Propagates the matrix data value in the specified position $\langle$ix$\rangle$ to all other $x$ values.

\subsubsection{$<$iy,{\bf I}$>$ ysel}\index{b2plot, ysel}\label{sec:ysel}
Propagates the matrix data value in the specified position $\langle$iy$\rangle$ to all other $y$ values.

\subsubsection{$<$is,{\bf I}$>$ zsel}\index{b2plot, zsel}\label{sec:zsel}
Propagates the matrix data value in the specified position $\langle$is$\rangle$ to all other $z$
(species) values.

\subsubsection{$<$i,{\bf I}$>$ trgwsel}\index{b2plot, trgwsel}\label{sec:trgwsel}
Propagates the target data value from the specified element $\langle$i$\rangle$ to all other elements.

\subsubsection{$<$is,{\bf I}$>$ ezsel}\index{b2plot, ezsel}\label{sec:ezsel}
Propagates the Eirene data value in the specified position $\langle$is$\rangle$ to all other $z$
(species) values.

\subsubsection{$<$iz,{\bf I}$>$ trgzsel}\index{b2plot, trgzsel}\label{sec:trgzsel}
Propagates the target data value at the specified depth $\langle$iz$\rangle$ to all other $z$ values.

\subsubsection{$<$is,{\bf I}$>$ wzsel}\index{b2plot, wzsel}\label{sec:wzsel}
Propagates the wall data value in the specified position $\langle$is$\rangle$ to all other $z$
(species) values.
...
Adds the two real values $\langle$r1$\rangle$ and $\langle$r2$\rangle$ on the top of the real stack.

\subsubsection{$<$r1,{\bf R}$>$ $<$r2,{\bf R}$>$ r-}\index{b2plot, r-}\label{sec:r-}
Subtracts the real value on the top of the real stack $\langle$r2$\rangle$ from the real value
second from the top $\langle$r1$\rangle$.

\subsubsection{$<$r1,{\bf R}$>$ $<$r2,{\bf R}$>$ r*}\index{b2plot, r*}\label{sec:r*}
Multiplies the two real values $\langle$r1$\rangle$ and $\langle$r2$\rangle$ on the top of the real stack.

\subsubsection{$<$r1,{\bf R}$>$ $<$r2,{\bf R}$>$ r/}\index{b2plot, r/}\label{sec:r/}
Divides the real value second from the top of the real stack $\langle$r1$\rangle$ by the real value on the top of the real stack $\langle$r2$\rangle$. Division by 0 will yield an error and do nothing.

\subsubsection{$<$r1,{\bf R}$>$ $<$r2,{\bf R}$>$ r**}\index{b2plot, r**}\label{sec:r**}
Raises the real value second from the top of the real stack $\langle$r1$\rangle$ to the power of
the real value on the top $\langle$r2$\rangle$. $0^0$ will yield an error and do nothing.

\subsubsection{$<$number,{\bf R}$>$ rm+}\index{b2plot, rm+}\label{sec:rm+}
Adds $\langle$number$\rangle$ to the array on the top of the matrix stack.

\subsubsection{$<$number,{\bf R}$>$ rm-}\index{b2plot, rm-}\label{sec:rm-}
Substracts $\langle$number$\rangle$ from the array on the top of the matrix stack.

\subsubsection{$<$number,{\bf R}$>$ rm*}\index{b2plot, rm*}\label{sec:rm*}
Multiplies the array on the top of the matrix stack by $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rm/}\index{b2plot, rm/}\label{sec:rm/}
Divides the array on the top of the matrix stack by $\langle$number$\rangle$. Division by
zero will yield an error and do nothing.

\subsubsection{$<$number,{\bf R}$>$ rm**}\index{b2plot, rm**}\label{sec:rm**}
Raises each element of the array on the top of the matrix stack to the $\langle$number$\rangle$-th power. If $\langle$number$\rangle$=0, the result will be 1.0 for all cells.

\subsubsection{$<$number,{\bf R}$>$ rmmi}\index{b2plot, rmmi}\label{sec:rmmi}
Minimum of a matrix and $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rmma}\index{b2plot, rmma}\label{sec:rmma}
Maximum of a matrix and $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rem+}\index{b2plot, rem+}\label{sec:rem+}
Adds $\langle$number$\rangle$ to the array on the top of the Eirene stack.

\subsubsection{$<$number,{\bf R}$$>$$ rem-}\index{b2plot, rem-}\label{sec:rem-}
Substracts $\langle$number$\rangle$ from the array on the top of the Eirene stack.

\subsubsection{$<$number,{\bf R}$>$ rem*}\index{b2plot, rem*}\label{sec:rem*}
Multiplies the array on the top of the Eirene stack by $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rem/}\index{b2plot, rem/}\label{sec:rem/}
Divides the array on the top of the Eirene stack by $\langle$number$\rangle$. Division by
zero will yield an error and do nothing.

\subsubsection{$<$number,{\bf R}$>$ rem**}\index{b2plot, rem**}\label{sec:rem**}
Raises each element of the array on the top of the Eirene stack to the $\langle$number$\rangle$-th power. If $\langle$number$\rangle$=0, the result will be 1.0 for all cells.

\subsubsection{$<$number,{\bf R}$>$ remmi}\index{b2plot, remmi}\label{sec:remmi}
Minimum of an Eirene array and $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ remma}\index{b2plot, remma}\label{sec:remma}
Maximum of an Eirene array and $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rtrgm+}\index{b2plot, rtrgm+}\label{sec:rtrgm+}
Adds $\langle$number$\rangle$ to the array on the top of the target stack.

\subsubsection{$<$number,{\bf R}$>$ rtrgm-}\index{b2plot, rtrgm-}\label{sec:rtrgm-}
Substracts $\langle$number$\rangle$ from the array on the top of the target stack.

\subsubsection{$<$number,{\bf R}$>$ rtrgm*}\index{b2plot, rtrgm*}\label{sec:rtrgm*}
Multiplies the array on the top of the target stack by $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rtrgm/}\index{b2plot, rtrgm/}\label{sec:rtrgm/}
Divides the array on the top of the target stack by $\langle$number$\rangle$. Division by
zero will yield an error and do nothing.

\subsubsection{$<$number,{\bf R}$>$ rtrgm**}\index{b2plot, rtrgm**}\label{sec:rtrgm**}
Raises each element of the array on the top of the target stack to the $\langle$number$\rangle$-th power. If $\langle$number$\rangle$=0, the result will be 1.0 for all elements.

\subsubsection{$<$number,{\bf R}$>$ rtrgmmi}\index{b2plot, rtrgmmi}\label{sec:rtrgmmi}
Minimum of target data and $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rtrgmma}\index{b2plot, rtrgmma}\label{sec:rtrgmma}
Maximum of target data and $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rwm+}\index{b2plot, rwm+}\label{sec:rwm+}
Adds $\langle$number$\rangle$ to the array on the top of the wall stack.

\subsubsection{$<$number,{\bf R}$>$ rwm-}\index{b2plot, rwm-}\label{sec:rwm-}
Substracts $\langle$number$\rangle$ from the array on the top of the wall stack.

\subsubsection{$<$number,{\bf R}$>$ rwm*}\index{b2plot, rwm*}\label{sec:rwm*}
Multiplies the array on the top of the wall stack by $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rwm/}\index{b2plot, rwm/}\label{sec:rwm/}
Divides the array on the top of the wall stack by $\langle$number$\rangle$. Division by
zero will yield an error and do nothing.

\subsubsection{$<$number,{\bf R}$>$ rwm**}\index{b2plot, rwm**}\label{sec:rwm**}
Raises each element of the array on the top of the wall stack to the $\langle$number$\rangle$-th power. If $\langle$number$\rangle$=0.0, the result will be 1.0 for all elements.

\subsubsection{$<$number,{\bf R}$>$ rwmmi}\index{b2plot, rwmmi}\label{sec:rwmmi}
Minimum of wall data and $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ rwmma}\index{b2plot, rwmma}\label{sec:rwmma}
Maximum of wall data and $\langle$number$\rangle$.

\subsubsection{$<$number,{\bf R}$>$ meq}\index{b2plot, meq}\label{sec:meq}
If a matrix element is equal to $\langle$number$\rangle$, sets this matrix element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ mne}\index{b2plot, mne}\label{sec:mne}
If a matrix element is not equal to $\langle$number$\rangle$, sets this matrix element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ mge}\index{b2plot, mge}\label{sec:mge}
If a matrix element is greater than or equal to $\langle$number$\rangle$, sets this matrix element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ mgt}\index{b2plot, mgt}\label{sec:mgt}
If a matrix element is greater than $\langle$number$\rangle$, sets this matrix element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ mle}\index{b2plot, mle}\label{sec:mle}
If a matrix element is less than or equal to $\langle$number$\rangle$, sets this matrix element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ mlt}\index{b2plot, mlt}\label{sec:mlt}
If a matrix element is less than $\langle$number$\rangle$, sets this matrix element to 1.0.
Otherwise, sets it to zero.
...
If an Eirene array element is equal to $\langle$number$\rangle$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emne}\index{b2plot, emne}\label{sec:emne}
If an Eirene array element is not equal to $\langle$number$\rangle$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emge}\index{b2plot, emge}\label{sec:emge}
If an Eirene array element is greater than or equal to $\langle$number$\rangle$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emgt}\index{b2plot, emgt}\label{sec:emgt}
If an Eirene array element is greater than $\langle$number$\rangle$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emle}\index{b2plot, emle}\label{sec:emle}
If an Eirene array element is less than or equal to $\langle$number$\rangle$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emlt}\index{b2plot, emlt}\label{sec:emlt}
If an Eirene array element is less than $\langle$number$\rangle$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{epos}\index{b2plot, epos}\label{sec:epos}
If an Eirene array element is positive, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$i1,{\bf I}$>$ $<$i2,{\bf I}$>$ i+}\index{b2plot, i+}\label{sec:i+}
Adds the two integer values $\langle$i1$\rangle$ and $\langle$i2$\rangle$ on the top of the integer stack.

\subsubsection{$<$i1,{\bf I}$>$ $<$i2,{\bf I}$>$ i-}\index{b2plot, i-}\label{sec:i-}
Subtracts the integer value on the top of the integer stack $\langle$i2$\rangle$ from the integer value
second from the top $\langle$i1$\rangle$.

\subsubsection{$<$i1,{\bf I}$>$ $<$i2,{\bf I}$>$ i*}\index{b2plot, i*}\label{sec:i*}
Multiplies the two integer values $\langle$i1$\rangle$ and $\langle$i2$\rangle$ on the top of the integer stack.

\subsubsection{$<$i1,{\bf I}$>$ $<$i2,{\bf I}$>$ i/}\index{b2plot, i/}\label{sec:i/}
Divides the integer value second from the top of the integer stack $\langle$i1$\rangle$ by the integer value on top of the integer stack $\langle$i2$\rangle$. This is integer division! Division by 0 will yield an error and do nothing.

\subsubsection{$<$i1,{\bf I}$>$ $<$i2,{\bf I}$>$ i**}\index{b2plot, i**}\label{sec:i**}
Raises the integer value second from the top of the integer stack $\langle$i1$\rangle$ to the power of
the integer value on the top $\langle$i2$\rangle$. $0^0$ will yield an error and do nothing.

\subsubsection{$<$number,{\bf R}$>$ trgeq}\index{b2plot, trgeq}\label{sec:trgeq}
If a target element is equal to $\langle$number$\rangle$, sets this target element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ trgne}\index{b2plot, trgne}\label{sec:trgne}
If a target element is not equal to $\langle$number$\rangle$, sets this target element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ trgge}\index{b2plot, trgge}\label{sec:trgge}
If a target element is greater than or equal to $\langle$number$\rangle$, sets this target element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ trggt}\index{b2plot, trggt}\label{sec:trggt}
If a target element is greater than $\langle$number$\rangle$, sets this target element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ trgle}\index{b2plot, trgle}\label{sec:trgle}
If a target element is less than or equal to $\langle$number$\rangle$, sets this target element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ trglt}\index{b2plot, trglt}\label{sec:trglt}
If a target element is less than $\langle$number$\rangle$, sets this target element to 1.0.
Otherwise, sets it to zero.
...
If a wall element is equal to $\langle$number$\rangle$, sets this wall element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ wmne}\index{b2plot, wmne}\label{sec:wmne}
If a wall element is not equal to $\langle$number$\rangle$, sets this wall element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ wmge}\index{b2plot, wmge}\label{sec:wmge}
If a wall element is greater than or equal to $\langle$number$\rangle$, sets this wall element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ wmgt}\index{b2plot, wmgt}\label{sec:wmgt}
If a wall element is greater than $\langle$number$\rangle$, sets this wall element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ wmle}\index{b2plot, wmle}\label{sec:wmle}
If a wall element is less than or equal to $\langle$number$\rangle$, sets this wall element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ wmlt}\index{b2plot, wmlt}\label{sec:wmlt}
If a wall element is less than $\langle$number$\rangle$, sets this wall element to 1.0.
Otherwise, sets it to zero.
...
Reads the matrix of size ($\langle$nx$\rangle$ +2) $\times$ ($\langle$ny$\rangle$ +2) from file $\langle$filename$\rangle$ into the matrix stack.

\subsubsection{$<$nx,{\bf I}$>$ $<$ny,{\bf I}$>$ $<$nz,{\bf I}$>$ $<$filename,{\bf S}$>$ saws}\index{b2plot, saws}\label{sec:saws}
Reads the matrix of size ($\langle$nx$\rangle$ +2) $\times$ ($\langle$ny$\rangle$ +2) $\times$ $\langle$nz$\rangle$ from file $\langle$filename$\rangle$ into the
...
Reads the array of size $\langle$nwall$\rangle$ from file $\langle$filename$\rangle$ into the wall stack.
...
Plots $\langle$nx$\rangle$ graphs in the $x$-direction and $\langle$ny$\rangle$ in the $y$-direction on the same page.
...
and computational domain plots (default {\tt .true.}).
If {\tt .false.}, the plot region limits will be those defined
...
Sets number of decades to $\langle$n$\rangle$ for logarithmic plots (default 10).
...
Special cases: For double-null cases, HFS data is mapped to the inner midplane
$ix=jxi$, and for private flux data, it is mapped to the distance below the
X-point.
...
Changes the current font type to $\langle$number$\rangle$
\begin{itemize}
\item -25 TIMES ROMAN
\item -26 TIMES BOLD
\item -21 HELVETICA
...
Sets the current graph label to $\langle$label$\rangle$.

\subsubsection{$<$label, {\bf S}$>$ elbl}\index{b2plot, elbl}\label{sec:elbl}
Sets the current Eirene data graph label to $\langle$label$\rangle$.

\subsubsection{$<$number,{\bf I}$>$ $<$label, {\bf S}$>$ slbl}\index{b2plot, slbl}\label{sec:slbl}
Sets the species label of species index $\langle$number$\rangle$ to $\langle$label$\rangle$ for the current graph.

\subsubsection{$<$label, {\bf S}$>$ tlbl}\index{b2plot, tlbl}\label{sec:tlbl}
Sets the current target graph label to $\langle$label$\rangle$.

\subsubsection{$<$label, {\bf S}$>$ trgblbl}\index{b2plot, trgblbl}\label{sec:trgblbl}
Sets the current target bar label to $\langle$label$\rangle$.

\subsubsection{$<$label, {\bf S}$>$ wlbl}\index{b2plot, wlbl}\label{sec:wlbl}
Sets the current wall graph label to $\langle$label$\rangle$.

\subsubsection{$<$number,{\bf I}$>$ $<$label, {\bf S}$>$ wslbl}\index{b2plot, wslbl}\label{sec:wslbl}
Sets the species label of species index $\langle$number$\rangle$ to $\langle$label$\rangle$ for the current wall graph.

\subsubsection{$<$label, {\bf S}$>$ wallblbl}\index{b2plot, wallblbl}\label{sec:wallblbl}
Sets the current wall bar label to $\langle$label$\rangle$.
...
Sets font size for global header to $\langle$factor$\rangle$ times default size (default = 1.0).

\subsubsection{$<$factor,{\bf R}$>$ lbsz}\index{b2plot, lbsz}\label{sec:lbsz}
Sets label size to $\langle$factor$\rangle$ times default size (default = 1.0). Does not apply to the global header.

\subsubsection{$<$number,{\bf I}$>$ lfsz}\index{b2plot, lfsz}\label{sec:lfsz}
Sets line label font size to $\langle$number$\rangle$ (default = 0). If $\langle$number$\rangle$ is between 0 and 3, then the size
...
Toggles whether graphs will use nice axes with rounded values ({\tt .true.} by default).

\subsubsection{noax}\index{b2plot, noax}\label{sec:noax}
Does not plot axes.

\subsubsection{$<$factor,{\bf R}$>$ lwid}\index{b2plot, lwid}\label{sec:lwid}
Changes line width to $\langle$factor$\rangle$ times default width.
...
Changes width of dotted axes lines at zero to $\langle$factor$\rangle$ times default width.

\subsubsection{$<$ncol,{\bf I}$>$ zcol}\index{b2plot, zcol}\label{sec:zcol}\index{b2plot, colour table}
Plots axes line at zero with colour $\langle$ncol$\rangle$ (default 1). The colours available by default are:
\begin{enumerate}
...
a succession of colours starting from $|\langle$dcol$\rangle|$ with increasing index.
...
length of $\langle$length$\rangle$ meters.
...
maximum arrow length of $\langle$length$\rangle$ meters. The use of {\em vxa} and
{\em vya} can be superseded by the the commands {\bf earox} and {\bf earoy},
...
Changes default arrow width by $\langle$widthfactor$\rangle$ (default 1.0).

\subsubsection{$<$ncol,{\bf I}$>$ acol}\index{b2plot, acol}\label{sec:acol}
Plots overlaid arrows with colour $\langle$ncol$\rangle$ (default 1).
See {\bf zcol} above for a list of available colours.
...
Plots experimental data overlay with colour $\langle$ncol$\rangle$ (default 1).
...
Changes the experimental data overlay line width by $\langle$widthfactor$\rangle$ (default 1.0).

\subsubsection{$<$n,{\bf I}$>$ ytra}\index{b2plot, ytra}\label{sec:ytra}
Choose transformation function ($\langle$n$\rangle$.ne.0) for radial
coordinate to compare with experimental data, according to
{\tt n.s.r.z.a\_inner} and {\tt n.s.r.z.a\_outer}
files in the \${\tt \{SOLPSTOP\}/data.local} or \${\tt \{SOLPSTOP\}/data}
directories.
...
Plots separatrices with colour $\langle$ncol$\rangle$ (default 1).
...
Changes the default width of the separatrices line by $\langle$widthfactor$\rangle$ (default 1.0).
Also applies to the vertical separators drawn for 2-D plots.
...
Plots overlaid vacuum vessel outline with colour $\langle$ncol$\rangle$ (default 1).
See {\bf zcol} above for a list of available colours.

\subsubsection{$<$widthfactor,{\bf R}$>$ vwid}\index{b2plot, vwid}\label{sec:vwid}
Changes the default width of the vessel outline by $\langle$widthfactor$\rangle$ (default 1.0).
...
Sets colour of overlaid mesh and triangular grid to $\langle$ncol$\rangle$ (default 1).
See {\bf zcol} above for a list of available colours.
...
Sets colour of overlaid plasma outline to $\langle$ncol$\rangle$ (default 1).
See {\bf zcol} above for a list of available colours.

\subsubsection{$<$widthfactor,{\bf R}$>$ owid}\index{b2plot, owid}\label{sec:owid}
Changes the default width of the overlaid plasma outline by $\langle$widthfactor$\rangle$ (default 1.0).
...
Changes the default width of the wall data overlay line by $\langle$widthfactor$\rangle$ (default 10.0).
...
Starts contour levels at $\langle$level\textsubscript{min}$\rangle$ increasing by $\langle$delta$\rangle$ up
to $\langle$level\textsubscript{max}$\rangle$.

\subsubsection{$<$level\textsubscript{min},{\bf R}$>$ $<$level\textsubscript{max},{\bf R}$>$ $<$n,{\bf I}$>$ clvn}\index{b2plot, clvn}\label{sec:clvn}
Produces $\langle$n$\rangle$ contour levels going from $\langle$level\textsubscript{min}$\rangle$ to $\langle$level\textsubscript{max}$\rangle$.

\subsubsection{$<$n,{\bf I}$>$ $<$level$_1$,{\bf R}$>$ ... $<$level$_n$,{\bf R}$>$ clvs}\index{b2plot, clvs}\label{sec:clvs}
Sets $\langle$n$\rangle$ contour levels to the values specified by $\langle$label$_i$$\rangle$.
...
If $\langle$npplvm$\rangle$.lt.0, then this integral is equal (approximately)
to $\frac{1}{\|\langle\text{npplvp}\rangle\|}$ of the integral over the whole volume.
If $\langle$npplvm$\rangle$.ge.0, then the regions of positive and negative data
values are treated separately, with $\|\langle$npplvp$\rangle\|$ data intervals for
the positive and $\langle$npplvm$\rangle$ for the negative data range. In this case,
if $\langle$npplvp$\rangle$.ge.0 then 0 is added to the level list, otherwise, the
...
Starts contour levels at $\langle$level\textsubscript{min}$\rangle$ increasing by $\langle$delta$\rangle$ up
to $\langle$level\textsubscript{max}$\rangle$ for Eirene data contour plot.

\subsubsection{$<$level\textsubscript{min},{\bf R}$>$ $<$level\textsubscript{max},{\bf R}$>$ $<$n,{\bf I}$>$ eclvn}\index{b2plot, eclvn}\label{sec:eclvn}
Produces $\langle$n$\rangle$ contour levels going from $\langle$level\textsubscript{min}$\rangle$ to $\langle$level\textsubscript{max}$\rangle$ for Eirene data contour plot.

\subsubsection{$<$n,{\bf I}$>$ $<$level\textsubscript{1},{\bf R}$>$ ... $<$level\textsubscript{n},{\bf R}$>$ eclvs}\index{b2plot, eclvs}\label{sec:eclvs}
Sets $\langle$n$\rangle$ contour levels to the values specified by $\langle$label\textsubscript{i}$\rangle$ for Eirene data contour plots.
...
Starts contour levels at $\langle$level\textsubscript{min}$\rangle$ increasing by $\langle$delta$\rangle$ up
to $\langle$level\textsubscript{max}$\rangle$ for target data contour plot.

\subsubsection{$<$level\textsubscript{min},{\bf R}$>$ $<$level\textsubscript{max},{\bf R}$>$ $<$n,{\bf I}$>$ tclvn}\index{b2plot, tclvn}\label{sec:tclvn}
Produces $\langle$n$\rangle$ contour levels going from $\langle$level\textsubscript{min}$\rangle$ to $\langle$level\textsubscript{max}$\rangle$ for target data contour plot.

\subsubsection{$<$n,{\bf I}$>$ $<$level\textsubscript{1},{\bf R}$>$ ... $<$level\textsubscript{n},{\bf R}$>$ tclvs}\index{b2plot, tclvs}\label{sec:tclvs}
Sets $\langle$n$\rangle$ contour levels to the values specified by $\langle$label\textsubscript{i}$\rangle$ for target data contour plots.
...
Selects threshold levels at $\langle$level\textsubscript{min}$\rangle$ increasing by $\langle$delta$\rangle$ up
to $\langle$level\textsubscript{max}$\rangle$, for use in the wall data colour scheme.

\subsubsection{$<$level\textsubscript{min},{\bf R}$>$ $<$level\textsubscript{max},{\bf R}$>$ $<$n,{\bf I}$>$ wclvn}\index{b2plot, wclvn}\label{sec:wclvn}
Produces $\langle$n$\rangle$ threshold levels starting from $\langle$level\textsubscript{min}$\rangle$ to $\langle$level\textsubscript{max}$\rangle$ for the wall data
colour scheme.

\subsubsection{$<$n,{\bf I}$>$ $<$level\textsubscript{1},{\bf R}$>$ ... $<$level\textsubscript{n},{\bf R}$>$ wclvs}\index{b2plot, wclvs}\label{sec:wclvs}
Sets $\langle$n$\rangle$ threshold levels to the values specified by $\langle$label\textsubscript{i}$\rangle$ for the wall data colour scheme.
...
Sets the fill option to $\langle$fill\_opt$\rangle$.
If it equals 0, then colours
...
Copies {\tt Eirene} data onto {\tt B2.5} data. Specifically, the densities of neutrals
species (when a match between {\tt B2.5} and {\tt Eirene} species can be found)
computed from {\tt Eirene} ({\em dab2}) are added to the fluid neutral densities from
{\tt B2.5} ({\em na}).

\subsubsection{$<$value,{\bf R}$>$ evac}\index{b2plot, evac}\label{sec:evac}
Assigns real value $\langle$value$\rangle$ to all triangles from the top of the Eirene data stack that do not map to a {\tt B2.5} grid cell.
...
Writes out the top of the matrix stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) \verb:b2pl.exe.dir/:$\langle$filename$\rangle$.

\subsubsection{$<$filename,{\bf S}$>$ eprt}\index{b2plot, eprt}\label{sec:eprt}
Writes out the top of the Eirene stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) \verb:b2pl.exe.dir/:$\langle$filename$\rangle$.

\subsubsection{$<$x1,{\bf R}$>$ $<$y1,{\bf R}$>$ $<$x2,{\bf R}$>$ $<$y2,{\bf R}$>$ $<$x3,{\bf R}$>$ $<$y3,{\bf R}$>$ $<$x4,{\bf R}$>$ $<$y4,{\bf R}$>$ eavg}\index{b2plot, eavg}\label{sec:eavg}
Writes out the volume average of the data on top of the Eirene stack over the rectangular region delimited by the four doublets of coordinates [($x1$,$y1$);($x2$,$y2$);($x3$,$y3$);($x4$,$y4$)].

\subsubsection{$<$ix1,{\bf I}$>$ $<$ix2,{\bf I}$>$ $<$iy1,{\bf I}$>$ $<$iy2,{\bf I}$>$ $<$filename,{\bf S}$>$ mtab}\index{b2plot, mtab}\label{sec:mtab}
Writes out a table extracted from the matrix on the top of the stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) \verb:b2pl.exe.dir/:$\langle$filename$\rangle$. $0$ $0$ pairs for the $ix$ or $iy$ values will
output the entire $ix$ or $iy$ range, respectively.

\subsubsection{$<$filename,{\bf S}$>$ wprt}\index{b2plot, wprt}\label{sec:wprt}
Writes out the top of the wall stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) \verb:b2pl.exe.dir/:$\langle$filename$\rangle$.

\subsubsection{$<$iw1,{\bf I}$>$ $<$iw2,{\bf I}$>$ $<$filename,{\bf S}$>$ wtab}\index{b2plot, wtab}\label{sec:wtab}
Writes out a table extracted from the array on the top of the wall stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) \verb:b2pl.exe.dir/:$\langle$filename$\rangle$. A $0$ $0$ pair for the $iw$ value will
output the entire range.

\subsubsection{$<$filename,{\bf S}$>$ trgprt}\index{b2plot, trgprt}\label{sec:trgprt}
Writes out the top of the target stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) \verb:b2pl.exe.dir/:$\langle$filename$\rangle$.

\subsubsection{$<$iw1,{\bf I}$>$ $<$iw2,{\bf I}$>$ $<$depth1,{\bf I}$>$ $<$depth2,{\bf I}$>$ $<$filename,{\bf S}$>$ trgtab}\index{b2plot, trgtab}\label{sec:trgtab}
Writes out a table extracted from the matrix on the top of the target stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) \verb:b2pl.exe.dir/:$\langle$filename$\rangle$. $0$ $0$ pairs for the $iw$ or $depth$ values will
...
Writes out data from element ($\langle$ix$\rangle$,$\langle$iy$\rangle$) from the top of the matrix stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or \verb:b2pl.exe.dir/:$\langle$filename$\rangle$ (in the \SOLPSfive environment).

\subsubsection{$<$i,{\bf I}$>$ $<$filename,{\bf S}$>$ eoutp}\index{b2plot, eoutp}\label{sec:eoutp}
Writes out data from triangle element $\langle$i$\rangle$ from the top of the matrix stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or \verb:b2pl.exe.dir/:$\langle$filename$\rangle$ (in the \SOLPSfive environment).

\subsubsection{$<$i,{\bf I}$>$ $<$iz,{\bf I}$>$ $<$filename,{\bf S}$>$ outptrg}\index{b2plot, outptrg}\label{sec:outptrg}
Writes out data from element ($\langle$i$\rangle$,$\langle$iz$\rangle$) from the top of the target stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or \verb:b2pl.exe.dir/:$\langle$filename$\rangle$ (in the \SOLPSfive environment).

\subsubsection{$<$i,{\bf I}$>$ $<$filename,{\bf S}$>$ outpw}\index{b2plot, outpw}\label{sec:outpw}
Writes out data from element $\langle$i$\rangle$ from the top of the wall stack to $\langle$filename$\rangle$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) to \verb:b2pl.exe.dir/:$\langle$filename$\rangle$.

\subsubsection{f30}\index{b2plot, f30}\label{sec:f30}
Produces the \verb:fort.30: file containing the geometry for use within {\tt Eirene}.

\subsubsection{f31}\index{b2plot, f31}\label{sec:f31}
Produces the \verb:fort.31: file containing the plasma state for use within {\tt Eirene}.
...
in \${\tt SOLPSTOP/modules/B2.5/Database/Summers} (which can be superseded
by the contents of \${\tt SOLPSTOP/data.local/Summers}). One pre-selected
diagnostic line per ionization stage is provided. Data is only provided for
...
Generates the spectral line emissivities according to the {\tt ADAS} database \cite{ADAS}, stored in \${\tt SOLPSTOP/modules/adas}
or \${\tt SOLPSTOP/data.local/adas}, for species index $\langle$is$\rangle$, centered around wavelength $\langle$lambda$\rangle$ (in {\em \AA}),
with width $\langle$delta$\rangle$ (in {\em \AA}).
{\bf nspec} is an alias for {\bf nspe}.

\subsubsection{$<$n,{\bf I}$>$ mvsk}\index{b2plot, mvsk}\label{sec:mvsk}
Sets the movie skip factor to $\langle$n$\rangle$ (default 1).
...
Overwrites the inner midplane location index {\em jxi} with $\langle$ix$\rangle$.
...
Overwrites the outer midplane location index {\em jxa} with $\langle$ix$\rangle$.
...
Overwrites the radial reference location index {\em iyref} with $\langle$iy$\rangle$.
...
Averages over $\langle$n$\rangle$ chords for {\bf chor} and {\bf chvl}.

\subsubsection{$<$device name,{\bf S}$>$ pdev}\index{b2plot, pdev}\label{sec:pdev}
Sets value of {\tt PLPLOT} device name to $\langle$device name$\rangle$.

\subsubsection{$<$winid,{\bf I}$>$ swid}\index{b2plot, swid}\label{sec:swid}
Sets value of {\tt WIN\_ID} environment variable (part of {\tt TCL/TK} interface) to $\langle$winid$\rangle$.
...
Optional user routine. The choice of user routine is governed by $\langle$ig$\rangle$.
...
\begin{description}
  ...
  \item[ig = -12] Produces a file with coordinates of the grid cell centers ({\tt wuerz-g.dat}) and the plasma parameters (densities, temperatures, fluid velocities) ({\tt wuerz-d.dat}) in the outer divertor
  ...
  \item[ig = -8] Creates files {\tt pr.dat} (plasma pressure), {\tt prn.dat} (neutral pressure), {\tt prt.dat} (total pressure) and {\tt sm.dat} (neutral momentum source)
  ...
  \item[0 $\le$ ig $\le$ ny] Produces a file {\tt oleg.dat} with plasma parameters (density, temperature, parallel flow velocity, parallel heat flux) against the distance along the magnetic field along flux surface index $\langle$ig$\rangle$
  \item[ny $<$ ig $<$ 1000] Produces two files with estimated helium concentration ({\tt fraction.he}) and neutral pressure ({\tt pressure.ntr}) in the PFR for summary plots
  \item[ig $\ge$ 1000] Adds a line to the file {\tt ../sum\_tmp/spec\_pwr.dat} with the power entering both divertors, power radiated along the divertor legs, power radiated near the targets, areas of the separatrix branches and mean specific power radiated along the divertor legs per square meter. $\langle$ig$\rangle$ -1000 cells are treated as "close to the targets"
\end{description}
...
For instance, the 'blue\_to\_red' color maps are useful for data fields that
...
and shows examples of the 'jet', 'blue\_to\_red', and 'rainbow' palettes.
...
mach
-1.2 1.2 17 clvn
'rainbow_16' cltb 'rainbow palette' extl
surf
\end{verbatim}
...
\subsection{$<$chord file$>$ echor}\index{b2plot, echor}\label{sec:echor}\index{b2plot, chord plots}
...
Currently the following chord file examples (from AUG diagnostics) are provided (in the \${\tt SOLPSTOP/doc/solps/chords} directory):
\begin{itemize}
...
The command file may contain comment lines, starting with '*', '!' or '\#'.
...
\subsection{$<$chord file$>$ echbx}\index{b2plot, echbx}\label{sec:echbx}
Chord integral of Eirene data averaged over three chords. Equivalent to
using the {\bf 3 ave} command.

\subsection{$<$position file$>$ echvl}\index{b2plot, echvl}\label{sec:echvl}
The Eirene stack data value is plotted at the specified positions.

\subsection{$<$chord file$>$ echmp}\index{b2plot, echmp}\label{sec:echmp}
Plots a contour plot of the chord contribution from the triangles.

\subsection{$<$chord file$>$ ecam}\index{b2plot, ecam}\label{sec:ecam}
...
Creates a movie for timeslice $\langle$istart$\rangle$ to time slice $\langle$iend$\rangle$ of variable $\langle$varname$\rangle$. The
...
Creates a movie for timeslice $\langle$istart$\rangle$ to time slice $\langle$iend$\rangle$ of variable $\langle$varname$\rangle$ read
from file $\langle$filename$\rangle$.
...
\subsection{$<$z\textsubscript{shift},{\bf R}$>$ $<$UEDGE grid file$>$ uedgegrd}\index{b2plot, uedgegrd}\label{sec:uedgegrd}
Outputs the geometry in {\tt UEDGE} ASCII format. The grid can be shifted in the vertical
direction by an amount of $\langle$z\textsubscript{shift}$\rangle$ meters.
...
Computes a reaction rate as per the {\tt ADAS} specification. The $\langle$reac$\rangle$ string should be something of the
form 'acd89'. The matrix obtained contains then all the rates from these files, for all species in the run, extracted
from the files stored in \${\tt SOLPSTOP/modules/adas} or \${\tt SOLPSTOP/data.local/adas}.

\section{Environment variables}

\begin{description}
\item[B2PLOT\_DEV] is used to specify plotting to any combination of
  "x11", "ps" or "cgm" (default is "ps").  Under {\tt tcsh}, use
  \begin{verbatim}
setenv B2PLOT_DEV 'x11 ps'
  \end{verbatim}
\item[SOLPSTOP] used to specify the top of the {\tt SOLPS} tree.
\end{description}
...
\else
  \subfigure[a4p vesl outl sep 'JET/KL2.chords' chpl]{\label{fig:chpl}\includegraphics[height=6cm]{chpl.eps}}
  \subfigure[a4l phys 3 6581 1 nspec '658 nm CII' extl 'JET/KL2.chords' chor]{\label{fig:chor}\includegraphics[angle=90,height=6cm]{chor.eps}}
\fi
...
as obtained from the {\tt JET/KL2.chords} file in the
\${\tt SOLPSTOP/data/chords} directory. The second {\bf chpl} plot (\ref{fig:chor}),
...
\else
  \subfigure[phys 0 nseg 4 1. 2. 3. 4. wclvs 10.0 wwid wall mesh]{\label{fig:nseg}\includegraphics[height=6cm]{nseg.eps}}
  \subfigure[vesl sep phys wall 10.0 wwid sputchem 1 wzsel 3 ndec logw fnax 1 zsel 'Ychem D+' wallblbl surf]{\label{fig:Ychem}\includegraphics[height=6cm]{Ychem.eps}}
\fi
  \caption{Wall plots.}
  \label{fig:wallplots}
\end{figure}
...
by {\tt Eirene}, you need to add contributions from {\bf neutrad} as well.
Please be mindful however that {\bf neutrad} has dimensions $nspecies$ while
{\bf b2ra} has dimensions $ns$. Moreover, if you are intending to compute the
total radiation rate, you should also include the bremsstrahlung contribution
from {\bf b2br}. You would then have as your command line:

\begin{verbatim}
! b2plot < b2rad.plot ; ctrans -d ps.land.color gmeta | a4page > ! tmp.ps
a4l 2 1 page lbof logf phys
b2ra b2br m+ pvol abs 2  8 sumz neutrad pvol 2 zsel m+
1.0e4 fmin 1.0e8 fmax 'Total radiation loss due to Carbon' extl surf
b2ra b2br m+ pvol abs 9 19 sumz neutrad pvol 3 zsel m+
1.0e4 fmin 1.0e8 fmax 'Total radiation loss due to Neon' extl surf
\end{verbatim}
...
The file \${\tt SOLPSTOP/modules/B2.5/src/include/DIMENSIONS.F} and its local version
\${\tt SOLPSTOP/modules\allowbreak/B2.5/src/include.local/DIMENSIONS.F} contain key dimensions
that determine
the sizes of arrays that can be defined by namelists
in {\tt b2.*.parameters} files (for compatibility with
certain compilers).

Upon the first creation of the local version of the {\tt DIMENSIONS.F} file,
...
The quantities defined in {\tt DIMENSIONS.F} are:
\begin{description}
...
additional vacuum cells outside the gridded domain in {\tt Eirene}
\item[DEF\_NATM]\index{DEF\_NATM}\index{NATM} the maximum number of
neutral atomic species to be handled by {\tt Eirene}
...
energy bins for the diagnostic chords in {\tt Eirene}
...
passed from {\tt B2.5} to {\tt Eirene}
...
ion species to be handled by {\tt Eirene}
...
additional surfaces to be handled by {\tt DivGeo}
\item[DEF\_NMOL]\index{DEF\_NMOL}\index{NMOL} the maximum number of
neutral molecular species to be handled by {\tt Eirene}
...
plasma species to be handled by {\tt Eirene}
...
reaction cards that can be created by {\tt Uinp}
\item[DEF\_NRDS]\index{DEF\_NREAC}\index{NREAC} the maximum number of
molecular dissociation reaction cards that can be created by {\tt Uinp}
\item[DEF\_NREC]\index{DEF\_NREAC}\index{NREAC} the maximum number of
recombination reaction cards that can be created by {\tt Uinp}
\item[DEF\_NREL]\index{DEF\_NREAC}\index{NREAC} the maximum number of
elastic collisions reaction cards that can be created by {\tt Uinp}
\item[DEF\_NSD]\index{DEF\_NSD}\index{NSD} the maximum number of plasma
species handled in {\tt B2.5}
\item[DEF\_NSPCSRFG]\index{DEF\_NSPCSRFG}\index{NSPCSRFG} the maximum
number of groups of {\tt Eirene} surfaces for which the data can be
transferred from {\tt B2.5} ({\tt DG} specification "Surface special")
...
standard surfaces to be handled by {\tt Eirene}
...
The current default settings of {\tt DIMENSIONS.F}, which allow for a
...
and, as a prototypical example, the {\tt config.ITER.ifort64} in
...
If the user wishes to modify the compiling options used, the creation
of a {\tt config.}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}.local} file
is recommended, in which to store the modified settings. Since the {\tt Makefile}
reads those after the
{\tt config/compiler.\allowbreak}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}} file,
they need only contain the modified definitions.

\section{Defines}

\subsection{SOLPS-ITER}
\index{DEFINES, used throughout SOLPS-ITER}

We list here the possible compiler options that can be used when compiling the
various components of the \SOLPSITER code suite. The options below must therefore,
if desired, be declared as part of the \${\tt SOLPS\_CPP} environment variable,
which is defined in the
\${\tt SOLPSTOP/SETUP/config.}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}(.local)} files.
...
\item[FLUSH] \index{FLUSH} Allows for flushing of some output files
\item[F2003] \index{F2003} To be used if the compiler is compatible with {\tt Fortran 2003}
...
{\tt CLASSICAL\_CURRENT}) to the {\tt SOLPS\_CPP} environment variable
with
\begin{verbatim}
...
\subsection{Carre}
\index{DEFINES, used in compiling Carre}

%%grep #if $SOLPSTOP/modules/Carre/src/*/*.F | awk '{print $2}' | sort | uniq

We list here additional possible compiler options that can be used when compiling {\tt Carre}.

\begin{description}
\item[BUILDING\_CARRE] \index{BUILDING\_CARRE} Flags code for extended grids handling routine specific to {\tt Carre}
\item[EXTRA\_GRAPHS] \index{EXTRA\_GRAPHS} Turns on additional graphical output of intermediate steps of the {\tt Carre} grid generation
\item[MODIF] \index{MODIF} Allows for manual modifications to the grid generation process during the {\tt Carre} run
\item[READOPT] \index{READOPT} Allows for format-free keyboard input
\end{description}

%%grep #if $SOLPSTOP/modules/Carre/src/*/*.F | awk '{print $2,$1}' | sort | uniq

We list below where the {\tt Carre} compilation switches are used.
...
DBG & carre/saute.F \\
DBG & fcrr/fcrprp.F \\
EXTRA\_GRAPHS & carre/carre.F \\
GFORTRAN & carre/rdfrin.F \\
GFORTRAN & carre/rdfrre.F \\
MODIF & carre/maille.F \\
MODIF & carre/mailrg.F \\
READOPT & carre/rdfrch.F \\
READOPT & carre/rdfrin.F \\
READOPT & carre/rdfrre.F \\
    \hline
    \caption{Defines for Carre and where they are used.}
    \label{tab:Carredefines}
  \end{longtable}
\end{center}
...
if desired, be declared as part of the \${\tt SOLPS\_CPP} environment variable,
which is defined in the
\${\tt SOLPSTOP/SETUP/config.\allowbreak}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}(.local)} files.
...
\item[B25\_EIRENE] \index{B25\_EIRENE} Set internally by the {\tt Makefile} and used to identify differences between source code for stand-alone {\tt B2.5} and coupled {\tt B2.5-Eirene} applications
...
\item[SOLPS4\_3] \index{SOLPS4\_3} Set internally by the {\tt Makefile} to activates code specific to the {\tt SOLPS4-5 b2sxdr} converter
...
\item[COMPLICATED] \index{COMPLICATED} Uses a more "complicated" model for feedback schemes
\item[EASY] \index{EASY} Uses the "easy" model for chord integration
\item[GOT\_AMNSPROTO] \index{GOT\_AMNSPROTO} Links to AMNS database
...
If you want to activate any of these options, you should add
{\tt -DOPTION} (where {\tt OPTION} might, for example, be
{\tt CLASSICAL\_CURRENT}) to either the {\tt SOLPS\_CPP}
...
B25\_EIRENE & sources/b2stbc\_fb.F \\
...
F2003 & convert/b2cdftallies2mds.F \\
F77 & user/b2file.F \\
GFORTRAN & utility/ilutern\_solps.F \\
GFORTRAN & utility/ilutern\_7\_solps.F \\
GFORTRAN & utility/ilutern\_9\_solps.F \\
GFORTRAN & utility/ilutern\_11\_solps.F \\
GFORTRAN & utility/ma28.F \\
GFORTRAN & utility/ma28copy.F \\
GFORTRAN & utility/ma28copy3.F \\
GFORTRAN & utility/ma28copy4.F \\
...
ITM & ids/b2mod\_ual.F90 \\
ITM & ids/b2mod\_ual\_io\_data.F90 \\
ITM & ids/b2mod\_ual\_io\_grid.F90 \\
ITM & ids/b2mod\_ual\_io.F90 \\
ITM & ids/b2\_ual\_write.F90 \\
...
NAGFOR & b2plot/b2plot.F \\
NAGFOR & b2plot/dhdata.F \\
NAGFOR & b2plot/initialize\_plotting.F \\
NAGFOR & b2plot/set\_extralabel.F \\
NAGFOR & b2plot/spect.F \\
NAGFOR & convert/b2co.F \\
NAGFOR & convert/b2co\_movies\_cdf.F \\
NAGFOR & convert/b2co\_tally\_cdf.F \\
NAGFOR & convert/b2co\_time\_cdf.F \\
NAGFOR & convert/b2co\_wall\_cdf.F \\
NAGFOR & convert/b2mds.F \\
NAGFOR & ids/b2\_ual\_write.F90 \\
NAGFOR & modules/b2mod\_ppout.F \\
NAGFOR & modules/b2mod\_solpstop.F \\
NAGFOR & modules/b2mod\_types.F \\
NAGFOR & output/b2wgpj\_gnuplot.F \\
NAGFOR & preprocessing/b2agfs.F \\
NAGFOR & utility/calc\_atomic\_data.F \\
NAGFOR & utility/open\_file.F \\
NAGFOR & utility/usrnam.F \\
...
NO\_OPENMP\_B2SIHS & sources/b2sihs.F \\
...
USE\_MPI & convert/b2mddr.F \\
USE\_MPI & convert/b2xd.F \\
USE\_MPI & driver/b2co.F \\
USE\_MPI & driver/b2fu.F \\
USE\_MPI & driver/b2md.F \\
USE\_MPI & driver/b2mnastra.F \\
USE\_MPI & driver/b2mn\_opt.F \\
USE\_MPI & driver/b2mod\_driver.F \\
USE\_MPI & driver/b2mod\_main.F \\
USE\_MPI & driver/b2op.F \\
USE\_MPI & driver/b2pl.F \\
USE\_MPI & driver/b2rd.F \\
USE\_MPI & driver/b2ts.F \\
USE\_MPI & driver/b2uf.F \\
...
USE\_MPI & postprocessing/b2yg.F \\
USE\_MPI & postprocessing/b2yh.F \\
USE\_MPI & postprocessing/b2yi.F \\
USE\_MPI & postprocessing/b2yi\_gnuplot.F \\
USE\_MPI & postprocessing/b2ym.F \\
USE\_MPI & postprocessing/b2yn.F \\
USE\_MPI & postprocessing/b2yp.F \\
USE\_MPI & postprocessing/b2yq.F \\
USE\_MPI & postprocessing/b2yr.F \\
USE\_MPI & postprocessing/b2ytdr.F \\
USE\_MPI & postprocessing/b2yt.F \\
USE\_MPI & postprocessing/b2yv.F \\
USE\_MPI & preprocessing/b2ag.F \\
USE\_MPI & preprocessing/b2ah.F \\
USE\_MPI & preprocessing/b2ai.F \\
USE\_MPI & preprocessing/b2ar.F \\
USE\_MPI & preprocessing/b2fgmtry\_mod.F \\
...
B25\_EIRENE & main-routines/eirene.f \\
B25\_EIRENE & interfaces/couple\_SOLPS-ITER/eirmod\_extrab25.F90 \\
B25\_EIRENE & startup-routines/find\_param.f \\
DBG & interfaces/couple\_SOLPS-ITER/eirmod\_infcop.F \\
HPUX & modules/eirmod\_cinit.F \\
MPI\_VERSION & modules/eirmod\_mpi.F90 \\
NAGFOR & file-handling/get\_solpstop.f \\
NO\_B2\_CHEM\_SPUT & interfaces/couple\_SOLPS-ITER/eirmod\_infcop.F \\
NO\_GETENV & file-handling/get\_solpstop.f \\
TRACE & geometry/time-routines/checks.F \\
TRACE & geometry/time-routines/timea.F \\
TRACE & geometry/time-routines/timep.F \\
TRACE & geometry/time-routines/timer.F \\
TRACE & geometry/time-routines/timet.F \\
USE\_MPI & assistant/exit\_own.f \\
USE\_MPI & modules/eirmod\_mpi.F90 \\
USE\_MPI & photons/eirmod\_phot\_reflec.f \\
USE\_MPI & startup-routines/input.f \\
USE\_PXFGETENV & file-handling/get\_solpstop.f \\
WINDOWS & main-routines/eirene.f \\
    \hline
...
%% \subsection{Uinp}
%% \index{DEFINES, used in compiling Uinp}

%%grep #if $SOLPSTOP/modules/Uinp/*/*.[Ff]* | grep -v builds | awk '{print $2,$1}' | sort | uniq

%% We list here the possible compiler options that can be used when compiling {\tt Uinp}. The options below must therefore, if desired, be declared as part of the \${\tt SOLPS\_CPP} environment variable, which is defined in the \${\tt SOLPSTOP/SETUP/config.}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}(.local)} files.

%%grep #if $SOLPSTOP/modules/Uinp/src/*.F | awk '{print $2,$1}' | sort | uniq

%% We list below where the {\tt Uinp} compilation switches are used.
...
%% HPUX & uiblkd.F \\
%%     \hline
%%     \caption{Defines for Uinp and where they are used.}
%%     \label{tab:Uinpdefines}
%%   \end{longtable}
%% \end{center}
...
interface routines producing the input files for {\tt B2-Eirene} and {\tt Sonnet} or {\tt Carre}
                                                    & .trg \\
Structure file&Specification of the structures - to be used
by the interface routines producing the input files for {\tt Sonnet} or {\tt Carre}
                                                    & .str \\ \hline
...
({\tt Sonnet} or {\tt Carre}), and also to
...
({\tt Sonnet} or {\tt Carre}) and imported into {\tt DG}.
...
automatically saves a version of the file with a '{\tt ~}'
...
The \${\tt SOLPSTOP/modules/DivGeo/dg/equtrn}
...
\${\tt SOLPSTOP/modules/DivGeo/dg/equtrn/doxygen/refman.pdf}.
...
This is a script that invokes the {\tt dg2ef}
executable to convert a
{\tt DG} equilibrium file to the {\tt EFIT} format (specifically, the {\tt EFIT} "g" file).
...
This is a script that invokes the {\tt dg2vr}
executable to convert a
{\tt DG} equilibrium file to the {\tt Carre} (or TdeV) equilibrium
format.
This is invoked with (the file suffixes are recommended for clarity
and are not enforced):
\begin{quote}
...
executable to convert an {\tt EFIT} format (specifically, the {\tt EFIT} "g" file)
...
This is a script that invokes the {\tt nk2dg}
...
{\tt DG} equilibrium file by the specified amount.  To
invoke it:
\begin{quote}
...
maximum vertical coordinate. The code
...
points differ slightly for the {\tt Sonnet} and {\tt Carre} mesh generators.
So, before undertaking these tasks, the user should select the
appropriate output mode in {\tt Options|Output mode}.
Since only the {\tt Carre} code is distributed along with this manual,
...
{\tt Carre} will perform its own set of checks when invoked as well.
...
{\tt Carre}.  If you are having problems getting the surfaces you want,
...
\subsection{Limiter configurations}
\index{DG, limiter configurations}

If you are dealing with a limiter configuration, {\tt DG} needs to be
...
provides the actual equilibrium information.
...
geometry you really want to model. {\tt Import} the single-null
{\tt Topology}. {\tt Create} a {\tt Surface} in the core region of the
diverted equilibrium as usual - preferably at about the same depth that you
will want for the limiter case grid. Create also two closed polygons
to act as targets for this SN equilibrium. Declare them as {\tt Inner target}
and {\tt Outer target}, and include them as part of the {\tt Structure}
in the usual way, as decribed in Section~\ref{sec:createsurfaces}.
  \item {\tt Add} a {\tt Limiter configuration} variable, and cut and paste the
name of the limiter configuration equilibrium file into the
{\tt Real equilibrium} there. Specify the depth of the grid you wish to obtain
by setting the {\tt Core width} (in units of $m$).
  \item Include the limiter walls in the {\tt Structure}, plus some
grid-limiting parts if needed.
  \item The {\tt Target specification} variables refer to the
two halves of the limiter on either side of the tangency point. Provide there
all relevant material and sputtering properties. {\tt Targets \#1} and 
{\tt \#2} should be positioned so that you travel
clockwise around the plasma when going from {\tt Target \#1} to
{\tt Target \#2}. Use the real elements you want to be reconnected to the
...
Only the {\tt Core} zone for the {\tt Grid points} and the {\tt SOL} and
{\tt Core} areas for the {\tt Surfaces} are meaningful.
  \item In {\tt Global Eirene data}, make sure that the {\tt Major radius}
is set to its real positive value (in units of $cm$).
...
the limiter configuration equilibrium.
\end{enumerate}
...
{\tt Radial scale} or {\tt Poloidal scale} in the {\tt Limiter configuration}
layer.
...
user must check to be sure that all such constraints are satisfied.
Again, {\tt Carre} will perform its own series of such checks when invoked.
...
{\tt DG} allows you to import a mesh in the {\tt Sonnet} format. {\tt Carre}'s output
...
the mesh generator ({\tt Carre} or otherwise) is forcing the mesh to
...
b2fgmtry \index{b2fgmtry} & un*formatted & b2ag & b2ah b2ai b2co b2fu b2md b2mn b2pl b2ts b2uf b2xd b2yg b2yh b2yp b2yt b2\_ual\_write & geometry \\
b2fmovie \index{b2fmovie} & un*formatted & b2mn & b2ym & movie of plasma evolution \\
b2fparam \index{b2fparam} & un*formatted & b2mn & b2md b2fu b2pl b2ts b2uf b2xd b2yh b2yp b2\_ual\_write & run parameters used for plotting \\
b2fpardf \index{b2fpardf} & un*formatted & b2ah & b2co b2mn b2yh b2yt & transport parameters \\
b2fplasma \index{b2fplasma} & unformatted & b2fu b2mn & b2md b2pl b2ts b2uf b2xd b2\_ual\_write & plasma state used for plotting \\
b2fplasmf \index{b2fplasmf} & formatted & b2uf & b2fu & plasma state used for plotting \\
b2frates \index{b2frates} & un*formatted & b2ar & b2md b2mn b2pl b2yn b2yp b2yr b2yt b2\_ual\_write & atomic physics rates \\
b2fstate \index{b2fstate} & un*formatted & b2co b2mn b2rd & b2fu b2md b2pl b2ts b2uf b2xd b2yi b2yi\_gnuplot b2yp b2yr b2yt b2\_ual\_write & final plasma state \\
b2fstati \index{b2fstati} & un*formatted & b2ai & b2co b2mn b2yi b2yi\_gnuplot & initial plasma state \\
...
b2faveri \index{b2faveri} & formatted & & b2mn & initial averaged plasma solution \\
b2favere \index{b2favere} & formatted & b2mn & b2ye & final averaged plasma solution \\
