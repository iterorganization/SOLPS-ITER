...
\item[BCCON]\index{BCCON} {\tt integer array, length NS * NBC}, specifying
  ...
  \item[16] particle density to achieve specified total flux, used with ASTRA coupling. The total desired flux is summed over all {\tt BCCON=16} core boundaries.
  {\tt CONPAR(,,1)} specifies the desired particle flux in $s^{-1}$.
  \item[18] prescribe total main ion particle flux, used with ASTRA coupling.
  \item[19] particle flux feedback boundary condition, flux is summed over neutrals and ions, for coupling with ASTRA. The total desired flux is summed over all {\tt BCCON=19} core boundaries. This boundary condition applies to ions in their highest ionisation stage.
  {\tt CONPAR(,,1)} specifies the desired particle flux in $s^{-1}$.
...
\item[BCMOM]\index{BCMOM} {\tt integer array, length NS * NBC}, specifying
  ...
  {\tt MOMPAR(,,2)} $< \frac12$, then the velocity is set to exactly
...
\end{gather}
\begin{gather}
 D^{p}_{AN,0} = C + \sqrt{\frac{T_i}{m_N}} \frac{1}{( \sigma_{CX} n_i + \sigma_{in} n_e )},
\end{gather}
...
A note about sign conventions. The radial coordinate $y$ increases as one crosses the separatrix from the core (or private flux) region into the main SOL region (and so is minimum at the magnetic axis). The poloidal coordinate $x$ increases in the {\em clockwise} direction when viewing the poloidal plane with the central axis line on its left. The toroidal magnetic field component $b_z$ is taken as positive if the toroidal magnetic field is directed {\em clockwise} when viewing the tokamak from above. The poloidal magnetic field component $b_x$ follows the same convention as the $x$ component and is positive if when directed in the direction of increasing poloidal index, i.e. {\em clockwise}.
...
\begin{equation}
\widetilde{ \Gamma}_{ay} = \left( V_{ay}^{\left( E \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} \right) - D_a^n \frac{1}{h_y} \frac{\partial n_a}{\partial y} - \widetilde{D}_{ay}^p \frac{1}{h_y} \frac{\partial p_a}{\partial y} + D_{ayx} \frac{1}{h_x} \frac{\partial n_a T_i}{\partial x}
\end{equation}

\begin{equation}
\Gamma_{ax} = \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + \widetilde{V}_{ax}^{\left( dia \right)} + V_{ax}^{\left( AN \right)} + V^{corr \_ dpc} \right) n_a - D_a^n \frac{1}{h_x} \frac{\partial n_a}{\partial x} - \widetilde{D}_{ax}^p \frac{1}{h_x} \frac{\partial p_a}{\partial x}
\end{equation}

\begin{equation}
\Gamma_{ay} = \left( V_{ay}^{\left( E \right)} + \widetilde{V}_{ay}^{\left( dia \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} \right) - D_a^n \frac{1}{h_y} \frac{\partial n_a}{\partial y} - \widetilde{D}_{ay}^p \frac{1}{h_y} \frac{\partial p_a}{\partial y}
\end{equation}
...
\begin{equation}
\Gamma_{ax}^{\left( Eir \right)} = \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + V_{ax}^{\left( dia \right)} + V_{ax}^{\left( AN \right)} + V^{corr \_ dpc} \right) n_a - D_a^n \frac{1}{h_x} \frac{\partial n_a}{\partial x}
\end{equation}

\begin{equation}
\Gamma_{ay}^{\left( Eir \right)} = \left( V_{ay}^{\left( E \right)} + V_{ya}^{\left( dia \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} \right) - D_a^n \frac{1}{h_y} \frac{\partial n_a}{\partial y}
\end{equation}
...
\begin{equation*}
V_{ay}^{(AN)} = c_{V_a^{(AN)},0} + c_{V_a^{(AN)},1} \frac{10^{20}}{n_e} + c_{V_a^{(AN)},2} D_a^f + c_{V_a^{(AN)},3} \frac{6.25 \cdot 10^{-2}T_e}{e|B|}
\end{equation*}
...
\begin{equation}
D_{ANa}^n=\left( c_{D_a^N,0}+c_{D_a^N,1}\frac{10^{20}}{n_e}+c_{D_a^N,2}D_a^f+c_{D_a^N,3}\frac{6.25 \cdot
...
$D_a^f=c_{D_a^f,0} + c_{D_a^f,1} \frac{10^{20}}{n_i}
+ \frac{
	c_{D_a^f,2} \sqrt{\frac{T_i}{m_a}}
}{
	c_{D_a^f,3} n_i + c_{D_a^f,4} n_e
},$
...
\begin{matrix}
\frac{D_a^p}
{\left(1+\left(\frac{\frac{\sqrt{g}}{h_x} D_a^p \left| \frac{1}{h_x} \frac{\partial p_a}{\partial x} \right| }{\frac{\sqrt{g}}{h_x} \mathit{\alpha n}_a \frac{V_{T_i}} 4} \right)^{\gamma _1} \right)^{\frac 1{\gamma_1}}},
...
\begin{matrix}
\frac{D_a^p}
{\left(1+\left(\frac{\frac{\sqrt{g}}{h_y} D_a^p \left| \frac 1{h_y}\frac{\partial p_a}{\partial y} \right| }{\frac{\sqrt{g}}{h_x} \mathit{\alpha n}_a\frac{V_{T_i}} 4}\right)^{\gamma _1}\right)^{\frac 1{\gamma_1}}},
& \ V_{T_i}=\sqrt{\frac{8T_i}{\mathit{\pi m}_a}}, & \ z_a = 0 \\
D_a^p, & \ z_a \neq 0 \\
\end{matrix} \right.
\end{matrix}
...
\begin{equation*}
D_{ANa}^p=\frac 1{z_aT_e+T_i} \left( c_{D_a^p,0} + c_{D_a^p,1} \frac{10^{20}}{n_e} + c_{D_a^p,2} D_a^f + c_{D_a^p,3} \frac{6.25 \cdot
10^{-2}T_e}{e|B|}\right),
\end{equation*}
...
single main hydrogenic ion and an arbitrary amount of impurities. For cases
...
$S_{Therm,ea}^m = - \alpha_{ex} c_e^{(1)} \frac{n_a z_a^2 m_e}{b_x e \zeta_e} \frac{1}{c_{\sigma_{CL \parallel}}^{(Luciani)}} \frac{\partial T_e}{h_x \partial
x}, ~ z_a \ne 0$
...
$z_{eff\_imp} = \frac{\sum \limits_{a=0, a \ne a_{main}}^{ns - 1} n_a z_a^2}{n_{a_{main}} z_{a_{main}}^2}$, ~ $c_{\sigma_{CL \parallel}}^{(Luciani)} = c_{\kappa_{ex}^{(CL)}}^{(Luciani)}$
...
\end{matrix}
\right.
$, ~ $j_{ab, \max} = e K_{ab}^{(V_\parallel)} \sqrt{\frac{T_i}{m_p}} \left( n_a + n_b \right)$, ~ $c_{flab} = 0.5$
...
c_{\eta _a,3} \frac{6.25 \cdot 10^{-2} T_e}{e|B|}\right)
...
\frac 1
{1 + \frac{
\frac{\sqrt g}{h_x} \widetilde{\eta}_{CLax}  \left| \frac{\partial V_{\parallel a}}{h_x \partial x} \right|
}
{V_{m_a}^{(max)}}
}, &
...
$\widetilde{\eta }_{CLax} = c_{\eta_{ax}^{(CL)}}^{Luciani} \eta_{CLax}$
...
$\chi_e = \left(
	c_{h_e,0} + c_{h_e,1} \frac{10^{20}}{n_e} + c_{h_e,3} \frac{6.25 \cdot 10^{-2} T_e}{e|B|}
\right) f_{h_e} (x,y)$,
...
\begin{equation*}
\widetilde{\sigma}_{CL \parallel} = c_{\kappa_{ex}^{(CL)}}^{(Luciani)} \sigma_{CL \parallel}
\end{equation*}
...
The classical thermo-electric coefficient $\alpha_{CLx}$ is given as:
$\alpha_{CLx} = - b_x^2 \frac{e}{m_e} \frac{\widetilde{c_e}^{(2)}}{c_e^{(1)}} n_e \tau_e$
when {\tt 'b2sigp\_style'} is set to {\tt '2'} (Zhdanov formulation, default), and
$\alpha_{CLx} = b_x^2 \frac{e}{m_e} \sqrt{\frac 5 2} f_{\alpha} \left(z_{eff} \right) n_e \tau _e$ otherwise (Balescu formulation), and
...
\begin{equation*}
\widetilde{\widetilde{\alpha}}_{CLx} = c_{\alpha_{ex}}^{(F\text{lim})} \widetilde{\alpha}_{CLx}
\end{equation*}
\begin{equation*}
\widetilde{\alpha}_{CLx} = c_{\kappa_{ex}^{(CL)}}^{(Luciani)} \alpha_{CLx}
\end{equation*}
\begin{equation*}
c_{\alpha_{ex}}^{(F\text{lim})} = \left\{
\begin{matrix}
1, & \text{inside the separatrix} \\
\frac{1}{1+\left|\frac{\alpha_{CLx} \frac{1}{h_x} \frac{\partial T_e}{\partial x}}{c_{flal} j_{e \max}}\right|}, & \text{outside the separatrix} \\
\end{matrix} \right.
\end{equation*}
\begin{equation*}
j_{e \max} = \left| b_x \right| e n_e \sqrt{\frac{T_e}{m_e}}, ~ c_{flal} = 0.5
\end{equation*}
If {\tt 'b2sigp\_style'} is set to {\tt '1'}:
\begin{equation*}
\widetilde{\widetilde{\alpha}}_{CLx} = c_{\kappa_{ex}}^{(F\text{lim})} \widetilde{\alpha}_{CLx}
\end{equation*}
\begin{equation*}
\widetilde{\alpha}_{CLx} = c_{\kappa_{ex}^{(CL)}}^{(Luciani)} \alpha_{CLx}
\end{equation*}
...
\alpha_{ANy} = \left(
	...
	c_{\alpha,3} \frac{6.25 \cdot	10^{-2} T_e}{e |B|}
...
{\left(
	1 + \left(
			...
			{\frac{\sqrt{g}}{h_x} \alpha V_{h_i} n_a T_i}
		\right)^{\gamma_2}
...
$\chi_a^{(AN)} = \left(
c_{h_a,0} + c_{h_a,1} \frac{10^{20}}{n_e} + c_{h_a,2} D_a^f + c_{h_a,3}\frac{6.25 \cdot 10^{-2} T_e}{e|B|}
\right) f_{h_a} (x,y)$,
...
$V_{h_i}^{(ax)} = c_{flim,1} \frac{\sqrt g}{h_x} |b_x| \sqrt{\frac{T_i}{m_p}}
...
{\left(
	1 + \left(
			...
			{\frac{\sqrt{g}}{h_y} \alpha V_{h_i} n_a T_i}
		\right)^{\gamma_2}
\right)^{\frac 1{\gamma_2}}}
, & z_a = 0 \\
\end{matrix} \right.
...
\sigma_{AN} = \left(
	...
	c_{\sigma_a,3} \frac{6.25 \cdot 10^{-2} T_e}{e|B|} +
...
+
\sum \limits_{a=0}^{ns-1} c_{rec,a,0} c_{nsr}
\max \left\{0, -\frac{\sqrt g}{h_y} \Gamma_{ay} \right\}
\right] \right|_{S_0}
\end{equation}

$p_{n_a,a,1} = -10^{-2}, \;\; c_{rec,0,0} = 0,\;\; c_{rec,1,0} = 1$
...
$p_{n_a,a,1}$ is the radial leakage velocity, in units of the local species sound species. Negative velocities are directed out of the plasma.
...
\begin{equation}
\begin{matrix}
\left. \frac{\sqrt g}{h_y} \widetilde{\Gamma_{ay}} \right|_{S_0}& =
& \left\{
\begin{matrix}
\left. \left[
	-\frac 1{p_{n_a,a,1}} \frac{\sqrt g}{h_y}  \left(
		D^n_{ANa} + D^p_{ANa} \left(z_a T_e + T_i \right)
	\right) n_a
...
\begin{align}
S^{no\_mdf}_{n_a,0} & = 0 \\
S^{no\_mdf}_{n_a,1} & = - \frac{1}{p_{n_a,a,1}} \frac{\sqrt{g}}{h_y} \left( D^n_{ANa} + D^p_{ANa} \left( z_a T_e + T_i \right) \right)
\end{align}
...
\begin{equation}
\left. -\frac{\sqrt g}{h_y^2} D_{ay}^n \frac{\partial n_a}{\partial y} \right|_{S_0}
=
\left. \left[
...
\begin{equation}
\begin{matrix}
\left. -\frac{\sqrt g}{h_y} \widetilde{\Gamma_{ay}} \right|_{N}& =
& \left\{
\begin{matrix}
\left. \left[
	\frac 1{p_{n_a,a,1}} \frac{\sqrt g}{h_y}  \left(
...
\begin{equation}
\left. \frac{\sqrt g}{h_y^2} D_{ay}^n \frac{\partial n_a}{\partial y} \right|_N
=
\left. \left[
c_{n_a,a,2} \frac{\sqrt g}{h_y} \sqrt{\frac{z_a T_e + T_i}{m_a}} n_a
\right] \right|_N
...
{\em Weakly imposing a velocity:} BCMOM = 4
...
{\em b2stbc\_spb style:} {\tt BCMOM = 14}
...
\frac{\oint B \sqrt{g} \, dx}{\oint \sqrt{g} \, dx}
$, the integration is done over inner magnetic surfaces

$v_{big} \approx 10^{12}$, $n_{e0} = 10^{20}$, $p_{m,a,1}$ is a given value of the average parallel velocity
...
{\em Imposing the b2stbc\_spb style boundary condition on the velocity:} {\tt BCMOM = 13}
...
{\em Imposing the b2stbc\_spb style boundary condition for parallel velocity:} {\tt BCMOM = 13}
...
\frac{\sqrt g}{h_x} e \left. \left[
	c_{ch,6} \sum \limits_{b=0}^{ns-1} z_a \left(
		c_{n,a,7} |b_x| \sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_b}{\sum \limits_{b=0}^{ns-1} m_b n_b}} n_a
	\right) +
	\left( 1 - \gamma_e \right) |b_x| n_e \frac 1{\sqrt{2\pi}}
	\sqrt{\frac{T_e}{m_e}} \exp \left( -\frac{e \Phi}{T_e} \right)
\right] \right|_W$, new form

$\left. \frac{\sqrt g}{h_x} j_x \right|_W =
\frac{\sqrt g}{h_x} e \left. \left[
	c_{ch,6} \sum \limits_{a=0}^{ns-1} z_a \Gamma_{ax} +
	c_{ch,7} |b_x| n_e \frac 1{\sqrt{2\pi}}
	\sqrt{\frac{T_e}{m_e}} \exp \left( -\frac{e \Phi}{T_e} \right)
\right] \right|_W$, old form
\end{small}
...
{\em Imposing the b2stbc\_spb style boundary condition:} {\tt BCENE = 11}
...
$\left. -\frac{\sqrt g}{h_y} k_{ey} \frac{\partial T_e}{h_y \partial y} \right|_{S_0} = \left. \left[ \left( p_{h_e,1} \frac{\sqrt g}{h_y}\sqrt{\frac{T_e}{m_e}} \right) T_e n_e \right] \right|_{S_0}$
...
$\left. \frac{\sqrt g}{h_y} k_{ey} \frac{\partial T_e}{h_y \partial y} \right|_N = \left. \left[ \left( p_{h_e,1} \frac{\sqrt g}{h_y}\sqrt{\frac{T_e}{m_e}} \right) T_e n_e \right] \right|_N$
...
{\em Imposing the b2stbc\_spb style boundary condition for electron temperature:} {\tt BCENE = 15}

If bcene\_15\_style.eq.0:
...
$\left. \frac{\sqrt g}{h_x} \widetilde q_{ex} \right|_W = \left. \left[
	-\left(
		\frac{1 + \gamma_e}{1 -\gamma_e} + \frac{e\Phi}{T_e}
	\right)
	\max \left(
		0,\frac{\sqrt g}{h_x} |b_x| n_e
		\sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_a}{\sum \limits_{a=0}^{ns-1} m_a n_a}}
...
$\left. -\frac{\sqrt g}{h_x} \widetilde q_{ex} \right|_E = \left. \left[
	...
	\max \left(
		0,\frac{\sqrt g}{h_x} |b_x| n_e
		\sqrt{\frac{\sum \limits_{b=0}^{ns-1} p_b}{\sum \limits_{a=0}^{ns-1} m_b n_b}}
		-
		\frac{\sqrt g}{h_x} b_x \frac{j_{\parallel}}e
	\right) T_e
	+
...
$c_{rec,0,0} = 0$ and $c_{rec,1,0} = 1$ are particle recycling coefficients.
...
p_{h_i,1} \max \left\{0, -\frac{\sqrt g}{h_y} \widetilde f_y^{(E)} \right\} T_i
\right|_{S_0}$
...
$\left. -\frac{\sqrt g}{h_y} k_{iy} \frac{\partial T_i}{h_y \partial y} \right|_{S_0}
= \left.
...
{\em b2stbc\_spb boundary condition:}
...
{\em Boundary condition from b2stbc\_spb:} {\tt BCENI = 21}
...
{\em Boundary condition in b2stbc\_spb:}
...
{\em Boundary condition from b2stbc\_spb:} {\tt BCENI = 15}
...
{\em Boundary condition from b2stbc\_spb:}
...
then $e_{rec,0} = 0$, $e_{rec,1} = 0.3$, $c_{rec,0,0} = 0$, $c_{rec,1,0} = 1$

else $c_{rec,0,0} = 0$, $c_{rec,1,0} = 1$, $c_{rec,0,1} = 0$, $c_{rec,1,1} = 0.3$
...
{\em Imposing a b2stbc\_spb style boundary condition:} {\tt BCENI = 15}
...
\begin{center}\index{b2plot, plasma quantities}
  \begin{longtable}{|l|l|}
    ...
    b2espcr  & \index{b2plot, b2espcr} {\tt Eirene} species index \\
...
also all require an integer argument $<$i$>$,
...
\begin{center}\index{b2plot, stack commands}
  \begin{longtable}{|c|c|c|c|c|c|c|c|c|l|}
    ...
    nspe \index{b2plot, nspe}, nspec \index{b2plot, nspec} & 3 & 0 & 0 & 0 & 0 & 0 & 0 & \ref{sec:nspe}  & New spectrometer \\
...
$<$ig$>$ = JKLM. If M \textgreater 0, then the file {\tt radsrc.b2e} is produced,
...
\subsubsection{$<$is$_{start}$,{\bf I}$>$ $<$is$_{end}$,{\bf I}$>$ esumz}\index{b2plot, esumz}\label{sec:esumz}
Sums the Eirene data over the $z$-direction from $<$is$_{start}$$>$ to $<$is$_{end}$$>$, and then copies the
sum to the first $z$ position. If $<$is$_{start}$$>$=$<$is$_{end}$$>$=0, then the sum is done
over all species.
...
\subsubsection{$<$is$_{start}$,{\bf I}$>$ $<$is$_{end}$,{\bf I}$>$ ezrng}\index{b2plot, ezrng}\label{sec:ezrng}
Selects the subset over species extended from species index $<$is$_{start}$$>$ to
species index $<$is$_{end}$$>$ from the Eirene data.
...
Propagates the Eirene data value from element $<$i$>$ to all other elements.
...
Propagates the Eirene data value in the specified position $<$is$>$ to all other $z$
(species) values.
...
Adds $<$number$>$ to the array on the top of the Eirene stack.

\subsubsection{$<$number,{\bf R}$>$ rem-}\index{b2plot, rem-}\label{sec:rem-}
Substracts $<$number$>$ from the array on the top of the Eirene stack.

\subsubsection{$<$number,{\bf R}$>$ rem*}\index{b2plot, rem*}\label{sec:rem*}
Multiplies the array on the top of the Eirene stack by $<$number$>$.

\subsubsection{$<$number,{\bf R}$>$ rem/}\index{b2plot, rem/}\label{sec:rem/}
Divides the array on the top of the Eirene stack by $<$number$>$. Division by
zero will yield an error and do nothing.

\subsubsection{$<$number,{\bf R}$>$ rem**}\index{b2plot, rem**}\label{sec:rem**}
Raises each element of the array on the top of the Eirene stack to the $<$number$>$-th power. If $<$number$>$=0, the result will be 1.0 for all cells.

\subsubsection{$<$number,{\bf R}$>$ remmi}\index{b2plot, remmi}\label{sec:remmi}
Minimum of an Eirene array and $<$number$>$.

\subsubsection{$<$number,{\bf R}$>$ remma}\index{b2plot, remma}\label{sec:remma}
Maximum of an Eirene array and $<$number$>$.
...
If an Eirene array element is equal to $<$number$>$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emne}\index{b2plot, emne}\label{sec:emne}
If an Eirene array element is not equal to $<$number$>$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emge}\index{b2plot, emge}\label{sec:emge}
If an Eirene array element is greater than or equal to $<$number$>$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emgt}\index{b2plot, emgt}\label{sec:emgt}
If an Eirene array element is greater than $<$number$>$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emle}\index{b2plot, emle}\label{sec:emle}
If an Eirene array element is less than or equal to $<$number$>$, sets this element to 1.0.
Otherwise, sets it to zero.

\subsubsection{$<$number,{\bf R}$>$ emlt}\index{b2plot, emlt}\label{sec:emlt}
If an Eirene array element is less than $<$number$>$, sets this element to 1.0.
Otherwise, sets it to zero.
...
maximum arrow length of $<$length$>$ meters. The use of {\em vxa} and
...
Sets colour of overlaid mesh and triangular grid to $<$ncol$>$ (default 1).
See {\bf zcol} above for a list of available colours.
...
\subsubsection{$<$level$_{min}$,{\bf R}$>$ $<$level$_{max}$,{\bf R}$>$ $<$delta,{\bf R}$>$ eclvd}\index{b2plot, eclvd}\label{sec:eclvd}
Starts contour levels at $<$level$_{min}$$>$ increasing by $<$delta$>$ up
to $<$level$_{max}$$>$ for Eirene data contour plot.

\subsubsection{$<$level$_{min}$,{\bf R}$>$ $<$level$_{max}$,{\bf R}$>$ $<$n,{\bf I}$>$ eclvn}\index{b2plot, eclvn}\label{sec:eclvn}
Produces $<$n$>$ contour levels going from $<$level$_{min}$$>$ to $<$level$_{max}$$>$ for Eirene data contour plot.

\subsubsection{$<$n,{\bf I}$>$ $<$level$_1$,{\bf R}$>$ ... $<$level$_n$,{\bf R}$>$ eclvs}\index{b2plot, eclvs}\label{sec:eclvs}
Sets $<$n$>$ contour levels to the values specified by $<$label$_i$$>$ for Eirene data contour plots.
...
Assigns real value $<$value$>$ to all triangles from the top of the Eirene data stack that do not map to a {\tt B2.5} grid cell.
...
Writes out the top of the Eirene stack to $<$filename$>$ (in the \SOLPSfour environment)
or (in the \SOLPSfive environment) \verb:b2pl.exe.dir/:$<$filename$>$.
...
Writes out data from element ($<$ix$>$,$<$iy$>$) from the top of the matrix stack to $<$filename$>$ (in the \SOLPSfour environment)
...
Writes out data from triangle element $<$i$>$ from the top of the matrix stack to $<$filename$>$ (in the \SOLPSfour environment)
or \verb:b2pl.exe.dir/:$<$filename$>$ (in the \SOLPSfive environment).

\subsubsection{$<$i,{\bf I}$>$ $<$iz,{\bf I}$>$ $<$filename,{\bf S}$>$ outptrg}\index{b2plot, outptrg}\label{sec:outptrg}
Writes out data from element ($<$i$>$,$<$iz$>$) from the top of the target stack to $<$filename$>$ (in the \SOLPSfour environment)
or \verb:b2pl.exe.dir/:$<$filename$>$ (in the \SOLPSfive environment).

\subsubsection{$<$i,{\bf I}$>$ $<$filename,{\bf S}$>$ outpw}\index{b2plot, outpw}\label{sec:outpw}
Writes out data from element $<$i$>$ from the top of the wall stack to $<$filename$>$ (in the \SOLPSfour environment)
...
or \${\tt SOLPSTOP/data.local/adas}, for species index $<$is$>$, centered around wavelength $<$lambda$>$ (in {\em \AA}),
...
Sets value of {\tt PLPLOT} device name to $<$device name$>$.

\subsubsection{$<$winid,{\bf I}$>$ swid}\index{b2plot, swid}\label{sec:swid}
Sets value of {\tt WIN\_ID} environment variable (part of {\tt TCL/TK} interface) to $<$winid$>$.
...
Optional user routine. The choice of user routine is governed by $<$ig$>$.
...
\begin{description}
  ...
  \item[0 $\le$ ig $<$ nx] Produces a file {\tt oleg.dat} with plasma parameters (density, temperature, parallel flow velocity, parallel heat flux) against the distance along the magnetic field along flux surface index $<$ig$>$
  \item[nx $\le$ ig $<$ 1000] Produces two files with estimated helium concentration ({\tt fraction.he}) and neutral pressure ({\tt pressure.ntr}) in the PFR for summary plots
  \item[ig $\ge$ 1000] Adds a line to the file {\tt ../sum\_tmp/spec\_pwr.dat} with the power entering both divertors, power radiated along the divertor legs, power radiated near the targets, areas of the separatrix branches and mean specific power radiated along the divertor legs per square meter. $<$ig$>$-1000 cells are treated as "close to the targets"
\end{description}
...
Computes a reaction rate as per the {\tt ADAS} specification. The $<$reac$>$ string should be something of the
