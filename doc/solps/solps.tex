...
\begin{description}
\item[Fortran 90/95 Compiler] the main programs are written in {\tt FORTRAN 90}; some routines still require a "-r8" option
...
and an easier-to-read font.\index{ncar.cfg} The files can be converted into a PostScript file by means of the command:
\begin{verbatim}
...
If the user wants to use the heat transfer model for the monoblock or the reflector plates, then he must define in the DG model the inner/outer target and reflector plate plot zones using the label 'trg[i-o]' and '[i-o]rp' respectively. The {\tt wlld\_*.dat} produced should then be postprocessed by b2ymb (see \ref{sec:b2ymb}) or b2yrp (\ref{sec:b2yrp}).
