...
When using Git, checking that your local version is current is trivially achieved using the \verb!git status! command.
...
may be shared amongst multiple users on the same machines, so as not to duplicate large files and to
...
\$\verb!{SOLPSTOP}/lib/!\$\verb!{OBJECTCODE}! directory.
...
These environment variables are found in files config.\$\verb!HOSTNAME!.\$\verb!COMPILER!.
...
\begin{description}\index{Hostnames}
  ...
  \item[IPP] IPP-Garching domain: *.ipp.mpg.de, *.rzg.mpg.de
  \item[IPP-HGW] IPP-Greifswald domain: *.ipp-hgw.mpg.de
  \item[JET] JET domain: *.efda.org
  \item[ITM] ITM Gateway machine: *.efda-itm.org
...
\begin{description}\index{Platforms}
  \item[linux\_pgf90] Portland Group 32-bit Fortran compiler in use at JET and GA
  \item[linux.ifort64] Intel 64-bit Fortran compiler in use at ITER and IPP
\end{description}
...
When establishing the \SOLPSITER environment, the code
will check for the presence of the NAG and NCAR libraries and adjust compiler options
accordingly. If the NAG library is not available (it requires a pay licence), then the BLAS and
LAPACK packages will be used instead, and automatically included in the build from the \SOLPSITER 
Makefile.

Once the config files have been filled in with the appropriate information for the
...
\begin{description}\index{Compiling the code}\index{B2.5, compiling}\index{SOLPS-ITER, compiling}\index{Eirene, compiling}\index{DivGeo, compiling}\index{Carre, compiling}
  \item[solps] (Default) Compiles the necessary components for a full \SOLPSITER implementation, including:
    ...
    \end{description}
  \item[solps\_mpi] Compiles the necessary components for a full \SOLPSITER implementation, this time using MPI parallelization, including:
    \begin{description}
       \item[libs] Installs the needed libraries according to config file
       \item[carre] The \verb!Carre! grid generator and associated translators
       \item[divgeo] The \verb!DivGeo! graphical input file builder and accompanying equilibrium and template converters
       \item[b25eirene\_mpi] The \verb!B2.5-Eirene! main code and accompanying programs, MPI-parallelized
       \item[uinp] The \verb!Uinp! translator from \verb!DivGeo! output to \verb!B2.5-Eirene! input
       \item[triang] The \verb!Tria! and \verb!Triageom! triangulation utilities
       \item[manual] Builds this manual
    \end{description}
  \item[all] The same as {\bf solps} above, but in addition:
    ...
    \end{description}
  \item[all\_mpi] The same as {\bf solps\_mpi} above, but in addition:
    \begin{description}
      \item[b25\_mpi] The \verb!B2.5! stand-alone plasma fluid solver, with MPI parallelization enabled
      \item[eirene\_mpi] The \verb!Eirene! stand-alone Monte-Carlo neutral transport code, with MPI parallelization enabled
    \end{description}
  ...
  \item[b25\_mpi] The \verb!B2.5! stand-alone plasma fluid solver and accompanying programs only, with MPI parallelization enabled
  \item[eirene] The \verb!Eirene! stand-alone Monte-Carlo neutral transport code only
  \item[eirene\_mpi] The \verb!Eirene! stand-alone Monte-Carlo neutral transport code only, with MPI parallelization enabled
  \item[b25eirene] The \verb!B2.5-Eirene! main code and accompanying programs only
  \item[b25eirene\_mpi] The \verb!B2.5-Eirene! main code and accompanying programs only, MPI-parallelized
  ...
  \item[clean\_solps] Removes the compiled binary files for {\bf solps} build
  \item[clean\_solps\_mpi] Removes the compiled binary files for {\bf solps\_mpi} build
  \item[clean\_all] Removes all the compiled binary files
  \item[clean\_all\_mpi] Removes all the MPI-compiled binary files
  \item[clean\_carre] Removes the compiled binary files for {\bf carre} build
  \item[clean\_divgeo] Removes the compiled binary files for {\bf divgeo} build
  \item[clean\_b25] Removes the compiled binary files for {\bf b25} build
  \item[clean\_b25\_mpi] Removes the compiled binary files for {\bf b25\_mpi} build
  \item[clean\_eirene] Removes the compiled binary files for {\bf eirene} build
  \item[clean\_eirene\_mpi] Removes the compiled binary files for {\bf eirene\_mpi} build
  \item[clean\_b25eirene] Removes the compiled binary files for {\bf b25eirene} build
  \item[clean\_b25eirene\_mpi] Removes the compiled binary files for {\bf b25eirene\_mpi} build
  \item[clean\_uinp] Removes the compiled binary files for {\bf uinp} build
  \item[clean\_triang] Removes the compiled binary files for {\bf triang} build
\end{description}

The master \SOLPSITER \verb!Makefile! will actually start by compiling and installing all the included libraries by invoking the \verb!install.sh! script from the \$\verb!{SOLPSTOP}/lib/! directory. If the user wishes to re-install the libraries with a clean of any previously existing compiled files, the \verb!clean_install.sh! script is available for this purpose in the same location. These scripts require the user
...
smooth code operation. Ths is achieved by the command:

\index{setup.csh}
\begin{verbatim}
source setup.csh
\end{verbatim}
if one is using the C-shell or the TC-shell. If one prefers to use the K-shell, then the command is:
\index{setup.ksh}
\begin{verbatim}
source setup.ksh
\end{verbatim}
...
\begin{description}
  ...
  \item[sb2, sbb] Go to \$\verb!{SOLPSTOP}/src/B2.5!, the \verb!B2.5! top directory
  \item[sei] Goes to \$\verb!{SOLPSTOP}/src/Eirene!, the \verb!Eirene! top directory
  \item[ssd] Goes to \$\verb!{SOLPSTOP}/src/DivGeo!, the \verb!DivGeo! top directory
  \item[ssc] Goes to \$\verb!{SOLPSTOP}/src/Carre!, the \verb!Carre! top directory
  \item[sst] Goes to \$\verb!{SOLPSTOP}/src/Triang!, the \verb!Tria! top directory
  \item[ssu] Goes to \$\verb!{SOLPSTOP}/src/Uinp!, the \verb!Uinp! top directory
  \item[sbin] Goes to \$\verb!{SOLPSTOP}/bin/!\$\verb!{OBJECTCODE}!, the directory where all compiled binaries are stored
  \item[slib] Goes to \$\verb!{SOLPSTOP}/lib/!\$\verb!{OBJECTCODE}!, the directory where libraries are stored
  ...
  \item[sdg] Goes to \$\verb!{SOLPSTOP}/data/DivGeo/class/!\$\verb!{DEVICE}!, the directory where the \verb!DivGeo! models for the current modelled device are stored
  \item[\$DG] The location of the \verb!DivGeo! top directory
\end{description}

It is also possible to toggle between versions of the code compiled with and without debug options (array bounds checking, traceback, etc...) by use of the \verb!set_debug! and \verb!unset_debug! commands.
