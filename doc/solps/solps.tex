...
\begin{description}\index{Hostnames}
  ...
  \item[DIIID] General Atomics: *.fusion.gat.com
  \item[EAST] Academica Sinica, Hefei: *.ipp.cas.cn
...
\begin{description}\index{Platforms}
  \item[pgf90] Portland Group 32-bit Fortran compiler in use at JET and GA
  \item[ifort64] Intel 64-bit Fortran compiler in use at ITER and IPP
  \item[gfortran] The gfortran compiler in use at College of William and Mary
\end{description}
...
Adding the \verb!debug_! prefix in front of any of build targets (except \verb!clean*!) will compile
them in debug mode.
...
The number of scripts developped for various {\bf SOLPS} versions is very large. Efforts are currently underway to translate all the relevant ones for use within \SOLPSITERns. A non-exhaustive listing of these scripts, with some explanation as to their purpose, follows:
\begin{itemize}
...
%%find $SOLPSTOP/modules/B2.5/src -follow -maxdepth 1 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
%%find $SOLPSTOP/modules/Eirene -follow -maxdepth 2 -type d -print | sed -e "s:${SOLPSTOP}:SOLPSTOP:" | sed -e 's:[^/]*/:|   :g' | sed 's:\(\(|   \)*\)|   :\1+---:'
...
relies on to coompute things such as backscattering coefficients, atomic reactions, etc...
...
\$\verb!SOLPSTOP/modules/DivGeo! is a link (for SOLPS4.x versions).  If it is, do
\index{gtdr, DivGeo}
\begin{verbatim}
  cd $SOLPSTOP/modules
  gtdr DivGeo
...
When using for the first time, check to see whether
\$\verb!SOLPSTOP/modules/Carre! (accessible with the \verb!ssc! command)
...
When using for the first time, check to see whether
\$\verb!SOLPSTOP/modules/agg! is a link.  If it is, do
\index{gtdr, agg}
\begin{verbatim}
  cd $SOLPSTOP/modules
  gtdr agg
...
%%grep #if $SOLPSTOP/modules/b2/current/*/*.F | awk '{print $2}' | sort | uniq
...
%%grep #if $SOLPSTOP/modules/b2/current/*/*.F | awk '{print $2,$1}' | sort | uniq
...
%%grep #if $SOLPSTOP/modules/Eirene/current/*/*.F | awk '{print $2}' | sort | uniq
...
%%grep #if $SOLPSTOP/modules/Eirene/current/*/*.F | awk '{print $2,$1}' | sort | uniq
