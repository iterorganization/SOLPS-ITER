...
Additional multidimensional writes to b2time.nc can be made using the b2mwti\_2dwrite switch.
If set to 1 then a few 2D arrays (ne, Te, Ti) are output with each write to b2time.nc. If set
...
\begin{center}
    ...
    po2d \index{po2d}\index{b2time.nc, po2d} & potential & $V$ \\
    kin2d \index{kin2d}\index{b2time.nc, kin2d} & parallel kinetic energy & $J$ \\
    rsahi2d \index{rsahi2d}\index{b2time.nc, rsahi2d} & iz energy source/sink & $W$ \\
    rsana2d \index{rsana2d}\index{b2time.nc, rsana2d} & iz rate & $s^{-1}$ \\        
    rrahi2d \index{rrahi2d}\index{b2time.nc, rrahi2d} & rc energy source/sink & $W$ \\
    rrana2d \index{rrana2d}\index{b2time.nc, rrana2d} & rc rate & $s^{-1}$ \\        
    rcxhi2d \index{rcxhi2d}\index{b2time.nc, rcxhi2d} & cx energy source/sink & $W$ \\
    rcxna2d \index{rcxna2d}\index{b2time.nc, rcxna2d} & cx rate & $s^{-1}$ \\        
    ...
    fch2d \index{fch2d}\index{b2time.nc, fch2d} & Current & $A$ \\
    fna2d \index{fna2d}\index{b2time.nc, fna2d} & Particle flux & $s^{-1}$ \\            
    \hline
    \caption{Multidimensional time dependent quantities stored in b2time.nc}
    \label{tab:5db2time}
  \end{longtable}
\end{center}
