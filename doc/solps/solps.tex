...
when appropriate. Other {\tt B2.5} input files are treated in the same fashion. The
...
\begin{enumerate}\index{res\_av}
  ...
  \item{An averaging phase}, turned on by means of the {\tt "b2mndt\_av"} switch in the {\tt b2mn.dat} input file, usually with a smaller number of {\tt Eirene} trajectories, during which batch averaging will be done to obtain an averaged steady-state solution
  \item{A finalization phase} where the averaged steady-state solution is used for a small number of timesteps, and a larger number of {\tt Eirene} trajectories, to reduce statistical noise. This is done by running the {\tt res\_av} script.
\end{enumerate}
...
outboard top ({\tt tr}) divertors. These are listed below.
\begin{center}
    \begin{longtable}{|l|l|l|}
    \hline
    \multicolumn{3}{|l|}{\small\sl continued from previous page} \\
...
to ``2'' then additional quantities such as fluxes are added, see the table below.
\begin{center}
    \begin{longtable}{|l|l|l|}
    \hline
    \multicolumn{3}{|l|}{\small\sl continued from previous page} \\
...
contain some averaged data, listed in the table below.
