...
{\tt set\_mpi} and {\tt unset\_mpi} will turn on or off, respectively, compilation with MPI parallelization flags. By default, neither debug nor MPI flags are turned on.
...
\item Others
  ...
  \item[b2run]\index{b2run}\index{b2co}\index{b2fstate} Shortcut to the {\tt Makefile} launching {\tt B2.5}. A "-n" option allows for dry-run tests. A "-d" option allows to run within the chosen debugger. A "-s" option allows to run {\tt B2.5} in stand-alone mode. A "-m" option allows to run with MPI options. A "-o" option adds any other desired run options. The options can be grouped together inside quotes or given individually with multiple "-o" instances. A "-b" option can be used when invoking the {\tt b2co} program to obtain a provisional {\tt b2fstate} file while the main code is running. A "-h" option prints a help description. (See \ref{b2run} for fuller details)
  ...
  \item[ippsubmit]\index{ippsubmit} Command file to launch runs on the IPP cluster. See Section \ref{b2run} for details.
...
are provided:
\begin{description}
  \item{ippsubmit}\index{ippsubmit} for submissions on the IPP cluster
...
with the needed modifications to the \${\tt \{SOLPSTOP\}/runs/QSUB.*} actual submission scripts.
Each submission command file can handle five possible cases:
\begin{description}\index{QSUB files}
  \item{QSUB.*} Runs a stand-alone {\tt B2.5} case.
  \item{QSUB.*-gz} Runs a stand-alone {\tt B2.5} case where the output log file has been compressed (activated with the "-z" option).
  \item{QSUB.*\_coupled} Runs a coupled \SOLPSITER case.
  \item{QSUB.*\_coupled-gz} Runs a coupled \SOLPSITER case where the output log file has been compressed (activated with the "-z" option).
...
If {\tt <<Submission\_file>>} is not specified, {\tt itersubmit} looks for {\tt QSUB.iter\_coupled} or
for {\tt QSUB.iter} depending on whether a coupled or a standalone run is required. To request a stand-alone run explicitly, the user can use the "-s" option. Otherwise, the script will assume a coupled run is requested if an {\tt Eirene} {\tt input.dat} file is present in the run directory, and a stand-alone run otherwise.
...
\end{verbatim}
command. Standalone jobs are requested with the "-s" option.
...
itersubmit -M 8GB
\end{verbatim}
command, to obtain here a run using 8GB of memory. The "B" (for "bytes") is necessary in this syntax for SLURM submission, but should be omitted when using QSUB submission.

A listing of the available options for each submission script can be obtained by the
\begin{verbatim}
itersubmit -h
\end{verbatim}
command. All options (except "-h") can be used simultaneously and in any order, provided the name of the (optional) submission file is given last.

The {\tt QSUB.iter} script first of all changes directory to \${\tt \{SOLPSTOP\}}
...
Then the {\tt QSUB.iter} script returns back to the current run directory
