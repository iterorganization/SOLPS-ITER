...
\item Others
  ...
  \item[solps-iter\_update]\index{solps-iter\_update} Script to update to the latest \SOLPSITER distribution.
...
= S_{a \parallel}^m + S_{CF_a}^m + S_{fr_a}^m &+ S_{Therm_a}^{m} + S_{I_a}^m + S_{R_a}^m + S_{CX_a}^m
\end{align}
...
$S_a^m = S_{fr,a}^m + S_{Therm}^m + S_{I_a}^m + S_{R_a}^m + S_{CX_a}^m$,
...
\right] \right|_W$, if {\tt 'b2stbc\_istyle\_fchi'} is {\tt '1'}, or
...
\frac{\sqrt g}{h_x} e \left. \left[
	...
	\left( 1 - \gamma_e \right) |b_x| n_e \frac 1{\sqrt{2\pi}}
	\sqrt{\frac{T_e}{m_e}} \exp \left( -\frac{e \left( \Phi - p_{\Phi,2} \right)}{T_e} \right)
\right] \right|_W$, if {\tt 'b2stbc\_istyle\_fchi'} is {\tt '0'}.
\end{small}
...
\frac{\sqrt g}{h_x} e \left. \left[
	...
	\left( 1 - \gamma_e \right) |b_x| n_e \frac 1{\sqrt{2\pi}}
	\sqrt{\frac{T_e}{m_e}} \exp \left( -\frac{e \left( \Phi - p_{\Phi,2} \right)}{T_e} \right)
\right] \right|_E$, if {\tt 'b2stbc\_istyle\_fchi'} is {\tt '1'}, or
...
\frac{\sqrt g}{h_x} e \left. \left[
	...
	\left( 1 - \gamma_e \right) |b_x| n_e \frac 1{\sqrt{2\pi}}
	\sqrt{\frac{T_e}{m_e}} \exp \left( -\frac{e \left( \Phi -p_{\Phi,2} \right)}{T_e} \right)
\right] \right|_E$, if {\tt 'b2stbc\_istyle\_fchi'} is {\tt '0'}.
\end{small}
...
$\left. \frac{\sqrt g}{h_x} \widetilde q_{ex} \right|_W = \left. \left[
	...
	\right)
	\max \left(
		0, p_{n,a,1} \frac{\sqrt g}{h_x} |b_x| n_e
...
$\left. -\frac{\sqrt g}{h_x} \widetilde q_{ex} \right|_E = \left. \left[
	...
	\max \left(
		0, p_{n,a,1} \frac{\sqrt g}{h_x} |b_x| n_e
		\sqrt{\frac{\sum \limits_{b=0}^{ns-1} p_b}{\sum \limits_{b=0}^{ns-1} m_b n_b}}
		-
...
{\em Boundary condition from {\tt b2stbc\_spb}:} {\tt BCENI = 15}

$\left. \frac{\sqrt g}{h_x} \widetilde q_{ix} \right|_W = \left.
- p_{h_i,1} \frac{\sqrt g}{h_x} |b_x|
\sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_a}{\sum \limits_{a=0}^{ns-1} m_a n_a}} T_i
\sum \limits^{ns-1}_{\substack{a = 0, \\ z_a \neq 0}} n_a
-
...
{\em Imposing a {\tt b2stbc\_spb} style boundary condition:} {\tt BCENI = 15}
...
These are the tallies stored in {\tt b2time.nc}.
...
\item[BLOCK] \index{BLOCK} Can be used to implement blocking instructions
