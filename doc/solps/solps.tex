...
To use the \SOLPSITER code suite, the user will need, locally installed:
...
link to it should be put in the \$\verb!{SOLPSTOP}/lib/!\$\verb!{OBJECTCODE}! directory,
under the name \verb!libnag.a!. If the library is not available, then the BLAS and
LAPACK packages will be used instead. These latter libraries
are publicly available. The source code is distributed with \SOLPSITER and can be found in the \$\verb!{SOLPSTOP}/lib/blaslapack! directory. The BLASLAPACK library will be automatically built at compile time by the \SOLPSITER master \verb!Makefile!. 
...
The master \SOLPSITER \verb!Makefile! will first compile and install all the included libraries by invoking the \verb!install.sh! script from the \$\verb!{SOLPSTOP}/lib/! directory. If the user wishes to re-install the libraries with a clean of any previously existing compiled files, the \verb!clean_install.sh! script is available for this purpose in the same location. These scripts require the user
...
Some Windows implementations have been attempted, and require an additional \verb!-DWINDOWS! compiler switch, but are not currently being supported.
