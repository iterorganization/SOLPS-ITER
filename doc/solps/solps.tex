...
\item K.~and A.S.~Kukushkin, the {\tt DG} manual, with revisions by A.~Chin and D.~Stotler
...
\begin{description}\index{Compiling the code}\index{B2.5, compiling}\index{SOLPS-ITER, compiling}\index{Eirene, compiling}\index{DivGeo, compiling}\index{Carre, compiling}
  \item[solps] (Default) Compiles the necessary components for a full \SOLPSITER implementation, including:
    \begin{description}
      ...
      \item[ggd] The Generalized Grid Description ({\tt GGD}) package from IMAS
...
{\tt modules/ggd} for the Generalized Grid Description utility library,
...
\subsection{modules/ggd}
\index{ggd}\index{GGD}\index{IDS}

The {\tt modules/ggd} subdirectory contains the Generalized Grid Description utility routines which are needed for use of \SOLPSITER with IMAS and the production, archival and retrieval of code runs in Interface Data Structure (IDS) format.
...
negative quantities on the same scale and "M" the absolute values of negative quantities in a separate plot)
  ...
  \item[fluxt \#1]\index{fluxt} Gives the recycling fluxes for the first \#1 {\tt Eirene} strata. The default value is all the strata, as read from the {\tt b2.neutrals.parameters} file, or if not present, the \${\tt SOLPSTOP/modules/B2.5/src\allowbreak/include.local/DIMENSIONS.F} file, or if not present, the \${\tt SOLPSTOP/modules/B2.5/src\allowbreak/include/DIMENSIONS.F} file.
...
\item Others
  ...
  \item[b2run]\index{b2run} Shortcut to the {\tt Makefile} launching {\tt B2.5}. A "-n" option allows for dry-run tests. A "-d" option allows to run within the chosen debugger. A "-s" option allows to run {\tt B2.5} in stand-alone mode. A "-m" option allows to run with MPI options. A "-h" option prints a help description. (See \ref{b2run} for fuller details)
  ...
  \item[recover\_initial\_files]\index{recover\_initial\_files} For use to copy back the stored initial case files ({\tt .i} extension) before a run restart.
...
\begin{description}
  ...
  \item[move\_optional\_files]\index{move\_optional\_files}
  \item[move\_save\_files]\index{move\_save\_files}
...
\index{b2fgmtry}\label{sec:b2ag}\index{b2ag}\index{fort.30}
...
\index{b2fgmtry}\label{sec:b2yg}\index{b2yg}
...
\index{b2fpardf}\index{b2fparam}\label{sec:b2yh}\index{b2yh}\index{b2ah}
...
\index{b2yi}\index{b2yp}\index{b2ai}\index{b2mn}
...
\index{b2frates}\index{b2yr.dat}\label{sec:b2yr}\index{b2yr}
...
\index{b2fmovie}\label{sec:b2ym}\index{b2ym}
...
\index{b2ftrack}\label{sec:b2yn}\index{b2yn}
...
\index{b2ftrace}\label{sec:b2yq}\index{b2yq}
...
\label{sec:b2co}\index{b2co}\index{plasmastate}\index{B2.5, run recovery}\index{b2fstate}\index{recover\_initial\_files}
...
This procedure will re-start the run from the point at which the {\tt plasmastate} file was written. In cases with feedback schemes active, the feedback values will be grabbed from the relevant {\tt b2.*\_save.parameters.XXXX} files and a new set of
{\tt b2.*\_save.parameters} files will be created.

It is also possible to use the {\tt b2co} utility {\em while} {\tt SOLPS} is running, in order to obtain a {\tt b2fstate} file in the run directory that one may use to plot certain plasma quantities and monitor the current status of the run.

\newpage
\section{b2uf \& b2fu, converting the plasma output file}
\label{sec:b2uf}\label{sec:b2fu}\index{b2uf}\index{b2fu}\index{b2fplasma}\index{b2fplasmf}
...
\label{sec:b2ye}\index{b2ye}\index{error assessment}\index{b2time.nc}\index{b2favere}
...
\left(\frac{h_z\sqrt g}{h_x} \frac 4 3 \widetilde{\widetilde{\eta}}_{CLax}
...
\frac 4 3 \widetilde{\widetilde{\eta}}_{CLax} \frac{\partial \ln h_z}{h_x \partial x} V_{\parallel a} -
...
$\eta_{ax} = \eta_{ANa} + \frac{4}{3} \widetilde{\widetilde{\eta}}_{CLax}$
...
+ \sum \limits_{fluid~species} \left(
	\left(
                \frac{4}{3} \widetilde{\widetilde{\eta}}_{CLax} + \eta_{ANa}
        \right)
        \left(
...
\begin{equation*}
\widetilde f_{ix} = \frac 3 2 \Gamma_{ix} + V_{ix}^{(str)} n_i + 0 \cdot \frac{3}{2} \left( \frac{j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} + \widetilde{j_x}^{\left( s \right)}}{e} \right)
\end{equation*}
...
q_{ix} =& \left[
	...
	V_{ix}^{(str)} n_i +
        0 \cdot \frac{3}{2} \left( \frac{j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} + \widetilde{j_x}^{\left( s \right)}}{e} \right)
\right] T_e \\
...
\widetilde{\widetilde{\eta}}_{CLax}
...
Please check in subroutine {\tt b2tallies.F}!
...
\begin{center}\index{b2plot, additional commands}
  \begin{longtable}{|c|l|}
    ...
    tranprof \index{b2plot, tranprof}    & Create {\tt b2.transport.parameters} file \\
...
IMAS & ids/ggd\_access.F90 \\
IMAS & ids/ggd\_data.F90 \\
IMAS & ids/ggd\_object.F90 \\
IMAS & ids/ggd\_objectlist.F90 \\
IMAS & ids/ggd\_structured.F90 \\
IMAS & ids/ggd\_subgrid.F90 \\
IMAS & ids/ggd\_transform.F90 \\
IMAS & ids/ggd.F90 \\
...
ITM & ids/ggd\_access.F90 \\
ITM & ids/ggd\_data.F90 \\
ITM & ids/ggd\_object.F90 \\
ITM & ids/ggd\_objectlist.F90 \\
ITM & ids/ggd\_structured.F90 \\
ITM & ids/ggd\_subgrid.F90 \\
ITM & ids/ggd\_transform.F90 \\
ITM & ids/ggd.F90 \\
