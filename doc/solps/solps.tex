...
\item[BCCON]\index{BCCON} {\tt integer array, length NS * NBC}, specifying
  ...
  \item[10] leakage option for density, {\tt CONPAR(,,1)} specifies
  ...
  \item[15] prescribe the decay length for the density with drifts switched on,
  {\tt CONPAR(,,1)} specifies the gradient length in $m$
...
\item[BCENE]\index{BCENE} {\tt integer array, length NBC}, specifying
  ...
  \item[22] modified decay length for the electron temperature
  \item[23] Prescribe the poloidally averaged value of the ion temperature and introduce a poloidal variation as close as possible to the neoclassical solution. It is recommended to use this boundary condition together with a corresponding condition on the ion density ({\tt BCCON=21,22,23}). The average is taken over all core boundaries with {\tt BCENI=23}.
  {\tt ENIPAR(,1)} specifies the temperature in $eV$.
  \item[24] Feedback boundary condition with prescribed total ion flux, constant poloidally averaged ion temperature and a poloidal variation as close as possible to the neoclassical solution. It is recommended to use this boundary condition together with the corresponding condition on ion density ({\tt BCCON=21,22,23}). The flux is summed over all core boundaries with {\tt BCENI=24}.
  {\tt ENIPAR(,1)} specifies the energy flux in $W$.
  \item[25] constant electron temperature feedback scaled by electron temperature on the ring {\tt bc\_type21\_ref} away,
  {\tt ENEPAR(,1)} specifies the desired electron temperature in $eV$,
  {\tt ENEPAR(,2)} is the strength of the feedback.
  \item[26] Prescribe the poloidally averaged value of the ion temperature and introduce a poloidal variation in a simplified manner. This boundary condition is suitable for any plasma composition (i.e. when {\tt BCENI=23} fails). It is recommended to use this boundary condition together with a corresponding condition on the ion density ({\tt BCCON = 25,26,27}). The average is taken over all core boundaries with {\tt BCENI=26}.
  {\tt ENIPAR(,1)} specifies the temperature in $eV$.
  \item[27] Feedback boundary condition with prescribed total ion heat flux, constant poloidally averaged ion temperature and a poloidal variation in a simplified manner. This boundary condition is suitable for any plasma composition (i.e. when {\tt BCENI=24} fails). It is recommended to use this boundary condition together with a corresponding condition on the ion density ({\tt BCCON = 25,26,27}). The flux is summed over all core boundaries with {\tt BCENI=27}.
  {\tt ENIPAR(,1)} specifies the energy flux in $W$.
  \end{description}
\item[BCENI]\index{BCENI} {\tt integer array, length NBC}, specifying the
  ...
  feedback scheme.
  \item[21] from {\tt b2stbc\_spb}
  \item[22] Modified decay length for the ion temperature
...
\begin{figure}[hbtp]
\centerline{\includegraphics[height=8cm]{monoblock_cross_section}}
...
\begin{figure}[hbtp]
\centerline{\includegraphics[height=8cm]{ref_plate}}
...
\begin{equation}
E_{k,a}=\frac{m_a}2 \left[   u^2_{\parallel a}+\left( W_{a\perp}^{\left( dia \right)}+V_{a\perp}^{\left( E \times B \right)}  \right)^2    +   \left(  W_{ay}^{\left( dia \right)} + V_{ay}^{\left( E \times B \right)} \right)^2    \right]
\end{equation}
...
$n_e = \sum \limits^{ns-1}_{a=0} z_a n_a$, $n_i = \sum \limits^{ns-1}_{a=0} n_a$
...
{\em Prescribe the average value of the density:} {\tt BCCON = 25}
\begin{equation}
\left. \frac{\sqrt g}{h_x} \widetilde{\Gamma}_{ax} \right|_W
=
\left. \left[
...
{\em Imposing the average value of the density:} {\tt BCCON = 25}
...
\begin{center}\index{b2plot, Eirene quantities}
  \begin{longtable}{|l|l|}
    ...
    vxdena \index{b2plot, vxdena} & Atoms X-directed velocity $(m^2.s^{-1})$ \\
    vxdeni \index{b2plot, vxdeni} & Test ions X-directed velocity $(m^2.s^{-1})$ \\
    vxdenm \index{b2plot, vxdenm} & Molecules X-directed velocity $(m^2.s^{-1})$ \\
    vydena \index{b2plot, vydena} & Atoms Y-directed velocity $(m^2.s^{-1})$ \\
    vydeni \index{b2plot, vydeni} & Test ions Y-directed velocity $(m^2.s^{-1})$ \\
    vydenm \index{b2plot, vydenm} & Molecules Y-directed velocity $(m^2.s^{-1})$ \\
    vzdena \index{b2plot, vzdena} & Atoms Z-directed velocity $(m^2.s^{-1})$ \\
    vzdeni \index{b2plot, vzdeni} & Test ions Z-directed velocity $(m^2.s^{-1})$ \\
    vzdenm \index{b2plot, vzdenm} & Molecules Z-directed velocity $(m^2.s^{-1})$ \\
    \hline
...
Produces flow chart of Eirene array {\em vxdena} and {\em vydena} using a
maximum arrow length of $<$length$>$ meters. The use of {\em vxdena} and
{\em vydena} can be superseded by the the commands {\bf earox} and {\bf earoy}, 
