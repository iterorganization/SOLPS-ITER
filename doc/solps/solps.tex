...
$c_{th_e,a} = 0, a=0,\ldots,ns-1$ and $c_{th_i,a}=2.65, a=0,\ldots, ns-1$ if 21 moment approximation is not used.
...
$\left. \frac{\sqrt g}{h_y} \widetilde{\Gamma}_{ay} \right|_{S_1} = 0$ total particle flux (8), dpc
...
v_{big} = 10^{18},
\;\;
\left. n_a \right|_{S_1} \text{ is a given density}
\end{equation}
...
\begin{equation*}
\left. - \frac{\sqrt g}{h_y} \widetilde{\Gamma}_{ay} \right|_N = \left.
	\sum \limits_{a=0}^{ns-1} \left[
		c_{rec,a,0} c_{nsr}
		\max \left\{
...
$c_{rec,0,0} = 0,\;\; c_{rec,1,0} = 1,\;\; p_{n,0a,0} \approx 10^{21} $
...
\right] \right|_{S_0}$ weak velocity (4) dpc
...
$ \left. h_z \frac{\sqrt g}{h_y} \Gamma_{ay}^m \right|_{S_1} = \left. \left[
	p_{m,a,1} p_{m,a,2} h_z \frac{\sqrt g}{h_y} -
	p_{m,a,2} h_z \frac{\sqrt g}{h_y} V_{\parallel a}
\right] \right|_{S_1}$ \ \ \ \ \ weak velocity (4) dpc
...
\right]
\right|_W$, $p_{m,a,1}=0$, $u_{big} = 10^{18}$, $n_{e00} = 10^{20}$, dpc
...
$\left. -\frac{\sqrt g}{h_x} \Gamma_{ax}^m \right|_E = \left. \left[
	...
	u_{big} n_{e00} a_{m, a} \frac{\sqrt{g}}{h_x} V_{\parallel a}
\right]
\right|_E$, $p_{m,a,1}=0$, $u_{big} = 10^{18}$, $n_{e00} = 10^{20}$, dpc
...
$\left. \frac{\sqrt{g}}{h_y} j_y \right|_{S_0} = 0$, dpc
...
\frac{\partial}{h_x \partial x}
\left( \frac{1}{B^2} \right)
\sum \limits^{ns-1}_{\substack{a = 1, \\ z_a \neq 0}} m_a n_a \left\langle
	V^2_{a \parallel}
\right\rangle,
\end{equation*}
...
\right] \right|_{S_0}$, bcene=11

$c_{h_e,2} = -10^{-4}$,  $c_{h_e,7} = -1$,  $c_{h_e,2} \rightarrow p_{h_e,2} = -10^{-4}$,  $c_{h_e,7} \rightarrow p_{h_e,1} = -1$


$\left. -\frac{\sqrt g}{h_y} k_{ey} \frac{\partial T_e}{h_y \partial y} \right|_{S_0} = \left. \left[ \left( p_{h_e,1} \frac{\sqrt g}{h_y}\sqrt{\frac{T_e}{m_e}} \right) T_e n_e \right] \right|_{S_0}$, bcene=22
...
$\left. \frac{\sqrt g}{h_y} k_{ey} \frac{\partial T_e}{h_y \partial y} \right|_N = \left. \left[ \left( p_{h_e,1} \frac{\sqrt g}{h_y}\sqrt{\frac{T_e}{m_e}} \right) T_e n_e \right] \right|_N$, bcene=22

BCENE=9 (electron temperature decay length) $p_{h_e,1} = 10^{-2}$
...
\subsubsection{Energy balance equation for ions}

$S_{h_i} = S_{h_i,0} + S_{h_i,1} T_i + S_{h_i,2} n_i + S_{h_i,3} n_i T_i$

\paragraph{South. Private region}

$p_{h_i,1} = 10^{-2}$, $e_{rec,0} = 0$, $e_{rec,1} = 0.3$, $c_{rec,0,0} = 0$, $c_{rec,1,0} = 1$

BCENE=9 (ion temperature decay length)
...
p_{h_i,1} \sum \limits^{ns-1}_{\substack{a = 0, \\ z_a \neq 0}} \left(
	\frac{\sqrt g}{h_y} n_a \sqrt{\frac{z_a T_e + T_i}{m_a}}
\right) T_i \right|_{S_0}$, bceni=22
...
$u_{u_{out}} = p_{m,a_{main},1} c_s |b_x|$, $c_s = \sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_a}{\sum \limits_{a=0}^{ns-1} m_a n_a}}$, $p_{m,a_{main},1} = 1.0$, \\\\ $p_{m,a_{main},1} = 1.0$
