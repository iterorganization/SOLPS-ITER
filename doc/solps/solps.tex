...
When referring to \SOLPSITER in scientific papers, please use \cite{WiesenPSI2014,BONNINPET15} and references therein. You will also find these references in the \$\verb!{SOLPSTOP}/doc! directory.
...
\begin{description}\index{Compiling the code}\index{B2.5, compiling}\index{SOLPS-ITER, compiling}\index{Eirene, compiling}\index{DivGeo, compiling}\index{Carre, compiling}
  ...
  \item[b25\_all] The \verb!B2.5! stand-alone plasma fluid solver and accompanying programs only, plus \verb!b2xd! the \SOLPSfive to \SOLPSfour converter
  ...
  \item[b25eirene\_all] The \verb!B2.5-Eirene! main code and accompanying programs only, plus \verb!b2xd! the \SOLPSfive to \SOLPSfour converter
  ...
  \item[local] Creates the local directories in \verb!B2.5! and \verb!Eirene! used for local code modifications
...
On the \SOLPSITER web site (\href{https://portal.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}{https://portal.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}), in the folder {\em Examples}, one can find a few example cases for \SOLPSITER runs, in both \verb|B2.5| stand-alone or \verb!B2.5-Eirene! coupled modes.
...
\item \verb!PERFMON!\index{PERFMON}, an environment variable used by
...
at the level \$\verb!{SOLPSTOP}/data.local! is also still available.
...
Contains various \verb!Matlab! routines provided by users for data analysis and post-processing.
...
rm GNUPLOT_COUNT gnuplot.*data
    ...
    \item[GNUPLOT\_TMP]\index{GNUPLOT\_TMP} causes the \verb!gnuplot.cmd!,
    \verb!gnuplot.data! and other temporary files to be written to the directory
    specified by this environment variable. Works for most scripts.
    Also moves the location of the \verb!GNUPLOT_EXTRA! and
...
negative quantities on the same scale and "M" the absolute values of negative quantities in a separate plot)
  ...
  \item[tflux \#1]\index{tflux} Gives the recycling fluxes for the first \#1 strata from \verb!b2.neutrals.parameters!. The default value is all the strata, as read from the \verb!b2.neutrals.parameters! file, or if not present, the \$\verb!SOLPSTOP/modules/B2.5/src/include.local/DIMENSIONS.F! file, or if not present, the \$\verb!SOLPSTOP/modules/B2.5/src/include/DIMENSIONS.F! file.
...
\item Others
  ...
  \item[b2run]\index{b2run} Shortcut to the \verb!Makefile! launching \verb!B2.5!. A "-n" option allows for dry-run tests. A "-d" option allows to run within the chosen debugger. A "-s" option allows to run \verb!B2.5! in stand-alone mode. A "-m"option allows to run with MPI options. (See \ref{b2run} for fuller details)
  ...
  \item[get\_git\_email]\index{get\_git\_email} Finds the user's email address from the \verb!.gitconfig! file.
  ...
  \item[import\_files\_from\_convert]\index{import\_files\_from\_convert}\index{b2yt} Imports and renames the files produced by \verb!b2yt! in a \verb!convert! directory at the same level as the current directory to prepare the new converted case for a run
  ...
  \item[str2ogr]\index{str2ogr} Converts a .str structure file into a \verb!DG! .ogr template file.
...
If one is running \verb!B2.5! in stand-alone mode, it is recommended to set the environment variable:
\begin{verbatim}
...
\begin{description}
  ...
  \item[-m $\backslash$"MPI-launcher + options$\backslash$"] Allows to launch \verb!b2XX! in MPI
  ...
  \item[-d "debugger\_name"] Allows to run \verb!b2XX! with the named debugger (example: \verb!b2run -d totalview b2mn!). If not debugger name is specified, the debugger defined with the DBX environment variable and specified in the \$\verb!{SOLPSTOP}/SETUP/setup.csh.!\$\verb!{HOST_NAME}.!\$\verb!{COMPILER}.local! or \$\verb!{SOLPSTOP}/SETUP/setup.csh.!\$\verb!{HOST_NAME}.!\$\verb!{COMPILER}! files will be used.
\end{description}

\index{SOLPSTOP file}
The use of \verb!b2run! allows for the run to be launched
in a file space that is outside the \$\verb!{SOLPSTOP}! tree, provided there is a file named
\verb!SOLPSTOP! in the run directory or above that contains the path to the \$\verb!{SOLPSTOP}! tree.
If the file named \verb!SOLPSTOP! is missing, the script \verb!QSUB.*! will construct \$\verb!{SOLPSTOP}! by
truncating the path to actual run directory \$\verb!{SOLPSTOP}/runs/<RUN_DIRECTORY>! to \$\verb!{SOLPSTOP}!.
...
in the \$\verb!{SOLPSTOP}/runs! directory (\verb!QSUB.*! files). Currently, the following commands
are provided:
\begin{description}
...
\end{description}
Users on other systems are encouraged to recopy these command scripts (found in \$\verb!SOLPSTOP\scripts!)
and adapt them to their local environment, along
with the needed modifications to the \$\verb!{SOLPSTOP}\runs\QSUB.*! actual submission scripts.
Each submission command file can handle five possible cases:
\begin{description}\index{QSUB files}
  \item{QSUB.*} Runs a stand-alone \verb!B2.5! case.
  \item{QSUB.*-gz} Runs a stand-alone \verb!B2.5! case where the output log file has been compressed (activated with the "-z" option).
  ...
  \item{ } Any otherwise named file specified by the user and given as an argument to the submission command (in which case the "-z" option is ineffective). The specified file must be located in the \$\verb!{SOLPSTOP}\runs! directory, or in the actual run directory on one above it.
\end{description}
where "*" stands for "iter", "itm", "jet", "lsf", or "sge", referring to the machine system on which the
submission script is to be used. The "\_coupled" scripts are used when the presence of an \verb!Eirene!
\verb!input.dat! input file is detected in the run directory.
All the submission scripts make use of \verb!b2run! so the same possibility of submitting runs from
outside the \$\verb!SOLPSTOP! file space is available.
...
\end{verbatim}
from your current run directory. This script climbs up the tree looking for \verb!<<Submission_file>>!,
...
If \verb!<<Submission_file>>! is not specified, \verb!itersubmit! looks for \verb!QSUB.iter_coupled! or
for \verb!QSUB.iter! depending on whether a coupled or a standalone run is required. To request a stand-alone run explicitly, the user can use the "-s" option. Otherwise, the script will assume a coupled run is requested if an \verb!Eirene! \verb!input.dat! file is present in the run directory, and a stand-alone run otherwise.
...
\end{verbatim}
The launching program is identified by the variable \$\verb!MPI_EXEC! inside the submission file, and options can be by hand as well using the \$\verb!MPI_OPTS! variable.

The \verb!QSUB.iter! script first of all changes directory to \$\verb!{SOLPSTOP}!
with the help of \verb!SOLPSTOP! file
and executes \verb!setup.csh! there. This is necessary
...
Then the \verb!QSUB.iter! script returns back to the current run directory
and executes the script \verb!b2run!.
...
\item["strahl"]\index{STRAHL} A collisional-radiative package developed at IPP--Garching \cite{BEHRINGER87A}, stored in the \verb!stra.dat!\index{stra.dat} file
...
\begin{verbatim}
    b2run b2yr
\end{verbatim}
will produce a \verb!b2yr.plt! file containing a series of
...
The \verb!b2yr.plt! file is an NCAR GKS file. The presence in the run directory of a file called \verb!ncar.cfg!, with the
...
the \verb!b2yt! converter is provided. See Section~\ref{sec:b2yt} for details
on how to use it.
...
\end{verbatim}
at the end of the \verb!b2ai.dat! file, to obtain a new starting point file
which will include the data from the already computed solution in the locally
present \verb!b2fstate! file, and to which are added fields corresponding to
a flat state for new species declared in the species list from \verb!b2ai.dat!.
...
\item[BCCON]\index{BCCON} \verb!integer array, length NS * NBC!, specifying
  ...
  \verb!CONPAR(,,1)! specifies the gradient length in $m$
  \item[16] particle density to achieve specified total flux, used with ASTRA coupling. The total desired flux is summed over all \verb!BCCON=16! core boundaries.
  \verb!CONPAR(,,1)! specifies the desired particle flux in $s^{-1}$.
  \item[18] prescribe total main ion particle flux, used with ASTRA coupling.
  \item[19] particle flux feedback boundary condition, flux is summed over neutrals and ions, for coupling with ASTRA. The total desired flux is summed over all \verb!BCCON=19! core boundaries.
  \verb!CONPAR(,,1)! specifies the desired particle flux in $s^{-1}$.
  \item[20] constant density feedback condition,
  \verb!CONPAR(,,1)! specifies the desired density in $m^{-3}$.
  \item[21] prescribe the value of the density and add a density perturbation to get a solution which is as close as possible to neoclassical theory. It is recommended to use this boundary condition together with corresponding condition on ion temperature (\verb!BCENI=24!). The total desired density is summed over all \verb!BCCON=21! core boundaries.
  \verb!CONPAR(,,1)! specifies the desired density in $m^{-3}$.
  \item[22] Feedback boundary condition: given total particle flux with constant average density. The poloidal density variation is added to make the solution as close as possible to the neoclassical value derived for pure H/D/T plasma. It is recommended to use this boundary condition with corresponding boundary condition on ion temperature (\verb!BCENI=23,24!). The total desired flux is summed over all \verb!BCCON=22! core boundaries.
  \verb!CONPAR(,,1)! specifies the desired particle flux in $s^{-1}$.
  \item[23] Feedback boundary condition: given sum of integrated neutrals and main ion particle fluxes with constant average density. The poloidal density variation is added to make the solution as close as possible to the neoclassical value derived for pure H/D/T plasma. It is recommended to use this boundary condition with corresponding boundary condition on ion temperature (\verb!BCENI=23,24!). The total desired flux is summed over all \verb!BCCON=23! core boundaries.
  \verb!CONPAR(,,1)! specifies the desired particle flux in $s^{-1}$.
  \item[24] constant density feedback scaled by density on the ring \verb!bc_type21_ref! away
  \verb!CONPAR(,,1)! specifies the desired density in $m^{-3}$.
  \verb!CONPAR(,,2)! is the strength of the feedback
  \end{description}
\item[BCMOM]\index{BCMOM} \verb!integer array, length NS * NBC!, specifying
  ...
  \item[12] Condition from \verb!b2stbc_spb! for the parallel momentum
  \item[13] sheath boundary condition from \verb!b2stbc_spb! for the parallel momentum
  \item[14] Condition from \verb!b2stbc_spb! for the parallel momentum
  ...
  \verb!MOMPAR(,,1)! specifies the parallelv elocity in $m.s^{-1}$
  \end{description}
\item[BCENE]\index{BCENE} \verb!integer array, length NBC!, specifying
  ...
  \item[16] Feedback boundary condition with constant temperature,
    ...
    feedback. Also see type [17] below. Available if \verb!bcene_16_style=0! (default).
    If \verb!bcene_16_style=1!, integrated electron heat flux with constant $T_e$, summed over all core boundaries with \verb!BCENE=16!.
...
\item[BCENI]\index{BCENI} \verb!integer array, length NBC!, specifying the
  ...
  \item[16] Feedback boundary condition with constant temperature,
    ...
    should be something like \verb!0.1! and specifies the strength of the
    feedback. Also see type [17] below. Available if \verb!bceni_16_style=0! (default).
    If \verb!bceni_16_style=1!, integrated ion heat flux with constant $T_i$, summed over all core boundaries with \verb!BCENI=16!.
  \item[17] Feedback boundary condition with constant shared temperature
  ...
  as per type [16] but with \verb!type20_! switches and a different
  feedback scheme.
  \item[21] from \verb!b2stbc_spb!
  \item[22] Modified decay length for the ion temperature
  \item[23] Prescribe the poloidally averaged value of the ion temperature and introduce a poloidal variation as close as possible to neoclassical solution. It is recommended to use this boundary condition together with corresponding condition on ion density (\verb!BCCON=21,22,23!). The average is taken over all core boundaries with \verb!BCENI=23!.
  \verb!ENIPAR(,1)! specifies the temperature in $eV$
  \item[24] Feedback boundary condition with prescribed total ion flux, constant poloidally averaged ion temperature and a poloidal variation as close as possible to neoclassical solution. It is recommended to use this boundary condition together with corresponding condition on ion density (\verb!BCCON=21,22,23!). The flux is summed over all core boundaries with \verb!BCENI=24!.
  \verb!ENIPAR(,1)! specifies the energy flux in $W$
  \item[25] Constant temperature feedback scaled by temperature on the ring \verb!bc_type21_ref! away
...
\item[BCPOT]\index{BCPOT} \verb!integer array, length NBC!, specifying the
  ...
  \item[11] sheath conditions, electron energy transmission from \verb!b2stbc_spb!
  \verb!POTPAR(,2)! specifies the bias potential in $V$
  \item[12] Imposes the currents due to drifts for the South core boundary. Must be used in conjunction with \verb!istyle_cur_contr_on_S_and_N=2!
  \item[13] Imposes the currents due to drifts for the South private flux and North boundaries. Must be used in conjunction with \verb!istyle_cur_contr_on_S_and_N=2!
  \item[16] Feedback condition of constant electric potential for a given prescribed current. The prescribed current (in $A$) is given by the sum of the \verb!POTPAR(,1)! of all the boundary conditions with \verb!BCPOT=16!. Still experimental, will not work for drift cases.
  ...
  \end{description}
\item[CONPAR]\index{CONPAR} \verb!real array, length NS * NBC * 3!, supplies
...
to \verb!ns! now runs from \verb!0! to \verb!ns-1!. \SOLPSITER uses the same
...
\end{description}
The \verb!fort.44.2! file that the converter will create will follow the \SOLPSITER
format.

One can also use \verb!b2yt! to flip the sign of the poloidal and/or toroidal components
...
to obtain a temporary file \verb!b2frates!. After \verb!b2yt! has been run successfully,
...
\verb!import_files_from_convert!\index{import\_files\_from\_convert},
that will take the converted files from \verb!b2yt! and
place them, after proper renaming, in the current run directory,
provided the \verb!convert! directory where \verb!b2yt!
...
It is also possible to use the \verb!b2co! utility {\em while} \verb!SOLPS! is running, in order to obtain a \verb!b2fstate! file in the run directory that one may use to plot certain plasma quantities and monitor the current status of the run.
...
to the \verb!Eirene! sources. The user may want to activate one of these schemes,
although there is no firm recommendation as to which will perform best. The switches
for those are:
\begin{description}
  \item[eirene\_neutr\_avg] The neutral sources are averaged over the last up to $neutr\_avg$ \verb!Eirene! calls. After $neutr\_avg$ calls, the history is cleared and the average arrays are rebuilt progressively again (ratcheting average scheme).
  ...
  \item[b2stbr\_eir\_src\_nhist] The neutral sources are averaged over the last $nhist$ \verb!Eirene! calls (moving average scheme).
\end{description}
...
If \verb!DivGeo! is not yet compiled, you will need to do \index{DivGeo, compiling}
\begin{verbatim}
  cd $DG
  gmake tags
  gmake listobj
...
Inserted into \verb!b2stbc_phys! for \verb!BCMOM=12! as  $c_{m,a,3} \rightarrow p_{m,a,2} = -10^{10},c_{m,a,6} \rightarrow p_{m,a,1} = -1$
...
Inserted into \verb!b2stbc_phys! for \verb!BCMOM=12! as $c_{m,a,3} \rightarrow p_{m,a,2} = -10^{10},\;c_{m,a,6} \rightarrow
p_{m,a,1} = 0$
...
$<$ig$>$ = JKLM. If M \textgreater 0, then the file \verb!radsrc.b2e! is produced,
...
If J \textgreater 0, then the codes produces a set of \verb!wlld_*.dat!,
...
from the fluid species followed by \verb!B2.5!. If you wish to also
include the radiation emanating from the neutral kinetic species computed
by \verb!Eirene!, you need to add contributions from {\bf neutrad} as well.
...
additional surfaces to be handled by \verb!DivGeo!
...
The current default settings of \verb!DIMENSIONS.F!, corresponding to a D+He+C DN case, are:
