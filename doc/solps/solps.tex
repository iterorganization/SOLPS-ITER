...
{\em parallel} velocity in {\tt B2.5}.
  ...
  \item Classical transport in {\tt B2.5} follows the Balescu treatment by default, while it uses
...
= \mbox{old form} = S_{a \parallel}^m + S_{CF_a}^m + S_{fr_a}^m + S_{Therm_a}^{m} + S_{I_a}^m + S_{R_a}^m + S_{CX_a}^m
\end{eqnarray*}
...
\begin{equation*}
\Gamma_{ex} = \sum \limits_{a=0}^{ns-1} z_a \Gamma_{ax}^{(he)} - \frac{j^{\parallel}_x} e
\end{equation*}
...
$z_{eff} = \frac{\sum \limits_{a=0}^{ns-1} z_a^2n_a}{n_e}$,
...
$j^{(\parallel)}_x = \sigma_{\parallel} \left( \frac{1}{e n_e h_x} \frac{\partial n_e T_e}{\partial x} - \frac{1}{h_x} \frac{\partial \Phi}{\partial x} \right) - \alpha_{ex} \frac{1}{h_x} \frac{\partial T_e}{\partial x}$,
