...
\${\tt \{SOLPSTOP\}/scripts} directory. The files described in this paragraph
...
\item[MARLOWE] \index{MARLOWE} is an {\tt Eirene} input file containing
  surface data
...
\item[SPUTER] \index{SPUTER} is an {\tt Eirene} input file containing
  sputtering data
\item[STRAHL] \index{STRAHL} is a {\tt B2} and {\tt B2.5} input file containing atomic
  physics rates
...
Contains atomic and molecular data necessary for {\tt DG}.
...
This directory is now located in {\tt modules/B2.5/Database}.
...
\begin{description}
  ...
  \item[no\_pflux]\index{no\_pflux} is a logical that, if {\tt .true.}, does not apply these
...
This routine first reads the formatted file {\tt b2ymb.dat} and writes the unformatted file {\tt mnb\_in.dat} that is needed by the {\tt FreeFem++}(\href{http://www.freefem.org}{http://www.freefem.org})\cite{MR3043640} heat transfer model of the monoblock.
...
This routine first reads the formatted file {\tt b2yrp.dat} and writes the unformatted file {\tt rps\_in.dat} that is needed by the {\tt FreeFem++}(\href{http://www.freefem.org}{http://www.freefem.org})\cite{MR3043640} heat transfer model of the reflector plates.
...
=
\left. \left[ \left.
p_{n_a,a,1} \frac{\sqrt g}{h_y} \sqrt{\frac{z_a T_e + T_i}{m_a}} n_a \right|_N
+
...
$\left. \frac{\sqrt g}{h_x} \widetilde q_{ex} \right|_W = \left. \left[
	-\left(
		\frac{1 + \gamma}{1 -\gamma} T_e + e\Phi
	\right)
	\max \left(
...
$\left. \frac{\sqrt g}{h_x} \widetilde q_{ex} \right|_W = \left. \left[
	-\left(
		\frac{1 + \gamma}{1 -\gamma} + \frac{e\Phi}{T_e}
	\right)
	\max \left(
		0,\frac{\sqrt g}{h_x} |b_x| n_e
		\sqrt{\frac{\sum \limits_{a=0}^{ns-1} p_a}{\sum \limits_{a=0}^{ns-1} m_a n_a}}
...
$\left. \frac{\sqrt g}{h_x} q_{ex} \right|_W = \left[
	p_{h_e,1} + e\frac{\Phi - p_{\Phi,2}}{T_e}
\right]
\frac{\sqrt g}{h_x} \nu_{e_{out}} T_e
- \left.
\frac{\sqrt g}{h_x} q_{ex}^{P.Sch}
...
$\left. -\frac{\sqrt g}{h_x} \widetilde q_{ex} \right|_E = -\left[
	p_{h_e,1} + e\frac{\Phi - p_{\Phi,2}}{T_e}
\right]
\frac{\sqrt g}{h_x} \nu_{e_{out}} T_e
+ \left.
...
\begin{center}
    ...
    \hline \endfoot
    \endlastfoot
fn3dl \index{fn3dl}\index{b2time.nc, fn3dl} & poloidal particle flux, inboard divertor & $s^{-1}.m^{-2}$ \\
...
fn3dr \index{fn3dr}\index{b2time.nc, fn3dr} & poloidal particle flux, outboard divertor & $s^{-1}.m^{-2}$ \\
...
\begin{description}
  ...
  \item[ig = -18] Produces input files for a DIVIMP run (not yet available). Requires one field on the matrix stack and one filename on the string stack to indicate which field is to be produced and where it is to be written
