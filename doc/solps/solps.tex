...
\item {\tt NETCDF\_ROOT}\index{NETCDF\_ROOT}, the root of the {\tt NetCDF} tree, and
...
\end{enumerate}
Of course, any additional desired definitions can be added. The preferred place for those is, for those variables that need not be shared by other users, the \${\tt \{SOLPSTOP\}/SETUP/setup.csh.}\${\tt \{HOST\_NAME\}.}\${\tt \{COMPILER\}.local} file.
...
the output from \SOLPSfive to an {\tt MDSplus} server in Garching.  (For more
...
In order to compile the code with {\tt MDSplus}, the environment variable
{\tt SOLPS\_CPP}\index{SOLPS\_CPP} should contain
{\tt -DMDSPLUS}\index{-DMDSPLUS}, and a
\begin{verbatim}
...
\end{verbatim}
to one of the {\tt setup.csh.*} files in \${\tt SOLPSTOP/SETUP}.]

{\tt b2md} requires as input {\tt b2md.dat} and this is searched for in the
present working directory, the {\tt baserun} directory, and in the
\${\tt SOLPSTOP/data.local/mdsplus} and \${\tt SOLPSTOP/data/mdsplus} directories,
...
\${\tt SOLPSTOP/data.local/commands} and
...
{\tt B2.5} and {\tt Eirene} source codes.
...
\item[USE\_PXFGETENV] \index{USE\_PXFGETENV} Uses {\tt PXFGETENV}
  instead of {\tt GETENV}
\end{description}
...
\item[CDF\_MOVIE] \index{CDF\_MOVIE} To be used if {\tt CDF} movie output is desired
...
\item[MDSPLUS] \index{MDSPLUS} Enables {\tt MDSplus} output
\item[NAG22] \index{NAG22} Specific to using the Nag Mark 22 or higher mathematical library
\item[NAGF95] \index{NAGF95} Specific to the {\tt NAG} F95 compiler
...
\item[NO\_F01AAF] \index{NO\_F01AAF} No {\tt NAG} routine {\tt F01AAF} --- uses
  its replacement instead (needed if linking with Version 17 or later of {\tt NAG})
...
\item[WOS] \index{WOS} Uses the {\tt WOS} libraries for accessing
...
There are currently no additional compiler options exclusive to {\tt Eirene}, but we list below where the joint {\tt B2.5-Eirene} switches are used.
