...
We list here the possible compiler options that can be used when compiling {\tt Uinp}.
The options below must therefore, if desired, be declared as part of the \${\tt SOLPS\_CPP} environment variable, which is defined in the
\${\tt SOLPSTOP/SETUP/config.}\${\tt {HOST\_NAME}.}\${\tt {COMPILER}(.local)} files.
