...
If your specific domain name is not on this list, please add it to {\tt whereami}.
...
\begin{description}
  ...
  \item[tib2] Test ion temperature ($m^{-3}$), dimension ({\tt NDX},{\tt NDY},{\tt NION})
  ...
  \item[wlabsrp(A)] Absorption rate for atoms, dimension ({\tt NLIM}+{\tt NSTS}, {\tt NATM})
  \item[wlabsrp(M)] Absorption rate for molecules, dimension ({\tt NLIM}+{\tt NSTS}, {\tt NMOL})
  \item[wlabsrp(I)] Absorption rate for test ions, dimension ({\tt NLIM}+{\tt NSTS}, {\tt NION})
  \item[wlabsrp(P)] Absorption rate for plasma ions, dimension ({\tt NLIM}+{\tt NSTS}, {\tt NPLS})
  \item[wlpump(A)] Pumped flux per atom ($A$), dimension ({\tt NLIM}+{\tt NSTS}, {\tt NATM})
  \item[wlpump(M)] Pumped flux per molecule ($A$), dimension ({\tt NLIM}+{\tt NSTS}, {\tt NMOL})
  \item[wlpump(I)] Pumped flux per test ion ($A$), dimension ({\tt NLIM}+{\tt NSTS}, {\tt NION})
  \item[wlpump(P)] Pumped flux per plasma ion ($A$), dimension ({\tt NLIM}+{\tt NSTS}, {\tt NPLS})
...
\begin{equation}
\begin{matrix}
\widetilde{ \Gamma}_{ax} =& \left( b_x V_{\parallel a} + V_{ax}^{\left( E \right)} + V_{ax}^{\left( AN \right)} + V_a^{corr\_dpc} \right) n_a + 0 \cdot \frac{\delta_{1,a}}{e} \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} \right) \\
...
\begin{equation}
\begin{matrix}
\widetilde{ \Gamma}_{ay} =& \left( V_{ay}^{\left( E \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} \right) \\
...
0 \cdot \frac{\delta_{1,a}}{e} & \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_x} \frac{\partial n_a}{\partial x} - \widetilde{D}_{p,ax} \frac{1}{h_x} \frac{\partial p_a}{\partial x}
\end{matrix}
\end{equation}

\begin{equation}
\begin{matrix}
\Gamma_{ay} =& \left( V_{ay}^{\left( E \right)} + \widetilde{V}_{ay}^{\left( dia \right)} + V_{ay}^{\left( AN \right)} \right) n_a + \frac{\delta_{1,a}}{e} \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} \right) \\
...
\frac{\delta_{1,a}}{e} & \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_y} \frac{\partial n_a}{\partial y} - \widetilde{D}_{p,ay} \frac{1}{h_y} \frac{\partial p_a}{\partial y}
\end{matrix}
\end{equation}
...
0 \cdot \frac{\delta_{1,a}}{e} & \left( j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + \widetilde{j_x}^{\left( vis \perp \right)} + 0 \cdot \widetilde{j_x}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_x} \frac{\partial n_a}{\partial x}
\end{matrix}
\end{equation}
...
+ \frac{\delta_{1,a}}{e} & \left( j_y^{\left( AN \right)} + j_y^{\left( in \right)} + \widetilde{j_y}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_y}^{\left( vis \perp \right)} + \widetilde{j_y}^{\left( visq \right)} \right) - D_{n,a} \frac{1}{h_y} \frac{\partial n_a}{\partial y}
\end{matrix}
\end{equation}
...
\begin{equation}
E_{k,a}=\frac{m_a}2 \left[   V^2_{\parallel a}+\left( W_{a\perp}^{\left( dia \right)}+V_{a\perp}^{\left( E \times B \right)}  \right)^2    +   \left(  W_{ay}^{\left( dia \right)} + V_{ay}^{\left( E \times B \right)} \right)^2    \right]
\end{equation}
...
\begin{equation*}
\widetilde f_{ix} = \frac 3 2 \Gamma_{ix} + V_{ix}^{(str)} n_i + 0 \cdot \frac{3}{2} \left( \frac{j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)}}{e} \right)
\end{equation*}
...
q_{ix} = \left[
	...
	V_{ix}^{(str)} n_i +
        0 \cdot \frac{3}{2} \left( \frac{j_x^{\left( AN \right)} + j_x^{\left( in \right)} + \widetilde{j_x}^{\left( vis \parallel \right)} + 0 \cdot \widetilde{j_x}^{\left( vis \perp \right)} + \widetilde{j_x}^{\left( visq \right)}}{e} \right)
\right] T_i
...
\widetilde f_{iy} = \frac 3 2 \Gamma_{iy} + V_{iy}^{(str)} n_i + \frac 3 2 \frac{\left(
	j_y^{(AN)} + \widetilde j_y^{(vis \parallel)} + 0 \cdot \widetilde j_y^{(vis \perp)} + \widetilde j_y^{(visq)} + j_y^{(in)}
\right)} e
\end{equation*}
...
containing the radiation sources (in $MW/m^2$). If L \textgreater 0, then the
...
\begin{description}
  ...
  \item[0 < ig $\le$ nx] Produces a file {\tt oleg.dat} with plasma parameters (density, temperature, parallel flow velocity, parallel heat flux) against the distance along the magnetic field along flux surface index $\langle$ig$\rangle$
  \item[nx < ig < 1000] Produces two files with estimated helium concentration ({\tt fraction.he}) and neutral pressure ({\tt pressure.ntr}) in the PFR for summary plots
  \item[ig > 1000] Adds a line to the file {\tt ../sum\_tmp/spec\_pwr.dat} with the power entering both divertors, power radiated along the divertor legs, power radiated near the targets, areas of the separatrix branches and mean specific power radiated along the divertor legs per square meter. $\langle$ig$\rangle$ -1000 cells are treated as "close to the targets"
\end{description}
