...
\ifpdf
\usepackage{thumbpdf}
\usepackage[pageanchor,pdftex,backref,hyperindex,colorlinks,bookmarks,bookmarksnumbered,pdfencoding=auto]{hyperref} %bookmarksopen,
\else
...
\begin{description}
  ...
  \item[ITER\_2588\_D+He+N] ITER full power case with N injection
...
\item Others
  ...
  \item[gtdr]\index{gtdr} Script to make a directory tree local. [obsolete]
  \item[gtfl]\index{gtfl} [ANK] Replaces the specified links by the files they are linked to. [obsolete]
  ...
  \item[update\_eirene]\index{update\_eirene} Updates the \${\tt SOLPSTOP/modules/Eirene} directory.
  \item[update\_libs]\index{update\_libs} Updates the \${\tt SOLPSTOP/src/lib} directory.
  \item[update\_links]\index{update\_links} Updates linked files to the {\tt SOLPS} Master in the current directory structure.
...
These are the tallies stored in {\tt b2tallies.nc}.
