...
\item Ad-hoc radial plasma boundary conditions (leakage / decay lengths) are no longer needed.
\item Fluid neutrals can be modelled further down in the divertor, closer to where the pumps are typically located. 
(or at least to where the pumps are located in {\tt B2.5-EIRENE} simulations.)
\end{itemize}

The move to the unstructured finite volume solver and extended grids is described in full in the 2021 NME paper of W. Dekeyser et al. \cite{DEKEYSER2021}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This section explains the discretization on the unstructured finite volume grid. 
The content of this section is copied from Ref. \cite{DEKEYSER2021}.
...
\begin{figure}[hbtp]
\ifpdf
    \centerline{\includegraphics[width=14cm]{{FiguresWG/mesh_angles.PNG}}
\else
    \centerline{\includegraphics[width=14cm]{FiguresWG/mesh_angles.eps}}
\fi
...
\begin{itemize}
\item {\tt $C_1$} is the center of control volume 1. It is given by {\tt fcCv(iFc,1)}.
\item {\tt $C_2$} is the center of control volume 2. It is given by {\tt fcCv(iFc,2)}.
\item {\tt $V_1$} is the first vertex of the face. It is given by {\tt fcVx(iFc,1)}.
\item {\tt $V_2$} is the second vertex of the face. It is given by {\tt fcVx(iFc,2)}. The convention is that
when we look from {\tt $V_1$} to {\tt $V_2$}, {\tt $C_1$} is on the left and {\tt $C_2$} is on the right.
...
\item {\tt $\gamma$} is the is the included angle between the the cell-connector vector and the normal vector
of face {\tt iFc}. {\tt fcQgam(iFc,0)} is the cosine of {\tt $\gamma$} and {\tt fcQgam(iFc,1)} is the sine of {\tt $\gamma$}. It will hold that 0.lt.fcQgam(,).le.1.
...
The B2.5 finite volume technique requires the evaluation of the fluxes across
cell faces:

\begin{align*}
 \mathbf{\Gamma} \cdot \mathbf{S} & = \Gamma_\theta S_\theta + \Gamma_r S_r \\ 
 & = (C_\theta u - D_{\theta \theta} \nabla_\theta u - D_{\theta r} \nabla_r u) S_\theta + 
 (C_r u - D_{r \theta} \nabla_\theta u - D_{r r} \nabla_r u) S_r,
 \end{align*}
...
\begin{equation*}
\mathbf{\Gamma}_d \cdot \mathbf{S} = - D_d (\nabla_r u S_\theta - \nabla_\theta u S_r) = \mp D_d \nabla_y u S
\end{equation*}

where we use the identity $\mathbf{S} = (\cos \alpha \ \mathbf{e}_\theta \pm \sin \alpha \ \mathbf{e}_r) S$.
Thus, net drift flow
...
\begin{itemize}
\item {\tt GRAD} computes gradients of a cell centered quantity on the cell faces, in poloidal and radial direction.
\item {\tt GRAD\_NT} computes gradients of a cell centered quantity on the cell faces, in tangential and normal direction.
\item {\tt GRADC} computes gradients of a cell centered quantity in the in cell center by first computing
the gradients on faces, and then interpolateing back to cell centers.
\item {\tt GRADC\_DIV} computes gradients of a cell centered quantity in the in cell center by 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Rhie-Chow corection in unstructured code}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
The switches {\tt b2npco\_pcm0}, {\tt b2npco\_pcm1}, {\tt b2npht\_pcm0} and {\tt b2npht\_pcm1} determine the strength of the corretions.
...
Some section of this manual still need to be rewritten to be consistent with the unstructured solver:

\begin{itemize}
\item Appendix \ref{cha:b2wdat_iout_4}
\item Appendix \ref{cha:Mladen_2}: remove, update, or integrate with another part of the manual.
\item Appendix \ref{sec:typical_BCs}
\item Add information on creation of new extended cases. For now we refer to the SharePoint presentation \newline 
{\tt SOLPS-ITER Extended Grids Workshops / 20230815\_carre2\_presentation.pdf}
\end{itemize}
...
Users can easily convert their existing structured cases to run in the WG code version. How to do this is explained in documents on the 
\SOLPSITER SharePoint website \newline 
((\href{https://sharepoint.iter.org/departments/POP/CM/IMAS/SOLPS-ITER}{https://sharepoint.iter.org\allowbreak/departments/POP/CM/IMAS/SOLPS-ITER})):
\newline
 {\tt SOLPS-ITER Extended Grids Workshops / Session\_case\_conversion} \newline 
 and the last section of the presentation 
 \newline{\tt SOLPS-ITER Extended Grids Workshops / 20230814\_extended\_grids\_presentation}.
