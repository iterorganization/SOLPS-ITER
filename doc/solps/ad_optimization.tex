...
\end{verbatim}
where {\tt set\_tao} allows to include extra compilation flags in the user's configuration files for {\tt B2.5}. An additional executable named {\tt b2optim\_tao.exe} is compiled in {\tt B2.5/builds/standalone.\allowbreak\$HOST\_NAME.\allowbreak\$COMPILER.[tgt|adj]}.
...
\end{description}
Additionally, the file {\tt PETSC-TAO.OUT} will be copied from {\tt b2mn.exe.dir} into the run directory, and will contain further details on the optimization (e.g. stopping criteria).
...
The last parameters could be omitted in a standard optimization with {\tt PETSc/TAO} BFGS and without nonlinear constraints.
