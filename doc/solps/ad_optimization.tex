...
\end{verbatim}
where {\tt set\_tao} allows to include extra compilation flags in the user's configuration files for {\tt B2.5}. Alternatively, one can do the same with command {\tt set\_ipopt}. In both cases an additional executable named {\tt b2optim\_tao.exe} or {\tt b2optim\_ipopt.exe} is compiled in {\tt B2.5/builds/standalone.\allowbreak\$HOST\_NAME.\allowbreak\$COMPILER.[tgt|adj]}.
...
\begin{description}
  ...
  \item[X0]\index{X0} {\tt real*8 array, dimension (NPAR\_OPT)}, initial guess for each optimization parameter. Must be set for MAP cost function as its value is employed for calculating priors. Defaults to $10*p_\infty$, where $p_\infty$ is a parameter currently set to $10^{30}$.
